...
\begin{enumerate}
\item Övning \code{expressions}  -- skim if too simple
\item Övning \code{programs}  -- do \code{hello world} then skim 
\item Övning \Alert{\code{functions}} -- check out Scala's functional aspects
\item Övning \code{data} -- check out Scala's basic OO aspects
\item Övning \code{sequences} -- only half-ready; try scala collections 
\end{enumerate}
