...
margin=30mm,showcrop=true]{geometry}
...
\begin{tikzpicture}[overlay,remember picture]%[overlay,remember picture]
    ...
    \node[text width=250mm, align=right] (intro) at ($(current page.north east)+(-14.5,-6.5)-(3.0,0)$) {
       \fontsize{28}{46}\sffamily\selectfont\textbf{Introduktion~~till} 
       };  
       
    \node[text width=250mm, align=right] (prog) at ($(current page.north east)+(-14.5,-8.5)-(3,0)$) {
       \fontsize{52}{46}\sffamily\selectfont\textbf{programmering} 
       };        

    \node[text width=250mm, align=right] (prog) at ($(current page.north east)+(-14.5,-24.8)-(3,4)$) {
       \fontsize{28}{46}\sffamily\selectfont{med~~Scala~~och~~Java} 
       };        
    
    \node[text width=250mm, align=right] (prog) at ($(current page.north east)+(-14.5,-26.0)-(3,4)$) {
    ...
    \node[above right] (picture) at ($(current page.south east)+(-16,7.8)-(3,0)$) {\includegraphics[scale=0.75]{gurka.jpg}};
...
\node[anchor=north,rotate=-90, align=left] (back) at ($(current page.north)+(0.0,-11.8)+(0,-2.5)$) {
\fontsize{14}{20}\sffamily\selectfont\textbf{Introduktion till programmering med Scala och Java} \hspace{3.4cm}\texttt{http://cs.lth.se/pgk}};
...
\node[above right, scale=1.3] (modulplan) at ($(current page.west)+(\LeftMarginBack,-11.0)+(2.5,0)$) {
\sffamily\input{../../plan/overview-generated.tex} 
};

\node[above right, text width=15cm,align=left] (collection-traits) at ($(current page.west)+(\LeftMarginBack,4.5)+(2.5,0)$) {
...
\node[above right, scale=1.3] (modulplan) at ($(current page.west)+(\LeftMarginBack,-13.5)+(2.5,0)$) {
