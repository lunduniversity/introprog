...
\usepackage[
layoutheight=297mm,layoutwidth=450mm,  %420+thickness
layoutvoffset=30mm,layouthoffset=30mm,
paperheight=347mm,paperwidth=510mm,
margin=20mm,includeheadfoot,showcrop=true]{geometry}
...
\begin{tikzpicture}[overlay,remember picture]%[overlay,remember picture]
    ...
    \node[text width=250mm, align=right] (intro) at ($(current page.north east)+(-14.5,-4.5)-(2,0)$) {
       \fontsize{28}{46}\sffamily\selectfont\textbf{Introduktion~~till} 
       };  
       
    \node[text width=250mm, align=right] (prog) at ($(current page.north east)+(-14.5,-6.5)-(2,0)$) {
       \fontsize{52}{46}\sffamily\selectfont\textbf{programmering} 
       };        

    \node[text width=250mm, align=right] (prog) at ($(current page.north east)+(-14.5,-26.2)-(2,4)$) {
       \fontsize{28}{46}\sffamily\selectfont{med~~Scala~~och~~Java} 
       };        
    
    \node[text width=250mm, align=right] (prog) at ($(current page.north east)+(-14.5,-27.2)-(2,4)$) {
    ...
    \node[above right] (picture) at ($(current page.south east)+(-16,5.5)-(2,0)$) {\includegraphics[scale=0.75]{gurka.jpg}};
    
    
\node[above right] (logo) at ($(current page.west)+(19.0,-20.3)+(4,-0.5)$) {\includegraphics[scale=0.54]{logoLUeng}};

    
        
\node[anchor=north,rotate=-90, align=left] (back) at ($(current page.north)+(0.0,-12.5)+(0,-2.5)$) {
\fontsize{14}{20}\sffamily\selectfont\textbf{Introduktion till programmering med Scala och Java} \hspace{2.4cm}\texttt{\small https://github.com/lunduniversity/introprog}};

\node[anchor=north] (back) at ($(current page.north)+(-0.4,-15.0)+(0,-2.5)$) {
\fontsize{16}{20}\sffamily\selectfont {\YEAR}};

\newcommand{\LeftMarginBack}{1.8}

\node[above right, scale=1.3] (modulplan) at ($(current page.west)+(\LeftMarginBack,-11.0)+(2,0)$) {
\sffamily\input{../../plan/overview-generated.tex} 
};

\node[above right, text width=15cm,align=left] (collection-traits) at ($(current page.west)+(\LeftMarginBack,4.5)+(2,0)$) {
...
\node[above right, scale=1.3] (modulplan) at ($(current page.west)+(\LeftMarginBack,-13.5)+(2,0)$) {
