...
%!TEX root = compendium/cover/cover-A3-part1.tex

\pagecolor{Background}
\color{Foreground}

\hyphenation{data-strukturer}
...
\begin{tikzpicture}[overlay,remember picture]%[overlay,remember picture]
    ...
    \node[text width=250mm, align=right] (prog) at ($(current page.north east)+(-14.5,-30.0)-(\RightMarginFront,\YPosFront)$) {
       \fontsize{18}{46}\sffamily\selectfont\texttt{https://lunduniversity.github.io/pgk}
       };
...
\node[above right, rotate=20, text=yellow] (PROVTRYCK) at ($(current page.north east)+(-19,-8)-(\RightMarginFront,\YPosFront)$){\huge \textbf{PROVTRYCK \today}};


\node[anchor=north,rotate=-90, align=left] (back) at ($(current page.north)+(0.0,-14)+(0,+2.0)$) {
\fontsize{16}{20}\sffamily\selectfont 
\bf\CompendiumTitle
...
\vspace{1em}
Kursen omfattar grundläggande programmeringsprinciper och viktiga datavetenskapliga begrepp. Du lär dig det moderna och kraftfulla programmeringsspråket \textbf{Scala}, att konstruera algoritmer, att använda datastrukturer och mycket annat. Det krävs inga förkunskaper i programmering. Kursen syftar till att ge en solid \textbf{grund för fortsatta studier} i programmering och avancerad systemutveckling.
...
\sffamily Licens: CC-BY-SA. Upplaga \CurrentYear. \\ \texttt{https://lunduniversity.github.io/pgk}
\end{minipage}
};
\else
This compendium is course literature for \textit{''Introduction to programming''} at \textbf{Lund University, Faculty of Engineering (LTH)}. The course comprises 7.5 credits and is compulsory in year 1 of the Master of Science in Engineering programmes in Computer Science and Engineering (D) and Information and Communication Engineering (C). %The course is also offered as an elective for other study programmes and as a freestanding course.

\vspace{1em}
The course covers fundamental programming principles and important computer science concepts. You will learn the modern and powerful programming language \textbf{Scala}, how to construct algorithms, how to use data structures, and much more. No prior programming knowledge is required. The course aims to provide a solid \textbf{foundation for further studies} in programming and advanced software development.

\vspace{1em}
\textbf{Welcome} to the fascinating world of computer science!
\vspace{5em}

{\Large Contents week by week:}
\end{minipage}
};

\node[above right, scale=1.3] (modulplan) at ($(current page.west)+(\LeftMarginBack,-14)+(1.0,\YPosBack)$) {
\begin{minipage}{0.33\textwidth}
\sffamily License: CC-BY-SA. Edition \CurrentYear. \\ \texttt{https://lunduniversity.github.io/pgk}
\end{minipage}
};
