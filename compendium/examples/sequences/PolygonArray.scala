class PolygonArray(val maxSize: Int):
  ...
  def draw(w: PolygonWindow): Unit = w.draw(points.take(n).toSeq)

  def append(pts: Pt*): Unit =
    for i <- pts.indices do points(n + i) = pts(i)
    n += pts.length

  def insert(pos: Int, pt: Pt): Unit = // exercise: change pt to varargs pts
    for i <- n until pos by -1 do points(i) = points(i - 1)
    points(pos) = pt
    n += 1

  def remove(pos: Int): Unit = // exercise: change pos to fromPos, replaced
    for i <- pos until n do points(i) = points(i + 1)
    n -= 1

  override def toString = points.mkString("PolygonArray(",",",")")
