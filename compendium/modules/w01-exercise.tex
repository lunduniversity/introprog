...
\Task\Pen \textit{Variabler}. Rita hur minnet ser ut efter varje rad. 
...
\Task \textit{Slumptal}. Använd en sökmotor och leta efter dokumentationen för \code{scala.math.random}.

\Subtask\Pen Undersök vad dokumentationen säher om funktionen \code{ramdom}.

\Subtask Anropa funktionen \code{math.random} upprepade gånger och notera vad som händer.
\begin{REPL}
scala> math.random
\end{REPL}
...
\Subtask Vad händer? Använd \textit{pil-upp} och kör nedan \code{while}-sats flera gånger. Förklara vad som sker.

\begin{REPL}
scala> while (math.random > 0.2) { println("gurka") }
\end{REPL}

\Subtask Ändra i \code{while}-satsen ovan så att sannolikheten ökar att fler \code{gurka}-strängar ska skrivs ut.
