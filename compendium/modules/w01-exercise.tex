...
\Subtask Evaluera uttrycket \code{res0 * 10} men byt ut \code{0}:an mot siffran efter \code{res} i utskriften från förra evalueringen. Vad har uttrycket för värde och typ?
\begin{REPLnonum}
scala> res0 * 10
\end{REPLnonum}
...
\Task Vilken typ har följande literaler? % uppgift 3
...
\Task Vilken typ och vilket värde har följande uttryck?  % uppgift 6
...
\Task \textit{Heltalsdivision}. Vilket värde och vilken typ har följande uttryck? % uppgift 9
...
\Task \textit{Booelska uttryck}. Vilket värde och vilken typ har följande uttryck?  % Uppgift 13
...
\Task \code{for}\textit{-sats}.  % Uppgift 20
...
\Task Repetition med \code{foreach}.  % Uppgift 21
...
\Task \code{while}\textit{-sats}.   % Uppgift 22
...
\Task Backticks. \code{val }\texttt{\`{}konstig val\`{} = 42}   %%%%!!! NY TASK
