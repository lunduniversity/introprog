...
\def\what{\emph{Sekvens, sats och block.}}
...
\def\what{\emph{Procedur med parameter.}}
...
\Subtask Gör en ny variant av \code{updateHighscore}, som \emph{inte} är en procedur utan i stället är en funktion som ger en sträng för senare utskrift. Testa funkiotnen i REPL.
...
\def\what{\emph{Modulo-operatorn {\tt \%} och Booleska värden.}}
...
\def\what{\emph{Skillnader mellan \code{var}, \code{val}, \code{def}.}}
...
scala> w = w + 1
<console>:12: error: value w_= is not a member of object $iw
       w = w + 1
\end{REPL}
...
\def\what{\emph{Skillnaden mellan \code{if} och \code{while}.}}
...
\def\what{\emph{Logik och De Morgans Lagar.}}
...
\def\what{\emph{Tilldelningsoperatorer.}}
...
\def\what{\emph{Dokumentation av paket i Java och Scala.}}
...
\SubtaskSolved Filtrera fram alla förekomster av en viss teckenkombination.
...
\def\what{\emph{Noggrannhet och undantag i aritmetiska uttryck.}}
...
\def\what{\emph{Modulo-räkning med negativa tal.}}

\QUESTBEGIN

\Task \what~Läs om moduloräkning här: \\
...
\def\what{\emph{Bokstavliga identifierare.}}

\QUESTBEGIN

\Task \what~Läs om identifierare i Scala och speciellt \emph{literal identifiers} här: \url{http://www.artima.com/pins1ed/functional-objects.html#6.10}.
...
\def\what{\emph{\code{java.lang.Integer}, hexadecimala litteraler, BigDecimal.}}
...
\def\what{\emph{Strängformatering.}}

\QUESTBEGIN

\Task \what~Läs om \code{f}-interpolatorn här:\\ 
...
\def\what{\emph{Multiplikationsvarning.}}
...
\def\what{\emph{Dekorera \code{Int} med extra operatorer.}}

\QUESTBEGIN

\Task \what\footnote{En utmanande överkursuppgift som visar Scalas kraftfullhet. Se fördjupningslänkar i facit.}\\Kim Kodmagiker tycker att \code{Math.multiplyExact} är för krångligt att skriva och utökar därför typen \code{Int} med en extra operator:
...
\Subtask Klistra in dekoratorklassen ovan i REPL och prova den extra operatorn.

\Subtask Hjälp Kim Kodmagiker att lägga till fler metoder i dekoratorklassen ovan som gör att det även går att använda \code{Math.subtractExact} och \code{Math.addExact} smidigt.

\Subtask Testa ett sammansatt uttryck som använder alla extrametoder på \code{Int}. Tycker du det blev mer lättläst eller mer kryptiskt med de nya metoderna?
