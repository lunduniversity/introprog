...
\TODO{Flytta del av kojo till övning, tex def kvadrat, se Sandras kommentarer
\url{https://git.cs.lth.se/courses/pgk/pgk-solutions/blob/master/TODO.md}
}

\ifPreSolution  %%%% BEGIN TASK <<<<<<<<<<<<<<<<<<<<<<<<<<<<

\Task \emph{Skriva satser i REPL. Strängkonkatenering. Körtidsfel. Kompileringsfel.}
...
\Subtask Skriv en sats efter prompten \code{scala>} som skriver ut en valfri hälsningsfras och tryck på \textit{Enter} så att satsen exekveras. 
...
\Subtask Skriv ett uttryck som konkatenerar orden \code{gurka} och \code{tomat} med hjälp av operatorn \code{+} på strängar. Vad har uttrycket för värde och typ? Vilken siffra står efter ordet \code{res} i variabeln som lagrar resultatet?

\Subtask Använd resultatet från konkateneringen, t.ex. \code{res0} (byt ev. ut \code{0}:an mot siffran efter \code{res} i utskriften från förra evalueringen), och skriv ett uttryck med hjälp av operatorn \code{*} på strängar som upprepar ordet \code{gurkatomat} 42 gånger. 

\Subtask Välj ett värde på argumentet efter operatorn \code{*} så att resultatsträngen tar så mycket plats att minnet blir fullt. Hur börjar felmeddelandet? Är detta ett körtidsfel eller ett kompileringsfel?

\Subtask Välj ett värde på argumentet efter operatorn \code{*} av en typ som gör att ett typfel genereras. Hur börjar felmeddelandet? Är detta ett körtidsfel eller ett kompileringsfel?

\else %%%%%%%%% BEGIN SOLUTION

\Task 

\Subtask till exempel:
\begin{REPLnonum}
scala> println("hejsan svejsan")
\end{REPLnonum}

\Subtask man får fortsätta skriva på nästa rad; detta indikeras med ett vertikalstreck:
\begin{REPLnonum}
scala> println("hejsan svejsan"
     | + "!" 
     | )
...
\begin{REPLnonum}
scala> "gurka" + "tomat"
res1: String = gurkatomat
\end{REPLnonum}
värde: \code{gurkatomat}, typ:  \code{String}
