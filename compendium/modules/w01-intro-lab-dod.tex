...
\begin{Kontrollfragor}
  Du borde kunna svara på de flesta av dessa frågor innan du börjar labba, för att få ut så mycket som möjligt av laborationen.

  \item Hur ser kommandot ut som loggar in på en annan dator via nätet (t.ex. om du vill logga in på skolan hemifrån)?
  ...
  \item Hur visar man vilka filer som finns i den aktuella katalogen?
  ...
  \item Antag att programmet \code{prog} producerar många sidor utskrift. Hur gör man för att titta på utskriften en sida i tage e?
...
\item Editering av kommandoraden och enkla Linux-kommandon.

\begin{Deluppgifter}
\item Öppna ett kommandofönster (Terminal).
\item Skriv några enkla Linux-kommandon, till exempel \code{pwd}, \code{ls}, \code{date} och \code{cal}.
...
\begin{Code}[language=bash,keywords=]
...
\url{https://fileadmin.cs.lth.se/pgk/workspace.zip} i katalogen \code{w01_linux}.\\
...
# ladda ned och packa upp filerna och gå till katalogen
curl -O https://fileadmin.cs.lth.se/pgk/workspace.zip
unzip workspace.zip
cd workspace
\end{Code}
...
\item Gå till katalogen \code{w01_linux/linux-lab-fs/projects/beta/data} och skriv ut en innehållsförteckning över katalogen. Skriv ut en förteckning över de filer vars namn innehåller strängen \code{report}.
...
\item Tag reda på hur stor filen \file{example.txt} i \file{workspace} är. Komprimera därefter filen och tag reda på storleken hos den komprimerade filen. Återställ sedan filen till sitt ursprungliga utseende.
\item \label{del:h} Skriv ut de rader i filen \file{example.txt} som innehåller ordet Linux. Kommandot för att leta i en fil heter \code{grep}. Se manualen med \code{man grep}.
\item \label{del:i} Samma som uppgift \ref{del:h}, men koppla om utskriften så att den hamnar i en fil med namnet \file{linux.txt}. Skriv ut denna fil på skärmen.
\item Räkna (med ett kommando) antalet rader i filen \file{linux.txt}. Du har nu räknat antalet rader som innehåller ordet Linux i filen \file{example.txt}.
\item \label{del:k} Tag bort filen \file{linux.txt}.
...
\item Gå till början av filen och leta upp den första förekomsten av ordet Linux. Leta sedan upp nästa förekomst, osv. Byt sedan alla Linux mot Xinu.
\end{Deluppgifter}
