...
\item Läs igenom hela laborationen nedan.
...
\Subtask Rita valfri bild på valfri bakgrund, t.ex. en rosa triangel med lila konturer mot svart bakgrund, med hjälp av några av procedurerna i tabellen nedan.% \ref{lab:kojo:kojo-procedures}. 
...
\Subtask\Pen Fundera på skillnader och likheter mellan de olika sorters repetitioner du sett ovan: \code{upprepa}, \code{while}, \code{for} och \code{foreach}. Vilken tycker du är enklast att använda? Antag att man vill skriva ut antalet repetitioner hittills i varje runda.
...
\Subtask Anropa din abstraktion inuti en \code{upprepa}-loop så att paddan ritar en stapel som är 10 kvadrater hög enligt bilden nedan.
...
\item Vad är en variabel? Ge exempel på tilldelning av variabler och användning av variabler i uttryck.
...
\Task Bygg vidare på uppgift XXX och gör så att när man trycker på tangenten X så sätter man omväxlande på och av koordinataxlarna. Använd en variabel \code{isAxesOn} och definiera en abstraktion \code{toggleAxes} som anropar \code{axesOn} och \code{axesOff} på liknande sätt som i föregående uppgift.
