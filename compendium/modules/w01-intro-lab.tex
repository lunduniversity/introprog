...
\item Ladda ned workspace via \url{https://fileadmin.cs.lth.se/pgk/workspace.zip} och undersök given kod i katalogen \texttt{w01\_kojo}.
\end{Preparations}

\subsection{Obligatoriska uppgifter}

Du ska skriva program i en editor (vscode) som använder kojo som ett grafikbibliotek, enligt instruktioner i \ref{appendix:kojo} och köra med \code{scala run}. Om det förekommer en penna i marginalen ska du anteckna något inför redovisningen.
...
\Task \textit{Installera och starta Kojo}. Öppna terminalen och skriv in följande kommandon för att ladda ned filen \texttt{kojo.scala} från internet, spara den i katalogen \texttt{\textasciitilde/w01-kojo}, och starta Kojo.
...
\end{Code}
% \noindent Det är bra att börja programmet med \code{sudda}.
...
\Subtask \label{task:timer} Skriv in koden nedan i REPL och kör upprepade gånger genom att skriva \code{main}. Tar det lika lång tid varje gång? Varför?
...
Om du skriver \code{selectColor()} i REPL öppnas ett nytt fönster där man kan välja en färg. Om du sedan trycker på OK eller ENTER-tangenten kommer färgen som du valde att bli ett värde i REPL.
Testa detta i REPL:

\begin{REPL}
scala> val valdFärg = selectColor() // välj en färg
val valdFärg: java.awt.Color = java.awt.Color[r=0,g=255,b=226]

scala> val c = new java.awt.Color(124,10,78,100)
val c: java.awt.Color = java.awt.Color[r=124,g=10,b=78]
