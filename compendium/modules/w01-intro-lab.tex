...
\Subtask\Pen Går det att tilldela variabeln \code{runda} ett nytt värde i loopen? 

\Subtask\Pen Går det att referera till namnet \code{runda} utanför loopen? 
...
\Subtask\Pen För var och en av de fyra repetitionskonstruktionerna du sett ovan, \code{upprepa}, \code{while}, \code{for} och \code{foreach}: skriv kod med penna på papper som skriver ut de första 100 jämna heltalen med blanktecken emellan: \code{2 4 6 8 10 12 ...} etc.\\
Vilken typ av loop tycker du är enklast att använda i detta fall?

\Subtask\Pen Ange vilken av \code{for} och \code{while} är lämpligast i dessa fall:
...
\Subtask Generalisera din abstraktion \code{kvadrat} genom att de den en parameter \code{sida: Double} som anger hur stor kvadraten blir. Rita flera kvadrater i likhet med bilden nedan.
...
\item Vad betyder \emph{sidoeffekt}? Förklara och ge exempel.
\item Vad betyder \emph{tillstånd}? Förklara och ge exempel.
...
\Task Bygg vidare på uppgift \ref{kojo:alt} och gör så att när man trycker på tangenten X så sätter man omväxlande på och av koordinataxlarna. Använd en variabel \code{isAxesOn} och definiera en abstraktion \code{toggleAxes} som anropar \code{axesOn} och \code{axesOff} på liknande sätt som i föregående uppgift.
