...
\Task \label{task:funcall} \what~  En funktion med två parametrar definieras med följande syntax i Scala: \vspace{0.5em} \\  \texttt{\code{def} \textit{namn}(\textit{parameter1}: \textit{Typ1}, \textit{parameter2}: \textit{Typ2}): \textit{Returtyp} = \textit{returvärde}}
...
\SubtaskSolved  \code{Int}

\SubtaskSolved  Kompilatorn försöker lista ut vad för returtyp det är och lägger till det själv.

\SubtaskSolved  För om det skulle bli error och du förväntar dig en returtyp men får en annan så kan det betyda att du har en bug i programmet. Det gör även programmet mer lättläsligt.

\SubtaskSolved  När man pratar om parameter menar man variablen som används som indata till funktionen, medans argument är den faktiska indatan. Så i detta fallet så är x parametern, vilket är namnet vi get parametervariablen, medans 42 är argumentet.

\SubtaskSolved  \code{46}
...
\Task  \what~ En funktion som inte returnerar något intressant värde, men som anropas för det den  kallas \textbf{procedur}. Definiera följande procedur i REPL: \\
...
\SubtaskSolved  \code{dallas}

\SubtaskSolved  \code{dallas}

\SubtaskSolved  Koden skriver ut \code{dallas} när du sparar variablen för proceduren skriver ut när den anropas, men den returnerar inget så \code{print(x)} skriver inte ut något

\SubtaskSolved  Det är en procedur så den returnerar inget att skriva ut

\SubtaskSolved  Det är en procedur så den returnerar inget att skriva ut

\SubtaskSolved  Det är en procedur så den returnerar inget att skriva ut

\SubtaskSolved  För att man ska vara säker på att den faktiskt är en procedur och inte gör något man inte väntat sig. Det är ett bra sätt att bugtesta för ifall det blir ett exception så kanske det finns något fel man borde kolla på.
...
\Task  \what~ Skriv och testa nedan två varianter av beräkning av avståndet mellan två punkter:
\begin{REPL}
