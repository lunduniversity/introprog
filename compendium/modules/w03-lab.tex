...
Om blockstorleken är $b$, så ligger koordinaten $(x, y)$ i \code{Graphics} på koordinaten $(xb, yb)$ i \code{SimpleWindow}.
...
\Subtask
Ändra nu i \code{square} så parametrarna \code{x} och \code{y} blir \code{Graphics}-koordinater istället för \code{SimpleWindow}-koordinater.
Ändra också sidlängden på kvadraten att vara lika stor som \code{blockSize}.
Anropet till \code{moveTo} ska alltså se ut såhär:
\begin{Code}
w.moveTo(x * blockSize, y * blockSize)
\end{Code}

\Subtask
Nu när \code{Graphics} koordinatsystem är förändrat behöver parametrarna till \code{Graphics.square} i \code{Mole.main} justeras.
Ändra parametrarna och testa ditt program genom att kompilera och köra med \texttt{scalac} respektive \texttt{scala}.
(Glöm inte att ange korrekt classpath.)

\Subtask \emph{(Frivilligt)} Prova gärna att rita ut ett Tetris-block i \code{Mole.main} med hjälp av \code{Graphics.square}.

\Task
Kvadraterna som du ritat påminner om konturen på en blockmullvad, men en blockmullvad är inte ihålig.
Du ska skapa en funktion som ritar en fylld kvadrat genom att rita linjer tätt intill varandra.
...
\Subtask
Byt ut alla \code{Graphics.square}-anrop i \code{Mole.main} mot \code{Graphics.block}-anrop.
...
Det finns några fördefinierade färger i \code{java.awt.Color}, till exempel \code{java.awt.Color.black} för svart \Eng{black} och \code{java.awt.Color.green} för grönt \Eng{green}.
...
\Subtask
Lägg till en funktion i objektet \code{Graphics} med namnet \code{rectangle} som tar fem parametrar \code{x}, \code{y}, \code{width} och \code{height} av typen \code{Int} och \code{color} av typen \code{java.awt.Color}.
...
\begin{Code}
for (row <- y until (y + height)) {
	for (col <- x until (x + width)) {
		block(row, col, color)
	}
}
\end{Code}
...
\code{Mole.drawWorld} ska inte ha någon parameterlista och returtypen ska vara \code{Unit} och den ska anropa \code{Graphics.rectangle} för att rita en rekltangel med färgen \code{Colors.soil} som precis täcker fönstret.
...
\Subtask
Lägg till följande funktion i objektet \code{Graphics}:
\begin{Code}
def waitForKey: Char = {
	do {
		w.waitForEvent
...
När det kommit en tangenttryckning anropas \code{w.getKey} för att ta reda på vilken bokstav eller vilket tecken det blev, och det resultatet blir också resultatet av \code{waitForKey}, eftersom det ligger sist i \texttt{\{\}}-blocket.
