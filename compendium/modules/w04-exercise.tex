...
scala> case class Point(x: Double, y: Double) {
         def distToOrigo: Double = math.hypot(x, y)   
         def add(p: Point): Point = Point(x + p.x, y + p.y)
         def +(p: Point): Point = add(p)
       }
\end{REPL}

\Subtask Använd metoden \code{distToOrigo} för att ta reda på vad punkten med koordinaterna (3, 4) har för avstånd till origo?

\Subtask Skriv satser som skapar två punkter (3,4) och (5, 6) och låt variablerna p1 och p2 referera till respektive punkt. Låt variabeln p3 bli summan av p1 och p2 med hjälp av metoden \code{add}. Vad får uttrycken p3.x resp. p3.y för värden?
...
\Task \TODO Gör något rekursivt med en lista för att visa hur syntaxen kan se ut med cons.
