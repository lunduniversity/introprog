...
\Task \emph{Några anvädnbara samlingar.} En \textbf{samling} \Eng{collection} är en datastruktur som samlar många objekt av samma typ. I paketen \code{scala.collection} och \code{java.util} finns en hel massa samlingar med en uppsjö metoder som är användbara till olika saker. De olika samlingarna i \code{scala.collection} är ordnade i en gemensam hierarki och det de har många gemensamma metoder -- därför har man nytta av det man lärt sig om metoderna i en Scala-samling när man ska lära sig använda en annan samling. Vi har redan tidigare sett samlingen \code{Vector}.:
...
\Subtask \code{b.indexWhere(s => s.startsWith("b"))}
...
\Task Samlingarna är tätt integrerade med övriga kodbibliotek, exempelvis \code{scala.util.Random} och \code{scala.io.Source}. 
...
\Subtask \emph{Skillnader mellan \code{Vector} och \code{List}.} \code{List} är sämre än \code{Vector} på nästan allt, utom att bearbeta elementen i \emph{början} av en \code{List}; vill man lägga till och ta bort i början av en \code{List} så kan det ibland vara ungefär dubbelt så fort. Det finns också ett fåtal speciella metoder som bara finns i \code{List} för att skapa en \code{List} och lägga till i början av en \code{List}. 
