...
\Subtask \emph{Skillnader mellan \code{Vector} och \code{List}.} Klassen \code{Vector} i Scala har ''under huven'' en avancerad datastruktur i form av ett s.k. självbalancerande träd, vilket gör att \code{List} är långsammare än \code{Vector} på nästan allt, \emph{utom} att bearbeta elementen i \emph{början} av sekvensen; vill man lägga till och ta bort i början av en \code{List} så kan det ibland vara ungefär dubbelt så fort jämfört med \code{Vector}. Det finns också ett fåtal speciella metoder som bara finns i \code{List} för att skapa en lista och lägga till i början av en lista. 
...
\Task \emph{Mängd.} En mängd är en samling som garanterar att det inte finns några dubbletter. Det går dessutom snabbt att kolla om ett element finns eller inte i en mängd. Ordningen på elementen i samlingen \code{Set} hamnar ibland, av effektivitetsskäl, i en förvånande ordning.
\begin{REPL}
...
\end{REPL}
Vad blir värde och typ av uttrycken i deluppgifterna nedan baserat på ovan deklarationer?
...
\Task Map. Samlingen \code{Map} är mycket användbar. Med den kan man mycket snabbt leta upp ett värde om man har en nyckel. Samlingen \code{Map} liknar en vektor, men där man kan ''indexera'' med vilken typ av värde som helst, t.ex. en sträng.
\begin{REPL}
scala> var huvudstad = 
...
\Subtask Hur snabbt går nedan på din dator? (Nedan är kört med en i7-4790K CPU @ 4.00GHz.) %sudo lshw -class processor
