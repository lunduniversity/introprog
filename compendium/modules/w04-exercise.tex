...
\Task \emph{Objekt med attribut (fält).} Ett objekt kan samla data som hör ihop. Data i ett objekt kallas \emph{attribut} eller \emph{fält}, \Eng{field}. Objekt som samlar enbart data kallas även \emph{post} \Eng{record}.
\begin{REPL}
...
\Task \emph{Klass med attribut.} Om du vill ha flera olika upplagor av objekt av samma typ, kan du använda en \textbf{klass}. Man skapar nya objekt med klassen som ''mall'' för objektet med nyckelordet \code{new}.
...
\Subtask\Pen Varför är attributet \code{nummer} föränderligt? (Jämför med objektet \code{mittKonto}.)


\Task \emph{Klass med attribut som parametrar.} Om man vill ge attributen initialvärden när objektet skaps med \code{new} kan placera attributen i en parameterlista till klassen. Koden som kompilatorn genererar, som körs när objektet skapas och som gör tilldelningen av initaialvärden, kallas \textbf{primär konstruktor} \Eng{primary constructor}.
