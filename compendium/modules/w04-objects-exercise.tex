...
\Task \what~En jar-fil används för att samla färdigkompilerade program, kod, dokumentation, resursfiler, etc, i en enda fil. En jar-fil är komprimerad på samma sätt som en zip-fil. Kursens använder vi ett paket med namnet \code{introprog} som ligger i en jarfilen som lämpligen ges namnet \LibJar~ där numret anger version och ändras vid varje ny utgåva.
...
\\ Spara filen med namnet  \code{introprog-X.Y.Z.jar} på lämplig plats där \code{X.Y.Z} byts ut mot senaste versionsnumret. Du kan ladda ner filen direkt i terminalen med kommandot \code{wget} så här, där optionen \code{-O} med stort O anger utfilens namn:
\begin{REPLnonum}
> wget -O introprog-X.Y.Z.jar http://cs.lth.se/pgk/lib
\end{REPLnonum}
Byt \code{X.Y.Z} mot senaste versionsnumret.
...
\end{REPL}
\noindent But ut \code{X.Y.Z} mot aktuell version. Du ska nu få upp ett fönster med en liten, röd kvadrat till höger i fönstret.
...
\end{REPLnonum}
där \code{X.Y.Z} ska bytas mot aktuell version. Ett testfönster öppnas när \code{main}-metoden körs. Klicka i fönstret på olika ställen och tryck på olika tangenter och observera vad som skrivs ut. Vad skrivs ut när pil-upp-tangenten trycks ned och släpps upp?
...
\Subtask Den valda färgen returneras inkapslad i en \code{Option}. Man kan få ut värdet med \code{get}.
\begin{REPL}
scala> introprog.Dialog.selectColor()
res1: Option[java.awt.Color] = Some(java.awt.Color[r=0,g=204,b=0])
scala> res1.get
res2: java.awt.Color = java.awt.Color[r=0,g=204,b=0]

\end{REPL}


\Subtask \code{None} returneras. Med \code{getOrElse(init)} på det returnerade \code{Option}-värdet kan man använda default-färgen även vid \emph{Cancel}.

\Subtask Defaultfärgen \code{init} returneras i en \code{Option}.
