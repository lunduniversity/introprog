...
\Subtask
Öppna en texteditor, till exempel Atom (se appendix~\ref{appendix:compile:edit}).
...
\Subtask
Gå tillbaka till din texteditor och deklarera ett singelobjekt med namnet \code{Mole} i din fil.
...
\Subtask\Pen
Inför redovisningen: säkerställ att du kan förklara vad \code{x}- och \code{y}-parametrarna innebär, genom att med papper och penna rita en enkel skiss av ungefär var positionerna $(0,0)$, $(300, 0)$, $(0, 300)$ och $(300, 300)$ ligger i ett fönster som är 300 bildpunkter \Eng{pixels} brett och 500 bildpunkter högt.

\Subtask
Ta bort anropet till funktionen \code{square} när du har koll på koordinater och koordinataxlarnas riktningar.  %visat den för din labbhandledare.
...
\Subtask
Importera namnet \code{java.awt.Color} i objektet \code{Colors}. (Har du glömt hur man importerar ett namn kan du gå tillbaka till övningarna i kapitel~\ref{exe:W02}.)

\Subtask 
Ändra sen alla \code{new java.awt.Color(...)} i objektet till \code{new Color(...)}.
...
\noindent\TODO{Fundera på hur illustrera lokal import?? Ska detta vara med här?}
