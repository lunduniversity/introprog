...
case class Huvud(harHår: Boolean)
...
case class Rymdvarelse (
      ...
      ben1:   Ben   = Ben(true),
      ben2:   Ben   = Ben(false),
      huvud1: Huvud = Huvud(false),
  var huvud2: Huvud = Huvud(true)
) {
...
\Task  \what~  Webbshoppen \textbf{UberSquare} säljer flyttbara kvadrater. I affärsmodellen ingår att ta betalt per förflyttning. Du ska hjälpa UberSquare med att utveckla en enkel prototyp för att imponera på riskkapitalister.
...
\begin{enumerate}%[nolistsep, noitemsep]
  ...
  \item Varje gång förflyttning sker ska en kostnad adderas till den ackumulerade kostnaden för respektive kvadrat.

  \item UberSquare är oroliga för att kvadraterna flyttas för långt bort och bestämmer därför att kostnaden för varje förflyttning ska vara den nya positionens avstånd till ursprungsläget vid kvadratens konstruktion multiplicerat med aktuell storlek på kvadraten.

  \item För att framstå som goda bestämmer UberSquare att det är gratis att skala.
\end{enumerate}
...
\Task \label{task:aux-constructor} \what~I tidigare uppgifter har vi möjliggjort alternativa sätt att skapa instanser genom default-argument eller genom fabriksmetoder med namnet \code{apply} i kompanjonsobjektet.
