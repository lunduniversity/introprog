...
\Subtask Hur kan man från en metods signatur få en ledtråd om att ett objekt har föränderligt tillstånd \Eng{mutable state}?
...
v0 until numberOfFrogs).foreach(i => frogArray(i).x(i*distanceBetweenFrogs))
while (noCollision) {
	for (frog <- frogArray) {
		for (frog2 <- frogArray) {
				noCollision = false
...
\Task  \what~  Webbshoppen UberSquare säljer flyttbara kvadrater. I affärsmodellen ingår att ta betalt per förflyttning. Du ska hjälpa UberSquare med att utveckla en enkel prototyp för att imponera på riskkapitalister.

\Subtask Implementera \code{Square} enligt efterföljande kodskiss och enligt dessa krav:
...
\begin{enumerate}[nolistsep, noitemsep]
  ...
  \item Kostnaden för varje förflyttning ska motsvara avståndet till ursprungsläget multiplicerat med storleken på kvadraten.
\end{enumerate}
...
\Task \label{task:aux-constructor} \what~I tidigare uppgifter har vi möjliggjort alternativt sätt att skapa instanser genom default-argument eller extra fabriksmetoder med namnet \code{apply} i kompanjonsobjektet.
