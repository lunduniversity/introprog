...
\begin{itemize}[nosep, label={$\square$}]
\item Det ska finnas en klass \code{Game} som skapas i huvudprogrammet. \code{Game} ska ha en metod kallad \code{start} som sätter igång spelet.
...
\item Ditt program ska byggas med hjälp av byggverktyget \code{sbt}, se appendix \ref{appendix:build}.
\end{itemize}
...
\item Om en blockmullvad krockar med en annan blockmullvad ska de byta riktning.
...
\end{CodeSmall}
 ...
 \item \textbf{Dela upp din kod i funktioner.} Din kod blir lättare att läsa och ändra i om du delar upp den i många små funktioner med bra namn. Här är ett exempel på några användbara  funktioner i \code{Game}-klassen:
\begin{CodeSmall}
 class Game(
