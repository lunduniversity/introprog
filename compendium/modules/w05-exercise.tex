...
\item Känna till på vilket sätt \code{Array} har en särställning i JVM.
\item Kunna välja när det är lämpligt att använda \code{Vector}, \code{Array} och \code{ArrayBuffer}.
\item Känna till att klassen \code{Arrays} har färdiga metoder för kopiering.
...
\item Kunna använda klassen \code{java.uti.Scanner} i Scala och Java för att läsa in heltalssekvenser från \code{System.in}. 
\end{Goals}
