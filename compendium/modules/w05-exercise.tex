...
\Subtask Programmet fungerar inte som det ska. Du behöver korrigera 3 saker för att programmet ska göra rätt. Rätta buggarna och prova igen med olika indata.
...
\Task \emph{Använda \code{jline} istället för \code{Scanner} i REPL.} Om du använder  \code{java.util.Scanner} i Scala REPL så ekas inte de tecken som skrivs. Om du vill se vad du skriver vid indata i REPL kan du använda \code{jline.console.ConsoleReader}. Då får du dessutom editeringsfunktioner vid inmatning med t.ex. Ctrl+A och Ctrl+K. Med pil upp och pil ner kan du bläddra i inmatningshistoriken.
\begin{REPL}
...
\Subtask Prova ovan rader i REPL. Vad händer om du matar in bokstäver i stället för siffror på sista raden ovan?
