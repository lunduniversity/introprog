...
\Subtask Samlingen \code{scala.Array} har speciellt stöd i JVM och är extra snabb att allokera och indexera i. Dock kan man inte ändra storleken efter att en Array allokerats. Behöver man mer plats för man kopiera den till en ny, större array. Koden nedan visar hur det kan gå till.
\begin{REPL}
...
scala> ys(3) = 45
\end{REPL}
Definiera funktionen \code{def extendAndAppend(xs: Array[Int], x): Array[Int]} som implementerar denna algoritm. 
...
\begin{algorithm}[H]
 ...
 $n \leftarrow$ anatlet element i $xs$ \\
...
\Subtask Samlingen \code{scala.collection.mutable.ArrayBuffer} är inte riktigt lika snabb i alla lägen som Array men storleksändring hanteras automatiskt. Om man stegvis vill förändra en sekvens kan man använda en ArrayBuffer. Vad händer nedan?
\begin{REPL}
scala> 
