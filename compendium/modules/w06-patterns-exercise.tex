...
\else
\begin{REPL}
scala> partnerSuit(Spades)
scala> val xs = Vector.fill(5)(Suit.values((math.random() * 4).toInt))
scala> xs.map(partnerSuit)
\end{REPL}
\fi

\Subtask \label{subtask:match-forgetcase}
Vi ska nu undersöka vad som händer om man glömmer en av case-grenarna i matchningen i \ifswedish\code{parafärg}\else\code{partnerSuit}\fi. ''Glöm'' alltså avsiktligt en av case-grenarna och klistra in den nya \ifswedish\code{parafärg}\else\code{partnerSuit}\fi med den ofullständiga matchningen. Hur lyder varningen? Kommer varningen vid körtid eller vid kompilering?

\Subtask Anropa \ifswedish\code{parafärg}\else\code{partnerSuit}\fi med den ''glömda'' färgen. Hur lyder felmeddelandet? Är det ett kompileringsfel eller ett körtidsfel?
...
\else
\begin{Code}
def partnerSuit(f: Suit): Suit = f match
  case Spades  => Clubs
...
\Subtask Skapa med hjälp av en editor igen en funktion \ifswedish\code{def parafärg(f: Färg): Färg}\else\code{def partnerSuit(f: Suit): Suit}\fi, nästintill likadan som den som vi skapade i deluppgift \ref{task:match-sealedtrait}\ref{subtask:match-sealedtrait-function}. Funktionen ska återigen utnyttja match-uttryck för att returnera paralellfärgen till argumentet som ges. Tänk på att denna gången är \ifswedish\code{Färg}\else\code{Suit}\fi inget \code{sealed trait}, utan istället en enumeration (\code{enum}). Klistra in funktionen i REPL.
\ifswedish
\begin{REPL}
...
\else
\begin{REPL}
scala> partnerSuit(Suit.Diamonds)
scala> val xs = Vector.fill(5)(Suit.values((math.random() * 4).toInt))
scala> xs.map(partnerSuit)
\end{REPL}
\fi
...
\TaskSolved \what
\emph{Ledtråd.} Följ receptets åtta steg: \code{def canEqual(other: Any) = other.isInstanceOf[Complex]}, en \code{override def equals(other: Any)} som ett match-uttryck med \code{case that: Complex => (that canEqual this) && re == that.re && im == that.im} och \code{case _ => false}, samt \code{override def hashCode = (re, im).##}.
