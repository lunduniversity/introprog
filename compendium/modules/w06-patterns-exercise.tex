...
\Task  \label{task:match-sealedtrait-caseobject} \what~  Skapa nedan kod i en editor och klistra in i REPL. Notera nyckelordet \code{sealed} som används för att försegla en typ. En \textbf{förseglad typ} måste ha alla sina subtyper i en och samma kodfil.
\begin{Code}
object Kortlek:
  sealed trait Färg
  object Färg:
...
\Subtask \label{subtask:match-sealedtrait} Skapa en funktion \code{def parafärg(f: Färg): Färg} i en editor, som med hjälp av ett match-uttryck returnerar parallellfärgen till en färg. Parallellfärgen till \code{Hjärter} är \code{Ruter} och vice versa, medan parallellfärgen till \code{Klöver} är \code{Spader} och vice versa. Klistra in funktionen i REPL. Passa även på att skriva en \textbf{import-sats} för det yttre objektet Kortlek, så slipper medlemmarna av objektet kan nås enkelt.
\begin{REPL}
