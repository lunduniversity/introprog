...
\else
\begin{Code}
    case 'g' if math.random() > 0.5 => "cucumber is tasty sometimes..."
\end{Code}
\fi
...
\Task \label{task:match-caseclass} \what~   Scalas \code{match}-uttryck är extra kraftfulla om de används tillsammans med \code{case}-klasser: då kan attribut extraheras automatiskt och bindas till lokala variabler direkt i case-grenen som nedan exempel visar (notera att \code{v} och \ifswedish\code{rutten}\else\code{rotten}\fi{} inte behöver deklareras explicit). Detta kallas för \textbf{mönstermatchning}. 
Vad skrivs ut nedan? Varför? Prova att byta namn på \code{v} och \ifswedish\code{rutten}\else\code{rotten}\fi.
...
scala> val g = Cucumber(100, true)
scala> g match { case Cucumber(v,rotten) => println("G" + v + rotten) }
\end{REPL}
\fi
...
\code{match} testar om kompanjonsobjektet \ifswedish\code{Gurka}\else\code{Cucumber}\fi{} är av typen \ifswedish\code{Gurka}\else\code{Cucumber}\fi{} med två parametervärden. De angivna parametrarna tilldelas namn, \ifswedish\code{vikt}\else\code{weight}\fi{} får namnet \code{v} och \ifswedish\code{ärRutten}\else\code{isRotten}\fi{} namnet \ifswedish\code{rutten}\else\code{rotten}\fi{} och skrivs sedan ut. Byts namnen dessa ges skrivs de ut i den omvända ordningen.
...
\else
\begin{REPL}
scala> sealed trait Suit
scala> case object Spades extends Suit
\end{REPL}
\fi
...
Skapa en funktion \ifswedish\code{def parafärg(f: Färg): Färg}\else\code{def sameColourSuit(f: Suit): Suit}\fi{} i en editor, som med hjälp av ett match-uttryck returnerar parallellfärgen till en färg. Parallellfärgen till \ifswedish\code{Hjärter}\else\code{Hearts}\fi{} är \ifswedish\code{Ruter}\else\code{Diamonds}\fi{} och vice versa, medan parallellfärgen till \ifswedish\code{Klöver}\else\code{Clubs}\fi{} är \ifswedish\code{Spader}\else\code{Spades}\fi{} och vice versa. Klistra in funktionen i REPL. Passa även på att skriva en \code{import}-sats för det yttre objektet \textbf{Kortlek}, så medlemmarna av objektet kan nås enkelt.
\ifswedish
\begin{REPL}
...
\else
\begin{REPL}
scala> sameColourSuit(Spades)
scala> val xs = Vector.fill(5)(Suit.values((math.random() * 4).toInt))
scala> xs.map(sameColourSuit)
\end{REPL}
\fi

\Subtask \label{subtask:match-forgetcase}
Vi ska nu undersöka vad som händer om man glömmer en av case-grenarna i matchningen i \ifswedish\code{parafärg}\else\code{sameColourSuit}\fi. ''Glöm'' alltså avsiktligt en av case-grenarna och klistra in den nya \ifswedish\code{parafärg}\else\code{sameColourSuit}\fi{} med den ofullständiga matchningen. Hur lyder varningen? Kommer varningen vid körtid eller vid kompilering?

\Subtask Anropa \ifswedish\code{parafärg}\else\code{sameColourSuit}\fi{} med den ''glömda'' färgen. Hur lyder felmeddelandet? Är det ett kompileringsfel eller ett körtidsfel?
...
\else
\begin{REPL}
Cannot extend sealed trait Suit in a different source file
\end{REPL}
\fi
Felmeddelandet fås av att REPL:en behandlar varje inmatning individuellt och tillåter därför inte att subtypen \ifswedish\code{Spader}\else\code{Spades}\fi{} ärver från \Eng{extends} supertypen \ifswedish\code{Färg}\else\code{Suit}\fi{} eftersom denna var förseglad \Eng{sealed}. Mer om detta senare i kursen...

\SubtaskSolved
Nu accepteras koden utan fel eftersom den förseglade typen \ifswedish\code{Färg}\else\code{Suit}\fi{} och alla dess subtyper (\ifswedish\code{Spader}\else\code{Spades}\fi, \ifswedish\code{Hjärter}\else\code{Hearts}\fi, \ifswedish\code{Ruter}\else\code{Diamonds}\fi, \ifswedish\code{Klöver}\else\code{Clubs}\fi) ligger i samma kodfil, inuti det yttre objektet \ifswedish\code{Kortlek}\else\code{Deck}\fi.

\SubtaskSolved
Förusatt att \ifswedish\code{import Kortlek._}\else\code{import Deck._}\fi{} har skrivits...
\ifswedish
\begin{Code}
...
\begin{REPL}
<console>:17: warning: match may not be exhaustive.
It would fail on the following input: Diamonds
\end{REPL}
\fi
...
\else
\begin{REPL}
scala.MatchError: Diamonds (of class Diamonds)
  at .sameColourSuit(<console>:17)
\end{REPL}
\fi
...
\Subtask Skapa med hjälp av en editor igen en funktion \ifswedish\code{def parafärg(f: Färg): Färg}\else\code{def sameColourSuit(f: Suit): Suit}\fi, nästintill likadan som den som vi skapade i deluppgift \ref{task:match-sealedtrait}\ref{subtask:match-sealedtrait-function}. Funktionen ska återigen utnyttja match-uttryck för att returnera paralellfärgen till argumentet som ges. Tänk på att denna gången är \ifswedish\code{Färg}\else\code{Suit}\fi{} inget \code{sealed trait}, utan istället en enumeration (\code{enum}). Klistra in funktionen i REPL.
\ifswedish
\begin{REPL}
...
\else
\begin{REPL}
scala> sameColourSuit(Suit.Diamonds)
scala> val xs = Vector.fill(5)(Suit.values((math.random() * 4).toInt))
scala> xs.map(sameColourSuit)
\end{REPL}
\fi
...
\end{Code}
\fi
Likt uppgift \ref{task:match-sealedtrait}\ref{subtask:match-sealedtrait-function} så kan även här en \code{import}-sats skrivas för att nå medlemmarna i \ifswedish\code{Färg}\else\code{Suit}\fi{} utan punktnotation.
...
scala> val x = "urka"
scala> x match
         case str if str.startsWith("g") => println("kanske cucumber")
         case vadsomhelst => println("ej cucumber: " + vadsomhelst)
scala> val g = "cucumber"
scala> g match
         case str if str.startsWith("g") => println("kanske cucumber")
         case vadsomhelst => println("ej cucumber: " + vadsomhelst)
\end{REPL}
...
\Subtask \emph{Fallgrop med små begynnelsebokstäver.} Innan du provar nedan i REPL, försök gissa vad som kommer att hända. Vad händer? Hur lyder varningarna och vad innebär de?
\begin{REPL}
...
\SubtaskSolved  Både \code{str} och \code{vadsomhelst} matchar med inputen, oavsett vad denna är på grund av att de har en liten begynnelsebokstav.\\
 \code{str} har dock en gard att strängen måste börja med $g$ vilket gör så endast \ifswedish\code{val g = "gurka"}\else\code{val g = "cucumber"}\fi{} matchar med denna. \code{val x = "urka"} plockas dock upp av \code{vadsomhelst} som är utan gard.
...
\else
\begin{REPL}
<console>:17: warning: unreachable code due to variable patter 'tomato' on line
16
\end{REPL}
\fi
Trots att en klass \ifswedish\code{tomat}\else\code{tomato}\fi{} existerar så tolkar Scalas \code{match} den som en \code{case}-gren som fångar allt på grund av en liten begynnelsebokstav. Detta gör så alla objekt som inte är av typen \ifswedish\code{Gurka}\else\code{Cucumber}\fi{} kommer ge utskriften \textit{tomat} och att sista caset inte kan nås.
...
\item Ta bort predikatet \ifswedish\code{ärÄtvärd}\else\code{isWorthEating}\fi{} i objektet \code{Main} och inför i stället en abstrakt metod \ifswedish\code{def ärÄtbar: Boolean}\else\code{def isEdible: Boolean}\fi{} i traiten \ifswedish\code{Grönsak}\else\code{Vegetable}\fi.
\item Inför konkreta \code{val}-medlemmar i respektive grönsak som definierar ätbarheten.
\item Ändra i huvudprogrammet i enlighet med ovan ändringar så att \ifswedish\code{ärÄtvärd}\else\code{isWorthEating}\fi{} anropas som en metod på de skördade grönsaksobjekten när de ätvärda ska filtreras ut.
\end{itemize}
...
\else
\begin{Code}
case class Broccoli(weight: Int, isRotten: Boolean) extends Vegetable:
  val isEdible: Boolean = (!isRotten && weight > 80)
\end{Code}
\fi
~\\
Därefter läggs följande till i \code{object Main} innan \ifswedish\code{def slumpgrönsak}\else\code{def randomVegetable}\fi:
...
\else
\begin{Code}
def randomBroccoli: Broccoli = Broccoli(randomWeight, randomRotten)
\end{Code}
\fi
~\\
Slutligen ändras \ifswedish\code{def slumpgrönsak}\else\code{def randomVegetable}\fi{} till följande:
...
\end{Code}
\Subtask Vad händer nu nedan, där \ifswedish\code{Gurka}\else\code{Cucumber}\fi{} nu har en överskuggad \code{equals} med innehållslikhet?
\ifswedish
\begin{REPL}
scala> val g1 = new Gurka(42, true)
scala> val g2 = g1
...
\else
\begin{REPL}
scala> val g1 = new Cucumber(42, true)
scala> val g2 = g1
scala> val g3 = new Cucumber(42, true)
scala> g1 == g2
scala> g1 == g3
\end{REPL}
\fi
...
\SubtaskSolved  \begin{enumerate}
\item En klass \ifswedish\code{Gurka}\else\code{Cucumber}\fi{} skapas med parametrarna \ifswedish\code{vikt}\else\code{weight}\fi{} av typen \code{Int} och ärÄtbar av typen \code{Boolean}.
\item \code{g1} tilldelas en instans av \ifswedish\code{Gurka}\else\code{Cucumber}\fi-klassen med \ifswedish\code{vikt = 42}\else\code{weight = 42}\fi{} och \ifswedish\code{ärÄtbar = true}\else\code{isEdible = true}\fi.
\item \code{g2} tilldelas samma \ifswedish\code{Gurka}\else\code{Cucumber}\fi-objekt som g1.
\item \code{g3} tilldelas en ny instans av \ifswedish\code{Gurka}\else\code{Cucumber}\fi-klassen med motsvarande parametrar som g1.
...
\SubtaskSolved
Se objektdiagrammet ovan: \code{g1} och \code{g2} refererar till samma \ifswedish\code{Gurka}\else\code{Cucumber}\fi-objekt medan \code{g3} refererar till ett eget objekt med samma innehåll --- därför ger \code{==} (som utan egen \code{equals} jämför referenslikhet) \code{true} för \code{g1 == g2} men \code{false} för \code{g1 == g3}.

\SubtaskSolved  I de första 3 raderna sker samma som i deluppgift \textit{a}. När nu dessa jämförelser görs mellan \ifswedish\code{Gurka}\else\code{Cucumber}\fi-objekten så överskuggas \code{Any.equals} av den \code{equals} som är specificerad för just \ifswedish\code{Gurka}\else\code{Cucumber}\fi. Eftersom båda objekten \code{g1} jämförs med också är av typen \ifswedish\code{Gurka}\else\code{Cucumber}\fi{} så matchar den med \ifswedish\code{case that: Gurka}\else\code{case that: Cucumber}\fi. Denna i sin tur jämför vikterna hos de båda gurkorna och returnerar en \code{Boolean} huruvida de är lika eller inte, vilket de i båda fallen är.
...
\SubtaskSolved
Koden bygger upp en liten DSL för polynom. Den förseglade typen \code{Term} har subtyperna \code{Const} (en konstant), \code{Var} (en variabel med en exponent, t.ex. \code{x}) och \code{Prod} (en produkt av en konstant och en mängd \code{Var}), medan \code{Poly} representerar en summa av termer. Metoden \code{*} multiplicerar termer (t.ex. ger \code{Const(1) * x} termen \code{x}) och upphöjningsmetoden ger en ny term med uppdaterad exponent. De fördefinierade variablerna \code{x}, \code{y}, \code{z}, \code{s} och \code{t} är \code{Var}-objekt och \code{Poly(...)} skriver ut sina termer som en summa --- vilket förklarar resultaten i REPL-exemplet ovan.

\SubtaskSolved
\emph{Ledtråd.} Inför addition (\code{+}) analogt med \code{*} genom att matcha på subtyperna: två \code{Prod} med samma variabeldel kan slås ihop genom att addera konstanterna, annars blir resultatet en \code{Poly} som innehåller båda termerna i sin mängd.
...
Exempel på metoder som finns både för \code{Vector} och \code{Option}:
...
\TaskSolved \what
I Scala är \code{try}-\code{catch} ett \textbf{uttryck} som ger ett värde, så \code{input} och \code{output} kan bindas med \code{val} direkt från resultatet i stället för att (som i Java) först deklarera oinitialiserade variabler:
\begin{Code}
@main def TryCatch(args: String*): Unit =
  val (input, output) =
    if args(0) == "safe" then
      try
        val in = args(1).toInt
        println("Skyddad division!")
        (in, 42 / in)
      catch
        case e: Exception =>
          println("Undantag fångat: " + e)
          println("Dividerar ändå med säker default!")
          (1, 42 / 1)
    else
      val in = args(0).toInt
      println("Oskyddad division!")
      (in, 42 / in)
  println(s"42 / $input == $output")
\end{Code}
Programmet beter sig precis som Java-versionen. De viktigaste skillnaderna: Scalas \code{try}-\code{catch} returnerar ett värde (här en tupel), \code{catch} använder mönstermatchning (\code{case e: Exception =>}), och Scala kräver varken att undantag fångas eller deklareras.
...
\TaskSolved \what
\emph{Ledtråd.} \code{reduce} ska förenkla ett \code{Poly} genom att slå ihop \code{Prod}-termer med samma variabeldel: gruppera termerna på deras \code{vars}, summera konstanterna inom varje grupp och bygg ett nytt \code{Poly} av de hopslagna termerna (uteslut termer vars konstant blev 0).
...
\TaskSolved \what
~\\\emph{Ledtråd.} Inför \code{def ===(that: Term): Boolean} som abstrakt metod i \code{Term} och överskugga den i varje subtyp med en innehållsjämförelse. Eftersom parametern har den specifika typen \code{Term} (inte \code{Any}) ger kompilatorn ett typfel om man försöker jämföra en \code{Term} med t.ex. en \code{String} eller \code{Vector}.
...
\TaskSolved \what
~\\\emph{Ledtråd.} Följ receptets åtta steg (se teori/föreläsning): \\\code{def canEqual(other: Any) = other.isInstanceOf[Complex]}\\och en\\\code{override def equals(other: Any)} som har ett match-uttryck med dessa fall:\\\code{case that: Complex => (that canEqual this) && re == that.re && im == that.im} \\\code{case _ => false}\\samt \\\code{override def hashCode = (re, im).##}
...
\TaskSolved \what
\emph{Ledtråd.} Tillämpa samma recept i arvshierarkin: ge \code{Number} en \code{equals} och låt varje subklass (\code{Complex}, \code{Rational}) överskugga \code{canEqual} med sin egen typ samt \code{equals} med \\\code{super.equals(that) && that.canEqual(this)} följt av jämförelse av de egna fälten. Objekt av olika subklasser blir då aldrig lika.
...
\begin{REPL}
scala> Vector(1, 2, 3) match
     |   case hela @ Vector(första, _*) => (första, hela)
val res0: (Int, Vector[Int]) = (1,Vector(1, 2, 3))
\end{REPL}

\SubtaskSolved
I sekvensmönster matchar \texttt{\_} exakt ett element medan \texttt{\_*} matchar noll eller flera kvarvarande element:
\begin{REPL}
scala> def beskriv(xs: Vector[Int]) = xs match
     |   case Vector()           => "tom"
     |   case Vector(x)          => s"ett element: $x"
     |   case Vector(första, _*) => s"börjar med $första"
scala> Vector(beskriv(Vector()), beskriv(Vector(9)), beskriv(Vector(1, 2, 3)))
val res0: Vector[String] = Vector(tom, ett element: 9, börjar med 1)
\end{REPL}
...
\TaskSolved \what
Ett extraktor-objekt definierar metoden \code{unapply} som kan användas i mönstermatchning. Här plockas värd och sökväg ut ur en http-adress:
\begin{Code}
object Http:
  def unapply(s: String): Option[(String, String)] =
    if s.startsWith("http://") then
      val rest = s.drop("http://".length)
      val i = rest.indexOf('/')
      if i >= 0 then Some((rest.take(i), rest.drop(i + 1)))
      else Some((rest, ""))
    else None
\end{Code}
\begin{REPL}
scala> "http://my.host.domain/path/to/this" match
     |   case Http(host, path) => s"host=$host, path=$path"
     |   case _                => "ingen träff"
val res0: String = host=my.host.domain, path=path/to/this
\end{REPL}
...
\TaskSolved \what
\emph{Ledtråd.} Polynomdivision är en öppen och klurig uppgift: gör som vid liggande stol --- dividera upprepat den ledande termen i resten med den ledande termen i nämnaren, multiplicera och subtrahera tills restens grad är mindre än nämnarens. Bygg vidare på \code{*}, \code{+} och \code{reduce}.
