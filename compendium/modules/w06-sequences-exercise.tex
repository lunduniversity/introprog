...
\WHAT{Samlingen \code{WrappedArray}.}
...
\begin{framed}
Minnesregel: \emph{''Arrays don't behave, but \code{WrappedArray}s do!''}.
\end{framed}
\noindent  Även om \code{Array} är primitiv så finns smart krångel ''under huven'' i Scalas standarbibliotek för att samlingar av typen \code{Array} ska bete sig nästan som ''riktiga'' samlingar och undvika man behöver bry sig om saker som s.k. boxning och typomvanling vid körtid, vilket man ofta behöver kämpa med som Java-programmerare.

Dock finns fortfarande begränsningar och anomalier vad gäller till exempel likhetstest. Om du vill att en array ska bete sig som andra samlingar kan du enkelt ''wrappa'' den med metoden \code{toSeq} i en \code{WrappedArray} som beter sig som en helt vanlig sekvenssamling utan att offra snabbheten hos en primitv array.
\begin{Code}
val as = Array(1,2,3)
val xs = as.toSeq
\end{Code}
\Subtask Undersök \code{as} och \code{xs} i REPL: Hur fungerar likhetstest? Vad har \code{xs} för typ?

\Subtask Läs här:
\url{http://docs.scala-lang.org/overviews/collections/arrays.html}
och ett exempel på vad mer man inte kan göra med en Array, förutom innehållslikhetstest med \code{as == bs}.
...
scala> val xs = as.toSeq
xs: Seq[Int] = WrappedArray(1, 2, 3)
...
scala> xs
res8: Seq[Int] = WrappedArray(42, 2, 3)
...
\noindent Varför går olika körningar olika snabbt även om for-loopen gör samma sak?
...
\WHAT{Tiskomplexitet för olika samlingar i Scalas standardbibliotek.}
