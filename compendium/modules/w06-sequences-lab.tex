...
\item \code{TestCardDeck.scala} testar klassen \code{CardDeck} och implementationen av \code{shuffle} med en kortlek som endast innehåller tre kort. Upprepade blandningar görs och förekomsten av varje möjlig permutation  registreras.
...
\Subtask Skapa din egen implementation av metoden \code{shuffle} i klassen \code{CardDeck}. Följ den givna algoritmen i stycke \ref{knuth-shuffle} noga. Du kan använd \code{cards.length} för att få fram längden på kortleken, men du gärna instället loopa med hjälp av \code{cards.indices.reverse}.

\Subtask Kör mainmetoden i \code{TestCardDeck} och kontrollera att blandningen är jämnt fördelad. Metoden \code{testShuffle} blandar en kortlek med tre kort och räknar hur ofta varje möjlig permutation dyker upp. Du bör få en utskrift med sex ($3!$) procentsatser som ska vara nästan lika.

\Task Fyll i de ofärdiga delarna av kompanjonsobjektet till \code{CardDeck}.

\Subtask Implementera metoden \code{full} som skapar en 52-korts standardlek ordnad efter färg och valör. Använd konstanterna i \code{Card}. %Tänk på att en \code{for}/\code{yield}-sats inte nödvändigtvis ger en \code{Array}, men att alla samlingar kan omvandlas till en sådan med \code{toArray}.

\Subtask Kör \code{testCreate} i \code{TestCardDeck} och kontrollera så att du får kort av alla fyra färger, och både ess och kungar.
