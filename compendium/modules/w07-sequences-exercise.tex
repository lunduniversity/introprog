...
\SubtaskSolved
\begin{Code}
def flips(n: Int): Vector[Boolean] = Vector.fill(n)(math.random() < 0.5)
\end{Code}
\begin{REPL}
scala> registerCoinFlips(flips(1000))
val res0: (Int, Int) = (499,501)
\end{REPL}
...
\SubtaskSolved
\begin{REPL}
scala> Vector(1, 2, 3, 2, 1).lastIndexOf(2)
val res0: Int = 3

scala> Vector(1, 2, 3, 4, 5).indexOfSlice(Vector(3, 4))
val res1: Int = 2

scala> Vector(2, 4, 6, 7, 8).segmentLength(_ % 2 == 0, 0)
val res2: Int = 3

scala> Vector("a", "abc", "ab").maxBy(_.length)
val res3: String = abc
\end{REPL}
...
\SubtaskSolved
\begin{REPL}
val fel: Vector[String] = Vector(z, ä, å, ö)
val rätt: Vector[String] = Vector(z, å, ä, ö)
\end{REPL}
...
\SubtaskSolved
\begin{Code}
@main def sorted(flagga: String, fil: String): Unit =
  val locale =
    if flagga == "-sv" then java.util.Locale.of("sv")
    else java.util.Locale.getDefault
  val coll = java.text.Collator.getInstance(locale)
  val ordning = Ordering.fromLessThan[String]: 
    (a, b) => coll.compare(a, b) < 0
  val rader = scala.io.Source.fromFile(fil).getLines().toVector
  rader.sorted(using ordning).foreach(println)
\end{Code}
Programmet körs med \code{scala sorted -sv textfil.txt} och skriver ut filens rader i korrekt svensk sorteringsordning.

\SubtaskSolved
Se länken ovan för fler detaljer och alternativ för lokalanpassad sortering.
...
val xs2 = as.toSeq
\end{Code}
\Subtask Hur fungerar likhetstest mellan två \code{ArraySeq}s? Vad har \code{xs2} ovan för typ? Går den att uppdatera?
...
\SubtaskSolved \code{xs2} erbjuder innehållslikhet och har typen \code{Seq[Int]} med den underliggande typen \code{ArraySeq[Int]}. Det går inte att göra tilldelning av element i en \code{ArraySeq} eftersom metoden \code{update} saknas, och den är oföränderlig. Den uppdateras därför inte när den urspringliga arrayen uppdateras.
...
\TaskSolved \what
\emph{Ledtråd.} Sammanställ en tabell över (amorterad) tidskomplexitet för \code{Vector}, \code{List}, \code{Array}, \code{ArrayBuffer}, \code{ListBuffer}, \code{Set} och \code{Map} för operationer som indexering, uppdatering, tillägg först/sist, infogning och medlemskapstest. Se Scalas dokumentation \emph{Collections --- Performance Characteristics}; t.ex. har \code{List} O(1) prepend men O(n) indexering, medan \code{Vector} har nästan O(1) för både åtkomst och uppdatering.
