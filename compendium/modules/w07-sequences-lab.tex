...
Med hjälp av klasserna \code{Test} och \code{PokerProbability} kan du testa så att dina implementationer fungerar.
...
\Subtask Implementera \code{tally} som ska ge en indexerbar sekvens med 13 platser där varje index representerar antalet av respektive valör. Ess räknas som lägst.
...
\begin{figure}[H]
 ...
 \end{minipage}\hfill
 \begin{minipage}[c]{\CardCaptionWidth}
  \caption{Färgstege \Eng{straight flush}: både stege och färg. Specialfall: Om färgstegens högsta kort är ett ess kallas den \textit{royal flush} eller \textit{royal straight flush}.}
 \end{minipage}
\end{figure}
