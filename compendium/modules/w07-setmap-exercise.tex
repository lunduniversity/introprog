...
Använd metoderna \code{split} och \code{toSet} för skapa ett uttryck som beräknar hur många unika ord det finns i strängen \code{hej} nedan:
\begin{REPLnonum}
...
val telnr = Map(
  ...
  "Björn"     -> 46462229009L,
  "Sandra"    -> 46462220368L,
  "Fröken Ur" -> 464690510L,
)
\end{Code}
En uppsättning \code{Map}-instanser, vid behov nästlade, kan med fördel användas för att bygga upp en i-minnet-databas där kombinationer av inbyggda samlingsmetoder, t.ex. \code{map} och \code{filter}, tillsammans med \code{for}-\code{yield}-uttryck, ger flexibla och effektiva sökmöjligheter. På veckans laboration ska du göra detta.
...
\Subtask Skapa nedan variabler i REPL. Hamnar mappningarna i \code{ys} i samma ordning som de genererades med for-uttrycket som skapade \code{xs}?
\begin{Code}
val xs = (for (i <- 2 to 16 by 2) yield (i, i + 1)).toMap
val ys = xs.toVector
\end{Code}

\Subtask Med \code{xs} och \code{ys} deklarerade i REPL enligt ovan, para ihop yttryck till vänster med rätt resultat till höger. Om du är osäker på de sammansatta uttrycken, prova enklare uttryck i REPL och undersök värde och typ.

\begin{ConceptConnections}
\input{generated/quiz-w07-mapops-taskrows-generated.tex}
\end{ConceptConnections}
...
\begin{ConceptConnections}
  \input{generated/quiz-w07-mapops-solurows-generated.tex}
\end{ConceptConnections}
...
\Task \what~Vi ska nu utnyttja ett riktigt listigt trick för att i ett enda kort uttryck implementera registrering med hjälp av samlingsmetoden \code{groupBy} och tabellmetoden \code{mapValues}.
...
\Subtask Skapa en funktion \code{freq} med nedan funktionshuvud som returnerar en tabell räknar antalet förekomster av olika heltal i \code{xs}. Testa \code{freq} på en sekvens av 1000 slumpvisa tärningskast och förklara hur funktionen \code{freq} fungerar. \emph{Tips:} Gör först \code{groupBy(???)} och sedan \code{mapValues(???)}.

\begin{Code}
def freq(xs: Vector[Int]): Map[Int, Int] = ???

def kasta(n: Int): Vector[Int] =
  Vector.fill(n)(util.Random.nextInt(6) + 1)
...
\WHAT{Läsa från fil och URL.}

\QUESTBEGIN

\Task \what~\TODO Gör ett enkelt spel om att gissa europas huvudstäder.

\href{https://raw.githubusercontent.com/lunduniversity/introprog/master/compendium/examples/europa.txt}{\texttt{compendium/examples/europa.txt}}
