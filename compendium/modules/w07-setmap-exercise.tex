...
\begin{Goals}
\input{modules/w07-setmap-exercise-goals.tex}
\end{Goals}
...
\vspace{1em}\noindent Koppla varje uttryck till vänster till ett utryck till höger som har samma värde:

\begin{ConceptConnections}
\input{generated/quiz-w07-setops-taskrows-generated.tex}
\end{ConceptConnections}
...
\begin{ConceptConnections}
\input{generated/quiz-w07-setops-solurows-generated.tex}
\end{ConceptConnections}
...
\Task \what~På veckans laboration ska vi göra automatisk språkbehandling av långa texter som vi delar upp i ord.

Med metoden \code{s.split(' ').toVector} kan man dela upp en sträng \code{s} i en vektor av ord. Använd metoderna \code{split} och \code{toSet} för skapa ett uttryck som beräknar hur många unika ord det finns i strängen \code{hej} nedan:
\begin{REPL}
...
\WHAT{Skapa 2-tupler med metoden \code{->} ''mappas till''.}

\QUESTBEGIN

\Task \what~Vi har tidigare sett hur två olika värden kan samlas i en 2-tupel, till exempel \code{(0, true)}. Par kan även skapas med hjälp av metoden \code{->} enligt nedan. Testa i REPL.
\begin{REPLnonum}
scala> ("Skåne", "Lund")          // ett strängpar
scala> "Skåne -> "Lund"           // operatornotation med ->
scala> "Skåne".->("Lund")         // punktnotation med ->
\end{REPLnonum}
...
\Subtask Fungerar det att på par skapade med \code{->} använda metoderna \code{_1} och \code{_2}?
...
\Subtask Vad blir det för felmeddelande om du råkar skriva ett komma för mycket före högerparentesen: ...\code{und",)}
...
\Subtask Implementera funktionen \code{lookupIndex} nedan, som med hjälp av samlingsmetoden \code{indexWhere} ska göra linjärsökning efter ett par i sekvensen där \code{key} finns på första platsen i paret.
...
Eller med funktioner som generella, återanvändbara dellösningar:
\begin{REPL}
...
\WHAT{Registrering med inbyggda samlingsmetoder.}
...
\WHAT{Läsa från och skriva till fil.}
...
\WHAT{Samlingsmetoden \code{sliding}.}

\QUESTBEGIN

\Task  \what~  I veckans labb kommer du att ha nytta av samlingsmetoden \code{sliding}, som ger en iterator för speciella delsekvenser av en sekvens, vilka kan liknas vid ''utsikten'' i ett ''glidande fönster''. Kör nedan i REPL och beskriv vad som händer.
...
%%<AUTOEXTRACTED by mergesolu>%%      %Uppgift 24
...
\Task  \what~ Studera skillnader i prestanda mellan olika samlingar här: \\ \href{http://docs.scala-lang.org/overviews/collections/performance-characteristics.html}{docs.scala-lang.org/overviews/collections/performance-characteristics.html} \\
