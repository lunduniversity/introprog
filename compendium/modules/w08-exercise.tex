...
\begin{Preparations}
\item Studera innebörden veckans begrepp. 
\end{Preparations}
...
\item Ändra i huvudprogrammet i enlighet med ovan ändringar så att \code{ärÄtvärd} anropas som en metod på de skördade grönsaksobjekten när de ätvarda ska filtreras ut.
\end{itemize} 
...
\Subtask Skapa en funktion \code{def parafärg(f: Färg): Färg} i en editor, som med hjälp av ett match-uttryck returnerar parallellfärgen till en färg. Paralellfärgen till \code{Hjärter} är \code{Ruter} och vice versa, medan parallellfärgen till \code{Klöver} är \code{Spader} och vice versa. Klistra in funktionen i REPL.
\begin{REPL}
...
\Task \emph{Använda \code{Option} och matcha på värden som kanske saknas.} Man behöver ofta skriva kod för att hantera värden som eventuellt saknas, t.ex. saknade telefonnummer i en persondatabas. Denna situation är så pass vanlig att många språk har speciellt stöd för sankande värden. 

I Java\footnote{Scala har också \code{null} men det behövs bara vid samverkan med Java-kod.} används värdet \code{null} för att indikera att en referens saknar värde. Man får då komma ihåg att testa om värdet saknas varje gång sådana värden ska behandlas, , t.ex. med  \code+if (ref != null) { ...} else { ... }+. Ett annat vanligt trick är att låta \code{-1} indikera sakande positiva heltal, till exempel saknade index, som får behandlas med \code+if (i != -1) { ...} else { ... }+. 
...
\Subtask\Pen Vilken typ har variabeln \code{carola} ovan? Vad hade typen blivit om cath-grenen hade returnerat en sträng i stället?
...
scala> Try{pang}
scala> Try{pang}.recover{case e: Throwable => s"desermerad bomb: $e"}
scala> Try{"tyst"}.recover{case e: Throwable => s"desermerad bomb: $e"}
...
scala> badOpt.map(_.getOrElse("bomben desermerad!")).get
scala> goodOpt.map(_.getOrElse("bomben desermerad!")).get
scala> xs.map(_.getOrElse("bomben desermerad!")).foreach(println)
