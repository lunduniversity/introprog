...
\Subtask Implementera metoden \code{unapply} som ska ta in en sträng som innehåller tonens namn och oktavnummer (t.ex. "E2"). Tänk på att en not kan vara på formen "E2"\ och "E\#2"\ . "E2"\ kommer översättas till 16 och "C1"\ till 0 och tänk på att hantera specialfallet då till exempel "C\#1"\ ska ge värdet 1. Använd attributet \code{toNumber} för att översätta en ton utan oktav ("C\#", "E") till ett nummer. Titta gärna på vad metoden \code{zipWithIndex.toMap} gör och använd den i \code{toNumber}.
...
\Subtask Metoderna kan testas genom att skriva ett testprogram eller efterhand som uppgift 4 implementeras.
