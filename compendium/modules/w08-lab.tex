...
\Subtask Implementera klassen \code{Guitar}. \code{toString} ska vara på samma format som i filen \textit{chords.txt}. \code{tuning} ska vara en strängrepresentation av gitarren stämning, alltså E2 A2 D3 G3 B3 E4.
...
\Task \code{database}. Kommer hålla reda på alla ackord i programmet. Testa varje metod innan nästa blir implemeterad. Det blir då lättare att upptäcka fel i koden. Metoderna kan testas genom att skriva ett testprogram eller efterhand som uppgift 4 implementeras.

\Subtask Börja med att implemetera \code{add} och \code{allChords}.

\Subtask Implementera \code{find} och \code{updateFilter}. \code{find} ska retunera alla ackord som innehåller söksträngen, alltså inte bara första förekomsten. Metoden \code{find} i \code{Vector} retunerar enbart första förekomsten. \code{updateFilter} ska ändrat filtersträngen i \code{database} till den angivna strängen.
