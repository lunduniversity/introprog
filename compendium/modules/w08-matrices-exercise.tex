...
\TODO{Flytta delövningar till extraövning, tex allt om imperativa varianter.}
...
\code{def yatzyPips(xss: Vector[Vector[Int]]): Vector[Int]} \\ som ger en vektor med tärningsvärdena för de kast i matrisen \code{xss} som gav yatzy enligt nedan. Använd din funktion \code{filterYatzy}.
\begin{REPL}
...
Fördel: \code{StringBuilder} är snabbare vid tillägg på slutet vid stora strängar (men här kommer det inte märkas eftersom strängen är så liten)\\
...
\Task\label{exe:classes:labprep}  \what~Under veckans laboration ska du simulera en enkel form av ''liv'' som består av celler i ett rutnät. För detta ändamål har vi nytta av en matris-klass som du ska implementera steg för steg i denna övning.
Skapa case-klassen nedan med en editor i filen \code{Matrix.scala}. Testa genom att använda \code{sbt console}, och gör efterföljande övningar.
\begin{Code}
...
\SubtaskSolved Man behöver ändra på 2 ställen från \code{String} till \code{Int}.
...
\SubtaskSolved Tack vare kompilatorns typinferens så blir typen på \code{bm} blir \code{Matrix[Boolean]}.

\SubtaskSolved Typen på \code{be} blir \code{Boolean}.

\SubtaskSolved \SubtaskSolved \SubtaskSolved \SubtaskSolved \SubtaskSolved
\begin{CodeSmall}
case class Matrix[T](data: Vector[Vector[T]]) {
...
\Subtask Implementera case-klassen \code{Table} enligt specifikationen ovan. Du kan förutsätta att alla rader har lika många kolumner som antalet element i \code{headings}, samt att alla rubrikerna i \code{headings} är unika. Detta förutsätts också gälla för indatafiler som läses in med \code{fromFile}.
