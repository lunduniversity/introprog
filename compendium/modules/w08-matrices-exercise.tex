...
\begin{Code}
  def concat[U](that: Cell[U]): Cell[String] = 
    Cell(value.toString + that.value.toString)
\end{Code}
Tar man bort typparametern \code{U} och i stället kräver \code{that: Cell[T]} kan man bara konkatenera celler av exakt samma typ \code{T} --- vilket är mer begränsande och hindrar t.ex. att slå ihop en \code{Cell[Int]} med en \code{Cell[String]}.
