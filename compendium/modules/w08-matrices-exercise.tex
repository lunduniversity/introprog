...
\begin{Goals}
\input{modules/w08-matrices-exercise-goals.tex}
\end{Goals}
...
\emph{Lösningsskiss --- öppen uppgift.} Modellera ett kast som en \code{Vector[Int]} med fem tärningar (1--6) och en omgång som \code{YatzyRows(rows: Vector[Vector[Int]])} med metoderna \code{roll} (lägg till en ny rad med fem slumpmässiga tärningar) och \code{reroll(indices)} (slå om valda tärningar i sista raden). Lägg predikaten \code{isYatzy}, \code{isThreeOfAKind}, \code{isFourOfAKind}, \code{isFullHouse}, \code{isSmallStraight} och \code{isLargeStraight} i kompanjonsobjektet; de flesta kan uttryckas med \code{groupBy}/\code{count} på tärningsvärdena (t.ex. yatzy = alla fem lika, liten stege = sorterade värden är \code{Vector(1,2,3,4,5)}). Bygg sedan ett huvudprogram som i en \code{while}-loop låter användaren kasta, välja vilka tärningar som ska slås om, och rapporterar vilka kombinationer som uppfylls.
...
%5.a)
\SubtaskSolved
\emph{Ledtråd.} \code{def concat[U](that: Cell[U]): Cell[String] = Cell(value.toString + that.value.toString)}. Tar man bort typparametern \code{U} och i stället kräver \code{that: Cell[T]} kan man bara konkatenera celler av exakt samma typ \code{T} --- vilket är mer begränsande och hindrar t.ex. att slå ihop en \code{Cell[Int]} med en \code{Cell[String]}.
