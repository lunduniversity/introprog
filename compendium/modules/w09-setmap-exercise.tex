...
\TaskSolved \what
\emph{Lösningsskiss --- öppen uppgift.} Läs raderna från URL:en med \code{scala.io.Source.fromURL(...)} och dela varje rad på \code{";"}. Bygg en nyckel-värde-tabell \code{Map} från landsnamn till uppgifterna (invånarantal, storlek, huvudstad). Generera slumpvisa frågor genom att lotta ett (eller två) länder och en frågetyp, jämför med tabellen och kontrollera användarens svar. Håll en \code{Set} med länder som redan besvarats rätt och filtrera bort dessa när nya frågor lottas, så att de inte återkommer.
...
\WHAT{Iterera i ett svep med \code{groupMap och groupMapReduce}.}

\QUESTBEGIN

\Task \what~ En LTH-student har skapat följande inköpslista:

\begin{Code}
enum Avdelning:
  case Grönsaker, Frukt, Mejeri, Kött, Spannmål

case class Matvara(vara: String, avdelning: Avdelning, pris: Int)

val inköpslista = List(
    Matvara("tomat",   Avdelning.Grönsaker, 10),
    Matvara("nötfärs", Avdelning.Kött,      60),
    Matvara("gurka",   Avdelning.Grönsaker, 5),
    Matvara("banan",   Avdelning.Frukt,     20),
    Matvara("mjölk",   Avdelning.Mejeri,    20),
    Matvara("korv",    Avdelning.Kött,      50),
    Matvara("äpple",   Avdelning.Frukt,     30),
    Matvara("apelsin", Avdelning.Frukt,     40),
  )

\end{Code}

\noindent För att göra en rationell uppdelning av listan och räkna ut kostnader studerar studenten metodern \code{groupMap} och  \code{groupMapReduce} noga här: \\ \url{https://superruzafa.github.io/visual-scala-reference/}


\Subtask Använd funktionen \code{groupMap} för att dela upp matvarorna utifrån avdelningen där de kan hittas.

\Subtask Använd funktionen \code{groupMapReduce} för att ta reda på vad den sammanlagda kostnaderna för varje avdelning kommer att vara. 
...
\begin{Code}
inköpslista.groupMap(_.avdelning)(_.vara)
\end{Code}

\SubtaskSolved

\begin{Code}
inköpslista.groupMapReduce(_.avdelning)(_.pris)(_ + _)
\end{Code}
...
% \end{figure}
% The in-book IO.scala listing above is disabled (source now lives in the introprog-scalalib repo, linked in
% the footnote); keep this label live so the footnote's \pageref{disk-access-code} resolves to this exercise's page.
\label{disk-access-code}
\SOLUTION

\TaskSolved \what
\emph{Ledtråd.} Använd funktionerna i singelobjektet \code{introprog.IO} för att skriva och läsa text till och från fil; se källkoden för \code{IO} i kursbiblioteket (länken i uppgiften).
