...
\Task \emph{Typklasser och implicita parametrar.} I Scala finns möjligheter till avancerad funktionsprogrammering med s.k. \textbf{typklasser}. Vi nosar lite i denna uppgift lite på hur implicita argument kan användas för att skapa typklasser för att illustrera hur implicita ordningarna är definierade i Scalas standardbibliotek. 
...
\Subtask \emph{Importera implicita ordningsoperatorer från en \code{Ordering}.} Om man gör import på en \code{Ordering} får man tillgång till implicita konverteringar som gör att jämförelseoperatorerna fungerar. Testa nedan variant av \code{isSorted} på olika sekvenstyper och se att \code{<=} nu fungerar.
\begin{CodeSmall}
def isSorted[T](xs: Seq[T])(implicit ord: Ordering[T]): Boolean = {  
  import ord._
  xs.zip(xs.tail).forall(x => x._1 <= x._2)
...
\Task  \label{task:custom-ordering} \emph{Skapa en egen \code{Ordering}.}  

\Subtask Med hjälp av metoden \code{by} i objektet \code{scala.math.Ordering} kan man skapa ordningar genom att bifoga ett predikat som avgör om 

Om du har en case-klass med många fält och vill ha en fördefinierad implicit sorteringsordning samt även erbjuda en alternativ sorteringsordning kan du placera ordningar 
\begin{Code}
...
\Task Ordered - [eller ska vi skippa det???] \TODO
...
\Subtask Sök upp dokumentationen för \code{java.util.Comparator}  \TODO
