...
scala> val ps = Vector(Person("Kim", "Finkodare"), Person("Robin","Fulhack")) 
scala> ps.sorted
\end{REPL}	
...
\Subtask Det blir även problem med denna implementation. Hur lyder felmeddelandet? Vad saknas?
\begin{REPLnonum}
...
Testa så att kompilatorn hittar ordningen för samlingar med värden av några grundtyper. Kontrollera även enligt nedan att det fortfarande blir problem för egendefinierade klasser, t.ex. \code{Person} enligt tidigare (detta ska vi råda bot på i uppgift \ref{task:custom-ordering}).
\begin{REPL}
scala> isSorted(Vector(1,2,3))
scala> isSorted(Array(1,2,3,1))
scala> isSorted(Vector("A","B","C"))
