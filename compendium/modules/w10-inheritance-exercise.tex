...
\else
\begin{REPL}
scala> class Cucumber(val weight: Int)
scala> class Tomato(val weight: Int)
scala> val gurkor = Vector(Cucumber(100), Cucumber(200))
scala> val vegetables = Vector(Cucumber(300), Tomato(42))
\end{REPL}
\fi

\Subtask Om en samling innehåller objekt av flera olika typer försöker kompilatorn härleda den mest specifika typen som objekten har gemensamt. Vad blir det för typ på värdet \ifswedish\code{grönsaker}\else\code{vegetables}\fi ovan?
...
\Subtask Du ska nu göra så att du kan komma åt vikten på alla grönsaker genom att ge gurkor och tomater en gemensam bastyp som de olika konkreta grönsakstyperna utvidgar med nyckelordet \code{extends}. Det heter att subtyperna \ifswedish\code{Gurka}\else\code{Cucumber}\fi och \ifswedish\code{Tomat}\else\code{Tomato}\fi \textbf{ärver} egenskaperna hos supertypen \ifswedish\code{Grönsak}\else\code{Vegetable}\fi.

Skapa en bastyp \ifswedish\code{Grönsak}\else\code{Vegetable}\fi med ett abstrakt attribut \ifswedish\code{vikt}\else\code{weight}\fi. Låt sedan de konkreta grönsakerna ärva bastypen:
...
scala> class Cucumber(val weight: Int) extends Vegetable
scala> class Tomato(val weight: Int) extends Vegetable
scala> val gurkor = Vector(Cucumber(100), Cucumber(200))
scala> val vegetables = Vector(Cucumber(300), Tomato(42))
\end{REPL}
\fi
När sker initialisering av attributet \ifswedish\code{vikt}\else\code{weight}\fi?

\Subtask Vad blir det nu för typ på variabeln \ifswedish\code{grönsaker}\else\code{vegetables}\fi ovan?

\Subtask Går det nu att summera vikterna i \ifswedish\code{grönsaker}\else\code{vegetables}\fi med uttrycket nedan? Varför?\\ \ifswedish\code{grönsaker.map(_.vikt).sum}\else\code{vegetables.map(_.weight).sum}\fi
...
\Subtask Traiten \ifswedish\code{Grönsak}\else\code{Vegetable}\fi har en abstrakt medlem \ifswedish\code{vikt}\else\code{weight}\fi. Den sägs vara abstrakt eftersom den saknar implementation -- medlemmen har bara ett namn och en typ men inget värde. Du kan instansiera den abstrakta traiten \ifswedish\code{Grönsak}\else\code{Vegetable}\fi om du fyller i det som ''fattas'', nämligen ett värde på \ifswedish\code{vikt}\else\code{weight}\fi. Man kan fylla på det som fattas i genom att ''hänga på'' ett block efter typens namn vid instansiering. Man får då vad som kallas en \textbf{anonym klass}, i detta fall en ganska konstig grönsak som inte är någon speciell sorts grönsak med som ändå har en vikt.

Vad får \ifswedish\code{anonymGrönsak}\else\code{anonymousVegetable}\fi nedan för typ och strängrepresenation?
\ifswedish
\begin{REPL}
...
\else
\begin{REPL}
scala> val anonymousVegetable = new Vegetable { val weight = 42 }
\end{REPL}
\fi

\Subtask Vad blir felmeddelandet om du skapar en anonym klass \ifswedish\code{Grönsak}\else\code{Vegetable}\fi med en kropp som saknar definition av vikt?
...
\else
\begin{REPLnonum}
scala> vegetables.map(_.weight).sum  
-- Error:                                                                                 
1 |vegetables.map(_.weight).sum
  |              ^^^^^^
  |             value weight is not a member of Object - did you mean wait?
-- Error:
1 |vegetables.map(_.weight).sum
  |                         ^
  |ambiguous implicit arguments: both object DoubleIsFractional in object Numeric and object ShortIsIntegral in object Numeric match type Numeric[B] of parameter num of method sum in trait IterableOnceOps
\end{REPLnonum}
\fi
Det första felmeddelandet beror på att vektorns element är av typen \code{Object} och medlemmen \ifswedish\code{vikt}\else\code{weight}\fi är inte definierat för denna typ. Det andra felmeddelandet är ett följdfel som beror på att en sekvens med element av typen \code{Object} inte kan summeras eftersom kompilatorn inte kan härleda att elementtypen är numerisk.

\SubtaskSolved  Attributet \ifswedish\code{vikt}\else\code{weight}\fi initialiseras vid konstruktion av \ifswedish\code{Gurka}\else\code{Cucumber}\fi resp. \ifswedish\code{Tomat}\else\code{Tomato}\fi. Värdet ges av resp. klassparameter.

\SubtaskSolved  \ifswedish\code{Vector[Grönsak]}\else\code{Vector[Vegetable]}\fi.

\SubtaskSolved  Ja. Eftersom den statiska typen för elementen i sekvensen är \ifswedish\code{Grönsak}\else\code{Vegetable}\fi (den dynamiska typen kan vara godtycklig subtyp av \ifswedish\code{Grönsak}\else\code{Vegetable}\fi) och alla instanser av denna typ garanterat har attributet \ifswedish\code{vikt}\else\code{weight}\fi som är av typen \code{Int} så kan kompilatorn vid \emph{kompileringstid} dra slutsatsen att summeringen är giltig och därmed kan kompilatorn kompilera koden till körbar maskinkod.
...
\begin{REPLnonum}
scala> val anonymousVegetable = new Vegetable { val weight = 42 }
val anonymousVegetable: Vegetable = anon$1@1edde8b6
scala> anonymousVegetable.toString                                                                                      
val res0: String = anon$1@1edde8b6
\end{REPLnonum}
\fi
Typen är \ifswedish\code{Grönsak}\else\code{Vegetable}\fi och blir här en s.k. \emph{anonym klass}, eftersom vi inte har använt en namngiven klass med \code{extends}, utan bara ''hängt på'' en klasskropp inom klammerparenteser direkt vid konstruktion. När du skapar anonyma klasser måste du använda nyckelordet \code{new}.
...
1 |new Vegetable { }
  |^
  |object creation impossible, since val weight: Int in trait Vegetable is not defined 
...
\Task \label{task:fyle} \what~   \ifswedish Man kan utvidga en klass med multipla traits med en kommaseparerad lista. På så sätt kan man fördela medlemmar i olika traits och återanvända gemensamma koddelar genom så kallad \textbf{inmixning}, så som nedan exempel visar.\else You can extend a class with multiple traits given as a comma-separated list. This lets you spread members across different traits and reuse shared code through so-called \textbf{mix-in composition}, as the example below shows.\fi

\ifswedish En alternativ fågeltaxonomi, speciellt populär i Skåne, delar in alla fåglar i två specifika kategorier: Kråga respektive Ånka. Krågor kan flyga men inte simma, medan Ånkor kan simma och oftast även flyga. Fågel i generell, kollektiv bemärkelse kallas på gammal skånska för Fyle.%
\footnote{\href{http://www.klangfix.se/ordlista.htm}{www.klangfix.se/ordlista.htm}}\else In the remote Welsh village of Llanffowl-y-Gwyll, whose inhabitants have --- for reasons long since lost to scholarship --- never seen the need to tell apart more than a couple of birds, the local taxonomy sorts all fowl into precisely two kinds: the \emph{Crawke} and the \emph{Quaddle}. A Crawke can fly but cannot swim, whereas a Quaddle can swim and, more often than not, fly as well. A bird in the general, collective sense is known in the old village dialect as a \emph{Fowlk}.%
\footnote{Affectionately modelled on a genuine alternative bird taxonomy from Scania (Skåne) in southern Sweden, whence the idea came; see \href{http://www.klangfix.se/ordlista.htm}{www.klangfix.se/ordlista.htm}.}\fi
...
trait CanFly        { val canActuallyFly = true }
trait CanPerhapsFly { val canActuallyFly = math.random() < 0.8 }

class Crawke extends Fowlk, CanFly, CannotSwim:
  val call = "kraak"

class Quaddle extends Fowlk, CanSwim, CanPerhapsFly:
  val call = "quawk"
  override def holler = print(call * 4)
\end{CodeSmall}
\fi

\Subtask \ifswedish En flitig ornitolog hittar 42 fåglar i en perfekt skog där alla fågelsorter är lika sannolika, representerat av vektorn \code{fyle} nedan. Skriv i REPL ett uttryck som undersöker hur många av dessa som är flygkunniga Ånkor, genom att använda metoderna \code{filter}, \code{isInstanceOf}, \code{ärFlygkunnig} och \code{size}.\else A diligent ornithologist finds 42 birds in a perfect wood where every kind of bird is equally likely, represented by the vector \code{flock} below. Write, in the REPL, an expression that works out how many of them are flight-capable Quaddles, using the methods \code{filter}, \code{isInstanceOf}, \code{canActuallyFly} and \code{size}.\fi
...
\begin{REPL}
scala> val flock =
         Vector.fill(42)(if math.random() > 0.5 then new Crawke else new Quaddle)
scala> flock.foreach(_.holler)
scala> val flyingQuaddles: Int = ???
\end{REPL}
\fi

\Subtask \label{subtask:fyle:sound} \ifswedish Om alla de fåglar som ornitologen hittade skulle väsnas exakt en gång var, hur många krax och hur många kvack skulle då höras? Använd metoderna \code{filter} och \code{size}, samt predikatet \code{ärSimkunnig} för att beräkna antalet krax respektive kvack.\else If every bird the ornithologist found were to holler exactly once, how many kraaks and how many quawks would be heard? Use the methods \code{filter} and \code{size}, together with the predicate \code{canActuallySwim}, to compute the number of kraaks and quawks respectively.\fi
\ifswedish
\begin{REPL}
...
\else
\begin{REPL}
scala> val kraaks: Int = ???
scala> val quawks: Int = ???
\end{REPL}
\fi
...
\SubtaskSolved
\ifswedish Det finns många olika sätt, några exempellösningar:\else There are many ways; here are a few example solutions:\fi
\ifswedish
\begin{Code}
...
\else
\begin{Code}
val flyingQuaddles: Int =
  flock.count(f => f.isInstanceOf[Quaddle] && f.canActuallyFly)
\end{Code}

\begin{Code}
val flyingQuaddles: Int =
  flock.filter(f => f.isInstanceOf[Quaddle] && f.canActuallyFly).size
\end{Code}

\begin{Code}
val flyingQuaddles: Int =
  flock.collect{case f: Quaddle if f.canActuallyFly}.size
\end{Code}

\begin{Code}
val flyingQuaddles: Int = flock.map(_ match
  case f: Quaddle if f.canActuallyFly => 1
  case _ => 0
).sum
\end{Code}
\fi
...
\else
\begin{Code}
val kraaks: Int = flock.filter(f => !f.canActuallySwim).size * 2
val quawks: Int = flock.filter(f => f.canActuallySwim).size * 4
\end{Code}
\fi
...
\Task  \what~  \ifswedish Om man vill förhindra att man kan göra \code{extends} på en klass kan man göra den final genom att placera nyckelordet \code{final} före nyckelordet \code{class}.\else If you want to prevent a class from being extended with \code{extends}, you can make it final by placing the keyword \code{final} before the keyword \code{class}.\fi

\Subtask \ifswedish Eftersom klassificeringen av fåglar i uppgiften ovan i antingen Ånkor eller Krågor är fullständig och det inte finns några subtyper till varken Ånkor eller Krågor är det lämpligt att göra dessa finala. Ändra detta i din kod.\else Since the classification of birds in the task above into either Quaddles or Crawkes is complete, and there are no subtypes of either Quaddles or Crawkes, it is appropriate to make these final. Change this in your code.\fi

\Subtask \ifswedish Testa att ändå försöka göra en subklass \code{Simkråga extends Kråga}. Vad ger kompilatorn för felmeddelande om man försöker utvidga en final klass?\else Try nonetheless to make a subclass \code{Swimcrawke extends Crawke}. What error message does the compiler give if you try to extend a final class?\fi
...
\SubtaskSolved  \ifswedish Sätt \code{final} framför \code{class} i klasserna.\else Put \code{final} in front of \code{class} in the classes.\fi

\SubtaskSolved  \ifswedish error: illegal inheritance from final class Kråga.\else error: illegal inheritance from final class Crawke.\fi
...
\Task  \what~ \ifswedish Det visar sig att vår flitige ornitolog från uppgift \ref{task:fyle} på sidan \pageref{task:fyle} sov på en av föreläsningarna i zoologi när hen var nolla på Natfak, och därför helt missat fylekategorin \code{Pjodd}. Hjälp vår stackars ornitolog så att fylehierarkin nu även omfattar Pjoddar. En Pjodd kan flyga som en Kråga men den \code{ÄrLiten} medan en Kråga \code{ÄrStor}. En Pjodd kvittrar dubbelt så många gånger som en Ånka kvackar. En Pjodd \code{KanKanskeSimma} där simkunnighetssannolikheten är $0.2$. Låt ornitologen ånyo finna 42 slumpmässiga fåglar i skogen och filtrera fram lämpliga arter. Undersök sedan hur dessa väsnas, i likhet med deluppgift \ref{task:fyle}\ref{subtask:fyle:sound}.\else It turns out that our diligent ornithologist from task \ref{task:fyle} on page \pageref{task:fyle} slept through one of the zoology lectures back when they were a fresher at the science faculty, and so missed the Fowlk category \code{Pibb} entirely. Help our poor ornithologist so that the Fowlk hierarchy now also takes in Pibbs. A Pibb can fly like a Crawke, but it \code{IsSmall} whereas a Crawke \code{IsLarge}. A Pibb chitters twice as many times as a Quaddle quawks. A Pibb \code{CanPerhapsSwim}, with a swim-capability probability of $0.2$. Let the ornithologist once again find 42 random birds in the wood and filter out the appropriate species. Then examine how they holler, as in subtask \ref{task:fyle}\ref{subtask:fyle:sound}.\fi
...
trait CanFly { val canActuallyFly = true }
trait CanPerhapsFly { val canActuallyFly = math.random() < 0.8 }
trait CanPerhapsSwim { val canActuallySwim = math.random() < 0.2 }
trait IsLarge { val isLarge = true }
trait IsSmall { val isLarge = false }

final class Crawke extends Fowlk, CanFly, CannotSwim, IsLarge:
  val call = "kraak"
...
\ifswedish I REPL:\else In the REPL:\fi
\ifswedish
\begin{REPL}
...
\else
\begin{REPL}
val flock = Vector.fill(42)(
  if math.random() < 0.33 then Crawke()
  else if math.random() < 0.5 then Quaddle()
  else Pibb()
)
flock.filter(f => f.isInstanceOf[Crawke]).size * 2
flock.filter(f => f.isInstanceOf[Quaddle]).size * 4
flock.filter(f => f.isInstanceOf[Pibb]).size * 8
\end{REPL}
\fi
