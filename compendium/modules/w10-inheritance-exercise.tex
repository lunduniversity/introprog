...
\Task \label{task:fyle} \what~   You can extend a class with multiple traits given as a comma-separated list. This lets you spread members across different traits and reuse shared code through so-called \textbf{mix-in composition}, as the example below shows.

In the remote Welsh village of Llanffowl-y-Gwyll, whose inhabitants have --- for reasons long since lost to scholarship --- never seen the need to tell apart more than a couple of birds, the local taxonomy sorts all fowl into precisely two kinds: the \emph{Crawke} and the \emph{Quaddle}. A Crawke can fly but cannot swim, whereas a Quaddle can swim and, more often than not, fly as well. A bird in the general, collective sense is known in the old village dialect as a \emph{Fowlk}.%
\footnote{Affectionately modelled on a genuine alternative bird taxonomy from Scania (Skåne) in southern Sweden, whence the idea came; see \href{http://www.klangfix.se/ordlista.htm}{www.klangfix.se/ordlista.htm}.}
...
\Subtask A diligent ornithologist finds 42 birds in a perfect wood where every kind of bird is equally likely, represented by the vector \code{flock} below. Write, in the REPL, an expression that works out how many of them are flight-capable Quaddles, using the methods \code{filter}, \code{isInstanceOf}, \code{canActuallyFly} and \code{size}.
...
\Subtask \label{subtask:fyle:sound} If every bird the ornithologist found were to holler exactly once, how many kraaks and how many quawks would be heard? Use the methods \code{filter} and \code{size}, together with the predicate \code{canActuallySwim}, to compute the number of kraaks and quawks respectively.
\begin{REPL}
...
\SubtaskSolved
There are many ways; here are a few example solutions:
\begin{Code}
...
\Task  \what~  If you want to prevent a class from being extended with \code{extends}, you can make it final by placing the keyword \code{final} before the keyword \code{class}.

\Subtask Since the classification of birds in the task above into either Quaddles or Crawkes is complete, and there are no subtypes of either Quaddles or Crawkes, it is appropriate to make these final. Change this in your code.

\Subtask Try nonetheless to make a subclass \code{Swimcrawke extends Crawke}. What error message does the compiler give if you try to extend a final class?
...
\SubtaskSolved  Put \code{final} in front of \code{class} in the classes.

\SubtaskSolved  error: illegal inheritance from final class Crawke.
