...
\subsection{Bakgrund}
När man skriver program skapar man ofta modeller av en viss, verklig \emph{domän}, som kan vara t.ex. försäkringskassans regelverk eller en fysiksimulering i ett datorspel. För att kunna skapa sådana modeller behöver man ofta skaffa sig omfattande \emph{domänkunskap} genom att noga sätta sig in i vad olika koncept i domänen innebär och hur de är relaterade. Under denna läroprocess kan man använda programkod för att skapa modeller av domänen, utifrån noga valda förenklingar av verkligheten. Förmåga att kunna skapa domänmodeller utgör en viktig grund för konsten att utveckla bra programvarusystem.
...
Symbolen \code{#} representerar en höjning ett halvt tonsteg och kallas \emph{sharp} engelska, och \code{C#} uttalas \emph{see sharp}. På svenska säger man \emph{siss}.\footnote{Man använder även b-förtecknet $\flat$, som uttalas \emph{flat} på engelska, för sänkning av en ton ett halvt tonsteg, men för enkelhetens skull bortser vi i vår modell från detta sätt att namnge toner.}
...
Stränginstrument som man spelar på genom att trycka på en greppbräda \Eng{fret board} med en hand, samtidigt som man knäpper på strängarna med den andra handen, kallas på engelska \emph{fretted instruments}. Gitarr och ukulele är exempel på sådana instrument. Dessa instrument kan skilja sig åt vad gäller antalet strängar och hur dessa strängar är stämda. Gitarr har 6 strängar, medan ukulele bara har 4 strängar. Dessa egenskaper modelleras i koden nedan.
...
\scalainputlisting[basicstyle=\ttfamily\fontsize{10}{13}\selectfont]{../workspace/w10_music/src/main/scala/music/instruments.scala}
...
Ett elektroniska instrument syntetiserar ljud med hjälp av analog och/eller digital elektronik, och kallas därför \textbf{synthesizer}, ofta förkortat \emph{synt} \Eng{synth}.
...
\Task Bygg vidare på terminalappen \code{music.Main} enligt nedan.
