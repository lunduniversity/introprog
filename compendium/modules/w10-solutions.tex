...
\Subtask
Koden laddar ner html-koden för sidan \href{http://kurser.lth.se/lot/?lasar=16_17&soek_text=&sort=kod&val=kurs&soek=t}{HÄR} och sparar den i en vektor. Sedan filtreras ut endast de rader som innehåller strängen ”kurskod” så att all onödig HTML-kod försvinner. Sedan konverteras detta, för varje rad, till \code{Course}-objekt med hjälp av metoden \code{fromHtml}. Eftersom variabeln \code{lth2016} är deklarerad som \code{lazy} kommer inte \code{download()} bli anropad förrän vi vill komma åt variabeln. Vi startar alltså processen genom att kalla på \code{lth2016}: 
...
En något mer effektiv version av \code{isSorted} som stoppar direkt när den upptäcker att ett element inte matchar med den sorterade versionen.
...
def isSorted(xs: Vector[Int]): Boolean = {
  ...
  var i = 0
  while (i < xs.length && xs(i) == xssort(i)) {
    i += 1
  }
  if(i == xs.length - 1)
    return true
...
\TODO TA BORT LÖSNING OCH ERSÄTT MED TIPS


\Subtask
Felmeddelandet på rad 3 berättar att kompilatorn inte kunde hitta något implicit värde för parametern y. Detta fixar vi på rad 4 då vi skapar en \code{implicit val} av rätt typ (\code{Int}). Detta gör att rad 5 lyckas kompilera.

\Subtask

Rad 4 kan ej exekveras för kompilatorn hittar inget implicit objekt som \code{extends CanCompare[T]} och vet därför inte hur den ska använda \code{compare}.

\Subtask

\begin{Code}
implicit object doubleComparator extends CanCompare[Double]{
...
\begin{Code}
implicit object stringComparator extends CanCompare[String] {
  override def compare(a: String, b: String): Int = {
    ...
    if (i == a.length || i == b.length) {
      //there is no position where the two strings differ
