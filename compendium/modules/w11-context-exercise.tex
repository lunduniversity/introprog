...
\SubtaskSolved
\emph{Ledtråd.} Gör \code{isSorted} generisk med en kontextparameter: \code{def isSorted[T](xs: Vector[T])(using ord: Ordering[T]): Boolean} och jämför intilliggande element med \code{ord.lteq(a, b)}. Tack vare \code{using}/\code{given} fungerar den för alla typer som har en \code{Ordering} i scope, t.ex. \code{Int} och \code{String}.
-

\SubtaskSolved
\emph{Ledtråd.} Verifiera genom att anropa \code{isSorted} med olika elementtyper så att rätt \code{Ordering} hämtas automatiskt; man kan också ge en egen ordning explicit i anropet via \code{using}-argumentet.
-
...
scala> val xs =
         Vector(Team("fnatic", 1499), Team("nip", 1473), Team("lumi", 1601))
scala> xs.sorted
scala> xs.sorted(using Team.nameOrdering)
\end{REPL}
...
\SubtaskSolved
\emph{Ledtråd.} Skapa egna ordningar med \code{Ordering.by(f)} där \code{f: T => S} mappar till en typ \code{S} som redan har en given ordning (t.ex. \code{Int} eller \code{String}). För omvänd ordning används \code{.reverse}.
-

\SubtaskSolved
\emph{Ledtråd.} Lägg en default \code{given Ordering[T]} i case-klassens kompanjonsobjekt --- ett av de sista ställen kompilatorn söker efter implicita värden. Då fungerar även jämförelseoperatorerna (\code{<}, \code{>} \dots) efter att medlemmarna i lämpligt ordningsobjekt importerats.
-
