...
\Task  \what~ Koden nedan är en modell av husdjur på Jorden med följande innebörd: Husdjur kan vara friska eller sjuka och föds i normalfallet friska. Det kan finnas många katter och hundar, som alla är olika slags husdjur.
...
\Subtask Förändras koden nedan så att efterföljande REPL-sats ej ger kompileringsfel?
\begin{Code}
...
case class Box[+A](x: A)
\end{Code}
Kompilatorn tillämpar reglerna för kovarians: en \code{Box[Cat]} är en suptyp till \code{Box[Any] då \code{Cat} är en subtyp till \code{Any}.

\SubtaskSolved 
\begin{itemize}
  \item Samlingen \code{Vector} är kovariant och därmed flexibel i sin typparameter. Kompilatorn betraktar \code{Vector[Cat]} som en subtyp till \code{Vector[Pet]} eftersom \code{Cat} är en subtyp till \code{Pet}. 
  \item Samlingen \code{Set} är invariant och därmed inte flexibel i sin typparameter. \code{Set[Cat]} är alltså \emph{inte} en subtyp till \code{Set[Pet]} \emph{även} om \code{Cat} är en subtyp till \code{Pet}.  
...
\SubtaskSolved Veterinären \code{Vet} är flexibel i sin typparameter och minustecknet anger kontravarians och därmed att \code{Vet[Pet]} är en subtyp till \code{Vet[Cat]} då \code{Cat} är en subtyp till \code{Pet}. 
\begin{REPL}
