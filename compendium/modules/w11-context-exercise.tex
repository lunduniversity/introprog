...
object Counter:
  ...
  object Delta:
    given default: Delta = Delta(0)

  var x: Int = 0
  def inc(using dx: Delta) = x + dx.value
  def dec(using dx: Delta) = x - dx.value
\end{Code}

\Subtask
\TODO uppgifter med flera olika givens i lokal kontext
...
\Subtask Förklara vad som händer vid anrop av \code{summon[GivenValue.GreetConfig]}. 
