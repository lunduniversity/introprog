...
\Task \emph{Översätta algoritmer och metoder från Java till Scala.} 

\Subtask Översätt klassen \code{Hangman} nedan från Java till Scala enligt dessa riktlinjer:
\begin{enumerate}[nolistsep, noitemsep]

\item Skriv Scala-koden med en texteditor i en fil som heter \texttt{hangman1.scala} och kompilera med \code{scalac hangman1.scala} i terminalen; använd alltså \emph{inte} en IDE så som Eclipse eller IntelliJ utan en ''vanlig'' texteditor så som \code{gedit}.

\item Översätt i denna första deluppgift \emph{så likt} den ursprungliga Java-kodens utseende (syntax)  som möjligt, med så få ändringar som möjligt. Du ska alltså behålla långa indrag, onödiga semikolon, onödiga \code{()}, onödiga \code|{}|, onödiga \code{System.out}, och onödiga \code{return}, även om det inte alls blir som man brukar skriva i Scala. Försök också i denna deluppgift göra så att betydelsen (semantiken) motsvarar den i Java, t.ex. genom att använda \code{var} overallt, även om det vore lämpligare med \code{val}. Du ska i \emph{nästa} deluppgift sedan göra om din Scala-kod så att den blir mer ''normal''.
 
\item En Javaklass med bara statiska medlemmar motsvara ett Scalaobjekt med ''vanliga'' medlemmar. Kalla Scala-objektet för \code{hangman}. När man använder ett Scala objekt som en modul\footnote{\href{https://en.wikipedia.org/wiki/Modular_programming}{en.wikipedia.org/wiki/Modular\_programming}} (alltså en samling funktioner i en gemensam, avgränsad namnrymd) brukar man ha liten begynnelsebokstav, då man i Scala i detta fall ofta följer konventionen för paketnamn; ett paket är ju också en slags modul.
...
\footnote{\href{https://github.com/lunduniversity/introprog/blob/master/compendium/examples/scalajava/Hangman.java}{github.com/lunduniversity/introprog/blob/master/compendium/examples/scalajava/Hangman.java}}. I samma bibliotek ligger även lösningarna till översättningen i Scala, men frestas \emph{inte} att kolla på dessa förrän du gjort klart översättningarna och fått dem att kompilera och köra felfritt! 
...
\TODO Flytta nedan kod till facit:
\scalainputlisting[numbers=left]{examples/scalajava/hangman2.scala}
...
%%% jobba vidare med ArrayList[Int] och ArrayList[Integer]
...
\Task Översätt nedan kod från Java till Scala. Skriv koden i en fil som heter \texttt{showInt.scala} och kalla Scala-objektet med \code{main}-metoden för \code{showInt}.
