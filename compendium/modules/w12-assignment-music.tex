...
\item Hämta given kod via \href{https://github.com/lunduniversity/introprog/tree/master/workspace/}{kursen github-plats} eller via kurshemsidan under \href{https://lunduniversity.github.io/pgk/#l%C3%A4nkar}{Länkar}.
\end{Preparations}
...
C, D$\flat$, D, E$\flat$, E, F, G$\flat$, G, A$\flat$, A, B$\flat$ och B.

Efter tonklassen med namnet \code{B} återkommer tonklassen med namnet \code{C}.
Symbolen ''$\flat$'' (skrives \code{b} i ascii) representerar en höjning ett halvt tonsteg. Tonklassen C$\flat$ uttalas \emph{sess} på svenska, och \emph{see flat} på engelska.\footnote{Man använder även $\sharp$-förtecknet, som uttalas \emph{sharp} på engelska, för höjning av en ton ett halvt tonsteg, men för enkelhetens skull bortser vi i vår modell från detta sätt att namnge toner.}
Notera att det är ett halvt tonsteg mellan \code{E} och \code{F}, samt mellan \code{B} och \code{C} (det finns därför varken E$\flat$ eller C$\flat$ i listan med tonklassnamn.\footnote{Varför det är på detta viset kan du läsa mer om på t.ex. Wikipedia, men du kan också nöja dig med att det helt enkelt är så på grund av historiska skäl.})

På ett piano motsvaras de vita tangenterna av tonklassnamen C D E F G A B och de svarta tangenterna motsvaras av tonklassnamnen C$\flat$ D$\flat$ F$\flat$ G$\flat$ A$\flat$.

En s.k. \textbf{tonklass} är ett värde av \code{enum}en \code{Pitch.Class}.
En tonhöjd \code{pitch} tillhör tonklassen \code{pitch.pitchClass}\footnote{Repetitionen av ordet \code{pitch} krävs ty \code{class} är ett reserverat ord i scala.}.

Med hjälp av heltalsdivision med 12 får man fram tonhöjdens så kallade \textbf{oktav}, alltså \code{number / 12 - 1}. Ett piano har normalt toner som spänner över 7 eller 8 oktaver.
En tonhöjd kan även namnges med en kombination av tonklassnamnet och tonens oktav, t.ex. \code{"C5"}.
...
Welcome to Scala 3.5.2 (26.0.2, Java OpenJDK 64-Bit Server VM).
Type in expressions for evaluation. Or try :help.

scala> import music.*

scala> Pitch("Db5").number
val res0: Int = 73

scala> Pitch("C") + 1
val res1: music.Pitch = Pitch(Db4)
\end{REPL}
...
I case-klassen \code{Chord} nedan finns en metod \code{name} som definierar vilka intervall som ingår i de olika ackordtyperna ovan, utom maj-sju-ackord. Den krångliga modulo-12-omräkningen innan matchningen gör så att intervall i olika oktaver behandlas lika, även för negativa intervall.
...
\Subtask Testa din kod och kontrollera så att ackordet \code{Chord("D4","Gb4","A4","Db5")} får namnet \code{"Dmaj7"}

\begin{REPL}
scala> Chord("D4","Gb4","A4","Db5").name()
res2: String = "Dmaj7"
\end{REPL}
