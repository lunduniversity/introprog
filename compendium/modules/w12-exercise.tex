...
\Subtask Du ska nu skapa en enkel webbserver. Den ska svara om du surfar till \\  \url{http://localhost:8089/fib/13} \\med det 13:e Fibbonnaci-talet\footnote{\href{https://sv.wikipedia.org/wiki/Fibonaccital}{https://sv.wikipedia.org/wiki/Fibonaccital}}. \\
Spara din webserver från föregående deluppgift som \texttt{fibserver.scala} och använd koden nedan och lägg till och ändra så att din server kan svara med Fibonaccital. 
\begin{CodeSmall}
  object compute {
...
\Subtask Vi ska nu använda det större heap-minnet i stälet för stack-minnet och därmed inte begränsas av stackens max-storlek. Skriv om \code{fastFib} så att den använder en \code{while}-sats i stället för ett rekursivt anrop. Denna uppgift är ganska svår, men om du kör fast kan du ju snegla i lösningarna i Appendix för inspiration. 
