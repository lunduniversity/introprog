...
\TODO Uppdatera denna övning så den drar nytta av nya api på kurser.lth.se, se även \url{https://github.com/lunduniversity/lth-courses/issues/3}

\Subtask Via denna URL kan du ladda ner en tab-separerad lista med alla kurser som ges på LTH under innevarande läsår: \url{https://fileadmin.cs.lth.se/pgk/lthkurser201819.txt} \\Vilken data finns i filen? Du kan undersöka detta t.ex. med:
\begin{REPLnonum}
...
\SubtaskSolved
Binärsökningen blir typiskt ungefär 50 gånger snabbare (exakt faktor varierar med maskin och datamängd). Det beror på att binärsökning är O(log n) medan linjärsökning är O(n) --- skillnaden växer med storleken på den sorterade samlingen.
...
\SubtaskSolved
\emph{Ledtråd.} Bygg upp resultatet i en \code{ArrayBuffer} och stoppa in varje element på rätt plats med \code{insert}; rad 3 i pseudokoden görs med en \code{while}-loop som söker fram positionen så länge föregående element är större. \code{insert} kan stoppa in på position \code{size} (precis efter sista elementet), men \code{insert} på \code{size + 2} ger \code{IndexOutOfBoundsException} eftersom positionen ligger utanför det tillåtna intervallet 0 till \code{size}.
...
\SubtaskSolved
En tråd skapas med \code{new Thread(...)} men startar inte förrän \code{start} anropas; \code{run}-metoden körs då i en egen tråd. Utan anrop av \code{start} händer ingenting parallellt.
...
\SubtaskSolved
\code{onComplete} registrerar en callback som körs (asynkront, på en annan tråd) när \code{Future} har slutförts, med ett \code{Try}-värde som är antingen \code{Success(resultat)} eller \code{Failure(undantag)}.

\SubtaskSolved
En callback för felhantering körs endast när \code{Future} misslyckas med ett undantag (\code{Failure}); vid \code{Success} körs den inte. I nyare Scala registreras detta med \code{onComplete} och matchning på \code{Failure} (metoden \code{onFailure} är föråldrad).
...
\TaskSolved \what
\emph{Lösningsskiss --- öppen uppgift.} Klasserna i \code{java.util.concurrent.atomic} (t.ex. \code{AtomicBoolean}, \code{AtomicInteger}, \code{AtomicReference}) erbjuder trådsäker uppdatering utan lås via metoder som \code{get}, \code{set}, \code{getAndSet} och \code{compareAndSet}. Uppdatera t.ex. en gemensam \code{AtomicReference} från flera \code{scala.concurrent.Future} samtidigt --- eftersom uppdateringen är atomär uppstår inga race conditions även om flera trådar skriver samtidigt. (Notera: \code{AtomicDouble} finns inte i standardbiblioteket utan t.ex. i Guava; använd \code{AtomicReference[Double]} eller \code{DoubleAdder} i stället.)
...
//    https://github.com/lunduniversity/introprog/blob/master/
//      compendium/examples/simple-web-server/webserver.scala
...
// ändra handleRequest i start i webserver.scala till
  def handleRequest(cmd: String, uri: String, socket: Socket): Unit = 
    ...
    val response: String = 
      if afterSlash.startsWith("fib/") then 
        fibResponse(afterSlash.stripPrefix("fib/"))
...
Kör i terminalen med \texttt{scala run webserver.scala} och beskriv vad som händer i din webbläsare när du surfar till servern.
