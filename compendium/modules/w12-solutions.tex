...
\Subtask \code{error: Cannot find an implicit ExecutionContext}. \code{Future} behöver en \code{ExecutionContext} för att kunna köras. \code{f} är av typen \code{Future[Unit]}.

\Subtask Funktionen \{printLater} har en \code{Future}, vilket innebär att när både \code{printLater} och \code{println} anropas i \code{foreach}-loopen exekveras dom jämnlöpande. Eftersom det tar längre tid att starta upp en \code{Future} för datorn är \code{println} snabbare och skriver ut \code{''alla är igång!''} först. Sedan skrivs \code{1 - 42} ut med oregelbundna mellanrum eftersom tråden pausas olika länge.

\Subtask \code{big} är en \code{Future[Int]}. Det stora talet har 7 520 383 siffror. \code{r} är av typen \code{Try[Int]} (se dokumentationen för \code{Future} om du är osäker)

\Subtask Eftersom exekveringen blockas tills den har fått ett resultat går det inte att fortsätta skriva i REPL medan uträkningen pågår. Väntar man för kort tid får man ett \code{TimeOutException} och uträkningen avbryts.
