...
scalaVersion := "2.12.7"
\end{Code}
\caption{En minimal konfigurationsfil för \sbt. Filen ska ha namnet \code{build.sbt} och vara placerad i projektets baskatalog.}
...
Med följande text i \code{build.sbt} får du automatisk nedladdning och tillgång till kursens Scala-bibliotek \texttt{introprog} med bl.a. klassen \code{PixelWindow}:

\begin{Code}
scalaVersion := "2.12.7"
libraryDependencies += "se.lth.cs" %% "introprog" % "1.1.1"
\end{Code}
Ändra ev. versionsnummer till senaste versionen. Du kan läsa mer om \code{introprog} här: \url{http://cs.lth.se/pgk/api}
...
Till exempel kan du lägga till paketet \code{jline}\footnote{\url{https://github.com/jline/jline2}} som gör det möjligt att göra terminalinläsning från tangentbordet med många bra finesser, t.ex. kommandohistorik med pil-upp, bara genom att lägga till denna rad i din \code{build.sbt} där \code{"2.14.5"} anger en specifik version.
