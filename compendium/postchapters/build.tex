...
\noindent Scala CLI kan skapa dokumentation baserat på dokumentationskommentarer (se vidare Appendix \ref{appendix:doc}), enligt nedan. Med optionen \code{--ouput} kan du ange destinationskatalog och med \code{--force} så skrivs ev. gammal dokumentation över.
\begin{REPLsmall}
...
\noindent I stället för att använda optioner i terminalen så kan du ge dessa som s.k. användningsdirektiv \Eng{using directives} i ''magiska'' kommentarer som borjar med \code{//> using} i valfri kod-fil. Om du har flera kodfiler i samma katalog brukar man skapa en speciell fil som heter \code{project.scala} och samla alla dessa magiska kommentarer där. Här visas ett exempel hur det kan se ut:
\begin{Code}
//> using scala 3.3
