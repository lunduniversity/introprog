...
\node(inptext) [right of=input, text width=2cm, xshift=1.5cm]{\ifswedish För\\människor\else For\\humans\fi};
\node (compile) [process, below of=input] {\bf\sffamily \ifswedish Kompilator\else Compiler\fi};
%\node(explain) [right of=compile, text width=5cm, xshift=3.0cm]{Översätter från källkod till maskinkod};
\node (output) [startstop, below of=compile] {\bf\sffamily \ifswedish Maskinkod\else Machine code\fi};
\node(outtext) [right of=output, text width=2cm, xshift=1.5cm]{\ifswedish För\\maskiner\else For\\machines\fi};
