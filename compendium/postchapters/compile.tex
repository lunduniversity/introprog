...
\end{enumerate}
\end{enumerate}
Kontrollera att terminalkommandot \texttt{scala} nu kan användas för att starta Scala REPL på din dator:
\begin{REPLnonum}
$ scala
...
För många språk, t.ex. Scala och Python, finns det ett interaktivt program ämnat för terminalen som gör det möjligt att exekvera enstaka programrader och direkt se effekten. Ett sådant program kallas \textit{Read-Evaluate-Print-Loop} (REPL), eftersom den tolkar en rad i taget och översätter till maskinkod som körs direkt och resultatet av evalueringen av din kod skrivs ut i terminalen.
...
\textit{Kommando} & \textit{Förk.} & \textit{Beskrivning} \\ \hline 
 ...
 \texttt{:load}     & \texttt{:lo} & ladda och köra en hel fil med kodrader i Scala REPL\\
 \texttt{:warnings} & \texttt{:w}  & visa beskrivning av ev. kompilatorvarningar \\ 
