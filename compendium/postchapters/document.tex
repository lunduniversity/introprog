...
\node (input) [startstop] {\bf\sffamily \ifswedish Källkod\else Source code\fi};
\node(inptext) [right of=input, text width=6cm, xshift=4.2cm]{\ifswedish med speciella dokumentationskommentarer före deklarationer\else with special documentation comments before declarations\fi};
\node (compile) [process, below of=input] {\bf\sffamily \ifswedish Dokumentationsverktyg\else Documentation tool\fi};
%\node(explain) [right of=compile, text width=5cm, xshift=3.0cm]{Översätter från källkod till maskinkod};
\node (output) [startstop, below of=compile] {\bf\sffamily \ifswedish Dokumentation\else Documentation\fi};
\node(outtext) [right of=output, text width=6cm, xshift=4.2cm]{\ifswedish t.ex. en webbsajt med dokumentation och navigationslänkar\else e.g. a website with documentation and navigation links\fi};
...
I terminalen skriv \\\code{scala doc . -o api} 
...
Placera programmet i dokumentationsmappen och kör det med \code{scala run .} när du är i dokumentationens mapp. Du öppnar sedan dokumentationen genom filen \texttt{index.html} i din webbläsare. \\
