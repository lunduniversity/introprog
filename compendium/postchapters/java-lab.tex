...
%  \item Speltiden för varje spelomgång ska mätas och sparas tillsammans med poängresultatet för respektive spelare.
  ...
  \item Koden i singelobjektet \code{UserInterface} ska använda omvandlingsmetoden \code{asScala} efter \code{import scala.jdk.CollectionConverters._} för att omvandla argument av typen \code{java.util.ArrayList} till \code{scala.collection.mutable.Buffer}\footnote{Notera att \code{asJava} på \code{Buffer} ger en Java-samling av typen \code{List}.}.
