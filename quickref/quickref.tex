...
Assignment operator & \code|x += 1|  &  \multicolumn{3}{l}{\Comment{expanded to x = x + 1 and works for all symbolic infix operators ending with =}} \\
\vspace{-0.4em}\\
...
\vspace{0.5em}\head{Enumerations}\vspace{0.75em}
...
\vspace{-0.75em} 
{\small \texttt{Array} has efficient \code{update}, but strange with generics. Special \code{Array} allocation syntax: \code{new Array[Int](n)}  ~~~Prefer \code{ArraySeq} (mutable or immutable, a "normal" collection better with generics) or \code{IArray} (prevents mutability)}
