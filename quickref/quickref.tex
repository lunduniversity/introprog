...
\LangTitle{Scala 3.3 Quick Ref @ Lund University}
...
\raggedright{\small
A compilation unit (here hello.scala) consists of top-level definitions such as val, var, def, import, class and object, which may be preceded by a package clause, e.g.: \code|package x.y.z |that places the compiled files in directory x/y/z/ in .scala-build

\begin{tabular}{@{}r @{\hspace{0.5em}}l} \\[-0.5em]
\textbf{Compile}:  & \verb|scala-cli compile .| \\
\textbf{Run}:  &  \verb|scala-cli run . -- Earth Moon| \\[0.1em]
...
%%% Alternative 1 (for saving space) %%%

% {\small \textbf{Option[T]} is like a collection with zero or one element. \textbf{Some[T]} and \textbf{None} are subtypes of Option. }
{\small \textbf{Option[T]}, subtypes: \textbf{Some[T]} and \textbf{None}. \textbf{Try[T]}, subtypes: \textbf{Success[T]} and \textbf{Failure[E]}. \textit{Conversion, use}: toOption} \\
{\small Predicates on \textbf{Option}: isEmpty, isDefined. Predicates on \textbf{Try}: isSuccess, isFailure} \\
{\small Rule of thumb: collection-like methods work on both \textbf{Option[T]} and \textbf{Try[T]}, e.g. foreach, map, filter, toVector, ...}
...
%%% Alternative 2 (for saving space) %%%

% {\small \textbf{Option[T]} is like a one element collection. \textbf{Option[T]} is either \textbf{Some[T]} (value present) or \textbf{None} (value absent).} \\
% {\small \textbf{Try[T]} is either \textbf{Success[T]} (value present) or \textbf{Failure[E]} (E is an exception). \textit{From} \textbf{Try[T]} \texit{to} \textbf{Option[T]}, \textit{use}: toOption} \\
% {\small Rule of thumb: collection-like methods work on both \textbf{Option[T]} and \textbf{Try[T]}, \textit{e.g.} foreach, map, filter, toVector, ...} \\
% {\small \textit{Predicates} on \textbf{Option[T]}: isEmpty, isDefined. \textit{Predicates} on \textbf{Try[T]}: isSuccess, isFailure}
...
% \multicolumn{2}{l}{\hspace{-0.62em}\code|val maybe: Option[String]| / \code|val maybe: Try[String]|} \\

% \code|maybe.get|
% & \Comment{if maybe == Some[T] / Success[T] then x else throws exception}\\

% \code|maybe.getOrElse(expr)|
% & \Comment{if maybe == Some[T] / Success[T]  then x else expr}\\
% \end{tabular}

% \begin{tabular}{@{}l @{\hspace{1.0em}}r}
% \code|maybe match { case Some(x) / Success(x) => expr1; case None / Failure(e) => expr2 } |
% & \\ \Comment{expr1 if Some(x) / Success(x) else expr2}\\
% \end{tabular}
% }
