...
& \Comment{a \textbf{pattern binders} with the @ sign binds a variable to (part of) a pattern} \\ 


\code|  case x => expr | 
& \Comment{\textbf{untyped variable pattern} matches any value, typical ''catch all'' at bottom: \textbf{\texttt{case \_ =>}}} \\ 

\code|}                         | & \Comment{Pattern matching on direct subtypes of a \textbf{sealed} class is checked if exhaustive by the compiler} \\
\end{tabular}
}% small
...
%\Newline
\vspace*{-1.25em}\head{Methods in trait \texttt{Set[A]}}\Newline
