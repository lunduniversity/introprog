...
\code{Random.nextPrintableChar()} \hfill \Comment{A random \code{Char} including \code{A} to \code{Z} or any other printable char.}\\
\code{Random.nextGussian()} \hfill \Comment{A random \code{Double} that is normally distributed with mean 0 and std dev 1.}\\
\code{Random.between(a, b)} \hfill \Comment{A random number (a, b is Int, Float or Double) uniformly distributed in \texttt{a.until(b)}}\\
...
\LangTitle{\color{gray}{Scala Native (LLVM, gcc)}}
\vspace{1em}
