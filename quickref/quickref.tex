...
\vspace{0.0em}\head{Expressions (continued)}\Newline
...
2-tuple value   & \code|(1, "hello")| &  \Comment{same as Tuple2(1, "hello")} & \multicolumn{2}{l}{\code|a \% b|  \Comment{~fulfills: (a / b) * b + (a \% b) == a}}   \\ 
...
\multicolumn{5}{l}{\hspace{-0.7em}Methods on tuples:~~\code|apply drop take head tail zip toArray toIArray toList| }\\[0.25em]
...
Assignment operator & \code|x += 1|  &  \multicolumn{3}{l}{\hspace{-0.7em}\Comment{expands to~~\code|x = x + 1|~~if no method += is available, works for all operators}} \\[0.25em]
...
\vspace*{-2.5em}\head{Extension methods}\Newline
{\small
...
extension (xs: Seq[Double])                        // note: no trailing colon
  ...
  def range: (Double, Double) = (xs.min, xs.max)
  def midrange: Double = Seq(xs.max, xs.min).mean
\end{Code} 
\vspace*{-0.95em}\begin{Code}
...
\vspace{-0.0em}\head{Contextual abstractions}\Newline
...
\begin{tabular}{@{}l @{\hspace{1.0em}}l}
  \code|rnd().isRight rnd().isLeft| & \Comment{Predicates to test if normal och error.} \\
  \code|rnd().merge| & \Comment{Unraps any value, type is common supertype, here \code+String | Int+} \\
