...
\definecolor{scalared}{RGB}{222, 52, 35}
\definecolor{pagenumcolor}{RGB}{10, 50, 200}
...
\newcommand{\PageNumColor}{pagenumcolor}
...
\lstdefinelanguage{Scala}{
  morekeywords={abstract,case,catch,class,def,%
    ...
    derives, end, extension, infix, inline, opaque, open, transparent, using}, % soft keywords excluding as
  otherkeywords={=>,<-,<:,>:,@,=>>,?=>},
  sensitive=true,
  morecomment=[l]{//},
  morecomment=[n]{/*}{*/},
...
\newcommand{\PageNumRect}[5]{
  \tikz[overlay, remember picture, circle, inner sep=2pt,minimum height=0.70cm] 
    \node[fill=#2,text=white,rotate=0] at #4 (name) {\centering\large\textbf{#1~\thepage}};% ~~{\thepage(#5)}}; 
}

\newcommand{\PageNumRectOdd}[4]{\PageNumRect{#1}{#2}{#3}{($(current page.north east)-(0.55,#3)$)}{#4}}
\newcommand{\PageNumRectEven}[4]{\PageNumRect{#1}{#2}{#3}{($(current page.north west)-(-0.55,#3)$)}{#4}}

\newcommand{\PageNumMarker}[3]{%param 1 = text, param 2 = offset from top
%\fancyhead{} % clear all header fields
\fancyfoot{} % clear all footer fields
%\fancyfoot[RO]{\thepage}
%\fancyfoot[LE]{\thepage}
\fancyhead{
\ifodd\thepage\PageNumRectOdd{#1}{\PageNumColor}{#2}{#3}
\else\PageNumRectEven{#1}{\PageNumColor}{#2}{#3}
\fi
}
\renewcommand{\headrulewidth}{0pt}
\renewcommand{\footrulewidth}{0pt}
...
\begin{document}
\PageNumMarker{}{0.75}{12}
%\LangMarker{Scala}{1.5cm}{12}
\LangTitle{ \vspace*{-1.0em} Scala 3.7 QuickRef @ Lund University}
...
%Example source file \code{hello.scala} with top-level definitions: 
\small
In file \texttt{hello.scala} (any \Comment{using}-directives must come first)
\begin{Code}
//> using scala 3.8.3
@main def hi(n: Int) = println("hi " * n)
\end{Code}%
\textbf{Compile and run single file:}\\
\verb|scala run hello.scala -- 42|\\
Main program arguments after \code{--}\\[0.2em]
\textbf{Compile all scala \& java files in current dir:}\\
\verb|scala compile . -w|\\
where \code{-w} or \code{--watch} recompile on change\\
the dot \code{.} gives access to code in current dir.
\\[0.2em]

\end{minipage}%
\hfill\begin{minipage}{0.42\linewidth}
\raggedright{\small\vspace{0.2em}
A compilation unit, e.g. \code{hello.scala}, can have top-level definitions such as \code{val}, \code{var}, \code{def}, \code{import}, \code{class} \code{object}, \code{trait}, ... which may be preceded by a package clause, e.g.: \code|package x.y.z | creates a name space and places the compiled bytecode in dir \code{.scala-build} under  \code{x/y/z/}\\[0.5em]

\textbf{Run all scala \& java files in current dir:}\\
\verb|scala run . -- 42|\\Main program arguments after optional \code{--} 
\\[0.2em]
\textbf{Start REPL:} ~\verb|scala repl .|\\[0.25em]
Before Scala 3.5 use: ~\verb|scala-cli repl .|\\[0.2em]
\hfill

%\begin{tabular}{@{}r @{\hspace{0.5em}}l} \\[-0.5em]
%\textbf{Compile}:  & \verb|scala compile hello.scala| \\
%\textbf{Run}:  &  \verb|scala run . -- Ada Joe| \\[0.1em]
...
A \textbf{definition} binds a name to a value/implementation, while a \textbf{declaration} just introduces a name (and type) of an abstract member. Template bodies with curly braces \code|{ ... }| are optional and can be replaced by \code{:} that opens an indentation region with significant leading whitespace.~Implementations after \code{=}~~also opens an indentation region.
...
& \code|  def member = "hi" | \Comment{Objects can contain definitions of members such as \code|def|, \code|val|, \code|var|, \code|object|, etc.}
\end{tabular}

\begin{tabular}{@{}p{\FirstColWidth} l l}\MoveUp
Class
& \code|class C(val x: Int): | & \hspace{-0.7em}\Comment{A template for objects to be allocated with \textbf{new} or apply.} \\
& \code|  def myMethod = ??? | & \hspace{-0.7em}\Comment{Members indented after colon, or use curly braces instead of colon.} \\[0.25em]
& \code|case class C(x: Int)| & \hspace{-0.7em}\Comment{Case class parameters become val members,} \\
...
\begin{tabular}{@{}p{\FirstColWidth} l l}\MoveUp
Trait
& \code|trait T(val x: Int):| & \hspace{-0.0em}\Comment{A trait is like an abstract class, but can be mixed in.}\\
& \code|  def myAbstractMethod: Int| & \hspace{-0.0em}\Comment{An abstract member declaration with no implementation.}\\[0.25em]
Mixin & \code|class C extends D, T| & \hspace{-4.2em}\Comment{A class can only \textbf{extend} one class, but \textbf{mix in} many traits separated with comma}\\
\end{tabular}

\begin{tabular}{@{}l l l}\MoveUp
Type Alias
& \hspace{-0.8em}\code|type A = AnotherType | & \Comment{Defines an alias A for the type AnotherType. Abstract if no \code|= ...|}
\end{tabular}
...
\vspace{-0.4em}\hspace{0.32em}\begin{minipage}{0.65\linewidth}%
...
\code|val f: [A] => Seq[A] => A = [A] => xs => xs.head| \hfill\Comment{polymorfic function type and lambda}\\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\vspace{0.25em}\head{Expressions}\Newline
...
try       &  \code|val result =  |  & \Comment{Evaluate function f: Throwable => T if exception thrown by expr} \\
...
\begin{tabular}{@{}l @{\hspace{-0.5em}}r @{\hspace{0.6em}}l | r l}
...
Placeholder syntax  & \code|_ + 1|  &   \Comment{same as x => x+1, if arg used once} & \code|+ -|\\

Object creation  & \code|new C(1,2)|  &   \Comment{class args (1,2) new is optional} & \code|* / %|\\

Self reference  & \code|this|  &  \Comment{refers to the object being defined} &  other special chars & \textbf{highest}  \\
\vspace{0.4em}

Supertype reference & \code|super.m|  &  \Comment{refers to member m of supertype} &\\ %\cline{4-5}
\end{tabular}
}%end small
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\vspace{0.25em}\head{Enumerations}
...
\code|  case Red, Green, Blue| & \texttt{Col.values} \Comment{returns} \texttt{Array(Col.Red, Col.Green, Col.Blue)} \\[0.25em]
%\vspace*{-0.5em}
...
\code|  case F extends Bin(0)| & \Comment{get parameter from case value:} \code|Bin.F.toInt == 0|\\
...
\vspace{-0.2em}\head{Contextual abstractions: \texttt{given}, \texttt{using}, \texttt{summon[T]}}\Newline 
{\small
\code|enum Lang { case En, Sv }| \hfill\Comment{enum assumed in examples below} \\
\code|case class Config(lang: Lang) | \hfill\Comment{case class assumed in examples below} \\[0.25em]

\textbf{Contextual abstraction} allows values to be inferred, and arguments to be filled in, based on expected type.
\code|object Config:|  \hfill\Comment{Given values in companion have lowest priority.}  \\
\code|  given default: Config = Config(Lang.En)|  \hfill\Comment{A given instance, the name \code|default:| is optional.} \\[0.3em]
\code|def greet(name: String)(using cfg: Config) = | \hfill\Comment{A using parameter, the name \code|cfg:| is optional.}\\ 
\code|  if cfg.lang == Lang.Sv then s"hej $name"| \hfill\Comment{If unnamed use:} \code|summon[Config].lang|\\
\code|  else "hello $name"| \hfill\Comment{where summon returns given of specified type}\\[0.3em]
A \code|using| argument can be omited: \code| greet("Anna")| \hfill\Comment{A value of given type is inferred, here Config.default}  \\
...
A ''\textbf{type class}'' in Scala is a \code{trait} with one type parameter and at least one abstract method. Example:\\[0.25em]
\code|trait CanShow[T]:| \hfill\Comment{A trait with one type parameter T}\\
\code|  extension (x: T) def show: String| \hfill\Comment{An abstract member with a parameter of type T} \\[0.5em]

Separately define given instances of type class CanShow for any type in any local scope:\\[0.25em]
\code|given CanShow[Config] with| \hfill\Comment{keyword \code{with} needed when implementing members of given types}
\code|  extension (x: Config) def show: String = s"Language is " + x.lang|\\[0.25em]
\Comment{Extension method show in type class CanShow is now available on Config implicitly:}\hfill \code|Config(Lang.En).show|
...
\vspace{-0.0em}\includegraphics[width=1.05\textwidth,trim=12mm 0 0 0cm]{../img/hierarchy.pdf}


\vspace*{0.5em}\head{Numbers}%\Newline
...
& \code|1 to 4| & \Comment{Range.inclusive(1, 4), incl. 1,2,3,4}\\ 
...
\code|hypot(x, y)| \Comment{ $\sqrt{x^2 + y^2}$ } \code|log(x)| \Comment{natural logaritm} \\
...
\Newline\vspace*{-0.25em}\head{\code{scala.util.Random}}\Newline
{\small
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\vspace*{-2.5em}\head{The Scala Standard Collection Library}
{
...
%\vspace{0.2em}\texttt{String} and \texttt{Array} has implicit conversions that\\ make sequence methods work as for other sequences. \\
...
{\small%
\code{List(0, 0, 0); List(List(0), List(0))}  \hfill \Comment{ same as } \code{List.fill(3)(0); List.fill(2,1)(0)}\\
\code{Map(1 -> "Sweden", 2 -> "Norway")} \hfill\Comment{same as }
\code{Map((1, "Sweden"), (2, "Norway"))}\\
\texttt{Vector.tabulate(3)(i => i + 10) } \hfill\Comment{gives } \code{Vector(10, 11, 12)}\\
\texttt{Vector.iterate(1.2, 3)(\_ + 0.5)} \hfill\Comment{gives } \texttt{Vector(1.2, 1.7, 2.2)}\\
\code{collection.mutable.Set.empty[Int]} \hfill\Comment{same as } \code{collection.mutable.Set[Int]()} \Comment{ etc.}\\
\Comment{On \textbf{mutable} and immutable Set, Map, ArraySeq, etc.:} \hfill \code{toSet}, \code{toMap}, \code{toSeq} \Comment{ etc. returns \textbf{immutable} collection}\\
%\code{Array.ofDim[Int](3,2)} \Comment{gives} \texttt{Array(Array(0, 0), Array(0, 0), Array(0, 0))} \Comment{same as} \texttt{Array.fill(3,2)(0)} 
}



\vspace{0.5em}\head{String \hfill implicitly \texttt{Seq[Char]} via \texttt{immutable.StringOps} }\Newline
...
Strings are implictly treated as \textbf{\code|Seq[Char]|}, so all \code|Iterable| and \code|Seq| methods also work.
...
\vspace*{-2.5em}\head{\texttt{Iterable[A]} ~~(continued)}\Newline
...
%\textbf{What} & \textbf{Usage} & \textbf{Explanation} \texttt{f} is function, \texttt{pf} is partial funct., \texttt{p} is predicate.\\ \hline
  ...
  Indexing & \texttt{xs(i) xs.apply(i) xs.lift(i)} & The element of xs at index i or IndexOutOfBoundsException. \newline \texttt{Some(x)} if i is an existing index, otherwise \texttt{None}\\   \cline{2-3}
...
\clearpage
%\Newline
\vspace*{-2.5em}\head{Mutation methods in \texttt{mutable.\{ArraySeq[A], ArrayBuffer[A], ListBuffer[A]\}}}\Newline%
\vspace*{-0.5em}%
{\small\renewcommand{\arraystretch}{1.125}
\begin{tabular}{@{}p{5.3cm}  p{6.6cm}}
...
\texttt{xs.append(x)~xs.addOne(x)~~xs~+=~x} & Insert x at end.  Return xs itself.\\   \cline{1-2}

\texttt{xs.prepend(x)~~~~x~+=:~xs} & Insert x in front.  Return xs itself.\\   \cline{1-2}
...
\texttt{xs.clear()~~xs.sortInPlace} & Remove all elements, return Unit. Sort in place, return itself. \\\cline{1-2}

\texttt{xs.filterInPlace(p).mapInPlace(f)} & ArrayBuffer, ListBuffer: update in place. Return itself. \\\cline{1-2}

\end{tabular}
}

\Newline
\vspace*{-0.5em}\head{\texttt{Set[A]}}%
\Newline\vspace*{-0.5em}%
...
\Newline
\vspace*{-0.75em}\head{Mutation methods in \texttt{mutable.Set[A]}}%
\Newline\vspace*{-0.5em}%
{\small\renewcommand{\arraystretch}{1.125}
\begin{tabular}{@{}p{5.1cm}  p{6.8cm}}

\texttt{xs~+=~x~~xs.addOne(x)~xs~-=~x} & Returns the same set with included/excluded element x. \\   \cline{1-2}

\texttt{xs ++= ys~~~xs.addAll(ys)} & Adds all elements in ys to set xs and returns itself. \\\cline{1-2}

\texttt{xs.add(x)~~~xs.remove(x)} & Adds/removes x to xs and returns true if xs was mutated. \\\cline{1-2}
...
\texttt{xs.filterInPlace(p).mapInPlace(f)} & Update in place, no duplicates remain. Returns itself.\\\cline{1-2}

\end{tabular}
}

\vspace*{0.7em}\head{\texttt{Map[K, V]}}%
\Newline\vspace*{-0.5em}%
...
\vspace*{0.25em}\head{Mutation methods in \texttt{mutable.Map[K, V]}}%
\Newline\vspace*{-0.5em}%
...
\texttt{ms.mapValuesInPlace(f)} & Update all values in place by applying f: (K, V) => V to each pair.\\   \cline{1-2}

\texttt{ms.filterInPlace(p)} & Filter in place, keep elems that satisfy p. Returns xs itself. \\\cline{1-2}
...
\vspace{0.5em} \head{\texttt{java.lang.Array[A]} \hfill implicitly \texttt{mutable.Seq[A]} via \texttt{ArrayOps}}\Newline

{\small \texttt{Array} has efficient \code{update}, but is strange with generics, and gives reference equality on \code{xs == ys}. \Comment{Shallow equality test:} \code{xs.sameElements(ys)} \hfill \Comment{Deep equality test:} \code{java.util.Arrays.deepEquals(xs, ys)}\\
 \code|val xs = new Array[Int](n)| \hfill\Comment{Allocate n Int values initialized to default value for number types: 0}\\
 \code|val ys = new Array[String](n)| \hfill\Comment{n String references initialized to default value of reference types: \code|null| }\\
\Comment{Copy from start:} \code|Array.copyOf(xs, newLen)| \Comment{From pos:} \code|Array.copy(xs, pos, xs2, toPos, n)| \\
\code{Array.ofDim[Int](3,2)} \Comment{gives} \texttt{Array(Array(0, 0), Array(0, 0), Array(0, 0))}\\
\textbf{Prefer:} \Comment{''normal'' collection:} \code{immutable.ArraySeq},~ \code{mutable.ArraySeq}, ~or immutable \code{IArray}\\
}
...
\vspace*{0.5em}\head{scala.util.\{Try, Success, Failure\} to handle exceptions as values
%, scala.concurrent.Future
}\Newline
...
\code|Try(expr1).recover{case e: Exception => expr2}| & \hspace{5.0em}\Comment{Success(expr2) if exception else Success(expr1)} \\
\end{tabular}
\code|Try(1/0) match |\\
...
\vspace{0.5em}\head{scala.io.\{Source, StdIn\} \hfill java.nio.file.\{Path, Paths, Files\} }\Newline
{\small
Alternative to scala.io, java.nio: use \textbf{Scala toolkit} os.write os.read etc, see page 12, which normally is preferred.


\vspace{1em} \Comment{\textbf{Read} string of lines from \textbf{file}, \code|fromFile| gives BufferedSource, \code|getLines| gives Iterator[String]}
...
{\small
\vspace{1em}\Comment{\textbf{Read} string from \textbf{standard in} (prompt string is optional) using readLine; \textbf{write} to \textbf{standard out} using println:}
\code|val input = scala.io.StdIn.readLine("> ") | \Comment{Reads keystrokes in terminal after printing prompt >} \\

\vspace{1em}\Comment{\textbf{Write} string to \textbf{file} using java.nio:}\\[0.5em]
\code|import java.nio.file.{Path, Paths, Files}|
\code|import java.nio.charset.StandardCharsets.UTF_8| \\[0.2em]

\begin{Code}
def save(fileName: String, data: String): Path =
  Files.write(Paths.get(fileName), data.getBytes(UTF_8))
\end{Code}
\Comment{Other common character encoding:} \code|java.nio.charset.StandardCharsets.ISO_8859_1| \\[0.5em]
...
\Newline\vspace*{-1.5em}\head{scala.jdk.CollectionConverters}\Newline\vspace*{-0.2em}
{\small
...
\Newline\vspace*{-0.5em}\head{Java utilities}\vspace*{1em}
{\small
\code|Thread.sleep(1000)| \hfill\Comment{Wait 1000 milliseconds.}\\
\code|System.currentTimeMillis()| \hfill\Comment{A \code{Long} with the current time in milliseconds since midnight 1970-01-01.}\\
\code|System.nanoTime()| \hfill\Comment{A \code{Long} with the number of nano seconds since system startup.}\\


\vspace{0.5em}
A \textbf{Scanner} reads tokens separated by whitespace from a String or File. \Comment{Throws \textbf{NoSuchElementException} if end of file.}
\code|val s = java.util.Scanner("hello 42 42.0 world")| \hfill\Comment{Create a Scanner from a String.}\\
\code|val f = java.util.Scanner(java.io.File("f.txt"))| \hfill\Comment{Create a Scanner from a File.}\\
\code|val in = java.util.Scanner(System.in)| \hfill\Comment{Create a Scanner that reads from standard in.}\\
\code|s.hasNext     s.hasNextLine| \hfill\Comment{True if at least one more token or line is available.}\\
\code|s.hasNextInt  s.hasNextDouble  s.hasNextBigDecimal| \Comment{  etc.} \hfill\Comment{True if number tokens is next.}\\
\code|s.next()      s.nextLine()| \hfill\Comment{Read next token or rest of line as a String.}\\
\code|s.nextInt()   s.nextDouble()   s.nextBigInteger| \Comment{  etc.} \hfill\Comment{Parse next token as number type.}\\
...
% \code|scala compile hello.scala --jvm 21| \hfill\Comment{Compile to specific JVM version, here Java JDK 21 LTS} \\[0.5em]
% \code|scala compile hello-js.scala --js| \hfill\Comment{Compile to javascript, include in HTML or run via nodejs} \\[0.5em]
% \code|scala compile hello-native.scala --native| \hfill\Comment{Compile to a binary using LLVM and gcc} \\[0.5em]

% }



\Newline\vspace*{-0.5em}\head{Create a single page web app using ScalaJS}\Newline
{\small
In file \texttt{hello-js.scala} \hfill\Comment{~ Note double colon before version when using dependencies for ScalaJS. }
\begin{Code}
//> using scala 3.8.3
...
\vspace*{0.5em}\head{Reserved words in Scala}\Newline
%% TODO: differentiate by color or something to show hard and soft keywords
...
\code{scala run hello.scala --toolkit default} \hfill\Comment{Use stable vers. of Scala toolkit, an extended std libary. }
\code{//> using toolkit default }  Directive in source. \hfill\code{//> using toolkit latest } for unstable vers. \\[0.5em]

\textbf{Manage files and os processes with \texttt{os-lib}}\\
...
\textbf{Write and run tests with \texttt{munit}}\hfill \Comment{compatible with junit, scala, sbt, metals, vscode etc}\\
In file \texttt{MyTests.test.scala} in a class extending \texttt{munit.FunSuite} as follows:\\
Run with: \code{scala test MyTests.test.scala --toolikt deafult}\\[-0.25em]
...
\vspace*{0.5em}\head{Using dependencies \hfill from Maven central, Github}\Newline\vspace*{-0.2em}
{\small
Using library dependency from Maven central, here \code{introprog} for simple graphics, as using-directive in source:\\
\code|//> using dep "se.lth.cs::introprog:1.4.0"| \hfill\Comment{Note: double colon adds Scala version to jar. }\\[0.5em]
Or as argument in terminal:
\code|scala repl --dep "se.lth.cs::introprog:1.4.0"|\\[0.5em]
