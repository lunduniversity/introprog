...
\vspace*{-2.2em}\head{Expressions (continued)}\Newline
{\small
...
\code|trait CanShow[T]:|\\
\code|  extension (x: T) def show: String|  \\[0.5em]

Separately define given instances of type class CanShow for any type in any local scope:\\[0.25em]
\code|given CanShow[Config] with| \hfill\Comment{keyword \code{with} needed when implementing given members}
\code|  extension (x: Config) def show: String = s"Language is " + x.lang|\\[0.25em]
Extension method show in type class CanShow is now available on Config implicitly:\hfill \code|Config(Lang.En).show|

\vspace{0.5em}

\textbf{Context bound}: Use colon after type parameter as a shorthand when using parameter is a type class:\\
\code|def rev[T: CanShow](x: T) = x.show.reverse  | \\\Comment{The colon syntax above reads ''A is CanShow'' and expands to a using parameter:} \\  
\code|def rev[T](x: T)(using CanShow[T]) = x.show.reverse|   
\\[0.5em]
...
\vspace{0.5em}\head{Opaque types}\Newline 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\vspace*{-2.0em}\head{The Scala Standard Collection Library}
{
...
{\small%
\code{Vector(0,0,0)} \Comment{ same as } \code{Vector.fill(3)(0)} \hfill\Comment{On mutable Set, Map:} \code{toSet}, \code{toMap} \Comment{etc. returns immutable}\\
\code{collection.mutable.Set.empty[Int]} \hfill\Comment{ same as } \code{collection.mutable.Set[Int]()}\\
\mbox{\code{Map("se" -> "Sweden","nk" -> "Norway")} \hfill\Comment{same as }
\code{Map(("se","Sweden"),("nk","Norway"))}}\\
\texttt{Vector.tabulate(3)(i => "s" + i.toString) } \hfill\Comment{gives} \code{ Vector("s0", "s1", "s2")}
...
{\small \texttt{Array} has efficient \code{update}, but is strange with generics, and gives reference equality on \code{xs == ys}. \Comment{Shallow content test:} \code{xs.sameElements(ys)} \hfill \Comment{Deep content test:} \code{java.util.Arrays.deepEquals(xs, ys)}\\
...
\head{Specific platforms: JVM, ScalaJS, Scala Native}
\vspace{1em}
...
\Newline\vspace*{-0.5em}\head{java.lang.Object, scala.AnyRef}\Newline%\vspace*{-0.2em}
%java.lang.\{Boolean, Byte, Character, Double, Float, Integer, Long, Short\

The class \texttt{java.lang.Object} corresponds to the Scala class \texttt{scala.AnyRef} including the following methods:\\[0.5em]
\code|def equals(other: Object): Boolean| \hfill\Comment{Type-unsafe equality test. Can be overridden.}\\
\code|def hashCode: Int| \hfill\Comment{An integer that is equal to every other instance's hashCode if \code{equals(other)} is true.}\\
...
\vspace{1em}TODO Recepie for equals and hashcode\\
TODO Recepie for equals and hashcode 



\vspace{1em}\head{java.lang.\{System, Thread\}}
...
\vspace{-0.1em}\head{Using the Scala toolkit: os, munit, ujson, upickle}\Newline
{\small
...
\code|os.write(wd/"f.txt", "hi")   os.read(wd/"f.txt")| \hfill\Comment{Write text to f.txt. Read f.txt as String.}\\
\code|os.copy(wd/"f.txt", wd/"cpy.txt")    os.list(wd)| \hfill\Comment{Copy f.txt to cpy.txt. List files in wd as Seq.}\\[0.5em]
...
\textbf{Write tests} in file \texttt{MyTests.test.scala} in a class extending \texttt{munit.FunSuite} as follows:\\
...
\vspace*{-0.1em}\textbf{Parse and serialize JSON} and access named values:\\
\vspace*{-0.5em}\begin{Code}
