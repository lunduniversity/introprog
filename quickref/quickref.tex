...
\LangTitle{Scala 3.0 Quick Ref @ Lund University}
...
A \textbf{definition} binds a name to a value/implementation, while a \textbf{declaration} just introduces a name (and type) of an abstract member. Below \code|defsAndDecl| denotes a list of definitions and/or declarations. Template bodies \code|{ ... }| are optional, can be replace by \code{:} that opens an indentation region.~~~ \code{=}~~also opens an indentation region
...
& \code|class C extends D, T| & \hspace{-0.9em}\Comment{A class can only \textbf{extend} one class but mix in many traits separated with \code{,}}\\
\end{tabular}
...
try       &  \code|val resultOfUnsafeExpr =  |  & \Comment{Evaluate partial function pf if exception in expr} \\
         &  \code|  try expr catch pf |  & \Comment{where pf e.g.: } \code|{case e: Throwable => someValue}|\\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\clearpage\vspace*{-2.5em}\head{Pattern matching, type tests and extractors}\vspace{0.5em}
...
\vspace{0.1em}{\small Matching with typed variable pattern \code| x match { case a: Int => a; case _ => 0}| is preferred over \\explicit isInstanceOf tests and casts: \code{ if (x.isInstanceOf[Int]) x.asInstanceOf[Int] else 0}}
...
\code|  case Red, Green, Blue| & \Comment{Array of values:} \texttt{Color.values(Col.Red.ordinal) == Col.Red} \\
%\vspace*{-0.5em}
& \Comment{value from String:} \code|Color.valueOf("Red") == Color.Red| \\ 
\code|enum Bin(val toInt: Int):| &  \Comment{parameteized enum, val is needed for class param to be externally visible.}\\
\code|  case T extends Bin(1)| & \Comment{get parameter from val:} \code|Bin.T.toInt == 1|\\
\code|  case F extends Bin(0)| & \Comment{you can also define members (def, val, etc) inside enums}\\
...
\code|val p = b.pair(new Box("zero"))  |
...
\vspace{0.5em}\head{Reading/writing from file, and standard in/out, string interpolation:}\Newline
{\small
\Comment{\textbf{Read} string of lines from \textbf{file} (fromFile gives BufferedSource, getLines gives Iterator[String]; also fromURL):}
...
{\small
Enable \code{.asJava} and \code{.asScala} conversions: \code{ import scala.jdk.CollectionConverters._ }
\vspace{0.5em}
