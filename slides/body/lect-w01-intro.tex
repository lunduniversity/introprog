...
\node (input)  [startstop]               {\ifswedish Indata-enhet\else Input device\fi};
\node (cpu)    [process, below of=input] {CPU};
\node (output) [startstop,below of=cpu]  {\ifswedish Utdata-enhet\else Output device\fi};

\node (mem) [right of=cpu, xshift=0.4\textwidth, draw = black, thick] {
\begin{minipage}{0.5\textwidth}\centering
\ifswedish\textbf{Minne} med minnesceller\else\textbf{Memory} with memory cells\fi
\vspace{1em}

\begin{tabular}{|l | l|}
address & \ifswedish innehåll\else content\fi \\ \hline
0   & 42 \\ \hline
1   & 13 \\ \hline
2   & 18 \\ \hline
3   & 21 \\ \hline
...
\item Ha picknick i \href{https://lundanamn.lund.se/wiki/Ada_Lovelace-parken}{Ada Lovelace-parken} på Brunnshög!
\end{itemize}
\end{Slide}
...
Det är ofta mycket lättare att åtgärda fel som kompilatorn pekar ut vid kompilering än att fixa ett exekveringsfel som kanske bara uppträder under vissa förutsättningar, t.ex. vid speciell indata.
...
\begin{tikzpicture}[node distance=1.4cm,scale=0.8]
  ...
  \node(inptext) [right of=input, text width=2cm, scale=0.8,xshift=1.5cm]{\ifswedish För\\människor\else For\\humans\fi};
  \node (compile) [process, below of=input] {\ifswedish Kompilator\else Compiler\fi};
  \node (output) [startstop, below of=compile] {\ifswedish Maskinkod\else Machine code\fi};
  \node(outtext) [right of=output, text width=2cm, scale=0.8,xshift=1.5cm]{\ifswedish För\\maskiner\else For\\machines\fi};
...
\node (input) [startstop] {\ifswedish Källkod\else Source code\fi};
\node (compile) [process, below of=input] {\ifswedish Kompilator\else Compiler\fi};
\node (output) [startstop, below of=compile] {\ifswedish Abstrakt ''maskinkod''\else Abstract ''machine code''\fi};
\node (interp) [process, below of=output] {\ifswedish VM interpreterar\else VM interprets\fi};
\node (output2) [startstop, below of=interp] {\ifswedish Konkret maskinkod\else Concrete machine code\fi};
\draw [arrow] (input) -- (compile);
\draw [arrow] (compile) -- (output);
...
\begin{REPLsmall}
> scala
Welcome to Scala 3.8.3 (21.0.5, Java OpenJDK 64-Bit Server VM).
...
\begin{Slide}{Installera verktyg på din egen dator}
  ...
  \begin{itemize}\SlideFontSmall
    ...
    \begin{enumerate}\SlideFontTiny
      ...
      \item Kanalen \texttt{\#frågor-och-svar} på kursens Discord-server.
      \item Fråga handledare på resurstid.
    \end{enumerate}
    
  \end{itemize}
\ifkompendium
Om du har problem med installation av verktyg på din egen dator, använd skolans datorer så att du kan dra nytta av undervisningen och inte försinkas på grund av teknikstrul. 
\fi
\end{Slide}
...
\item \ifswedish Exempel:\else Example:\fi \\
\begin{tabular}{l l}
\code|42| & \ifswedish heltalslitteral\else integer literal\fi\\
\code|42.0| & \ifswedish decimaltalslitteral\else floating-point literal\fi\\
\code|'!'| & \ifswedish teckenlitteral, omgärdas med 'enkelfnuttar'\else character literal, enclosed in 'single quotes'\fi \\
\code|"hej"| & \ifswedish stränglitteral, omgärdas med ''dubbelfnuttar''\else string literal, enclosed in ''double quotes''\fi \\
\code|true| & \ifswedish litteral för sanningsvärdet ''sant''\else literal for the truth value ''true''\fi\\
\end{tabular}
...
\item \code{Double} för decimaltal
...
\ifswedish Dessa \Emph{grundtyper} \Eng{basic types} finns inbyggda i Scala:\else These \Emph{basic types} are built into Scala:\fi

\begin{table}[H]
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{p{0.24\textwidth}|p{0.21\textwidth}|l}
\textit{\ifswedish Svenskt namn\else Swedish name\fi} & \textit{\ifswedish Engelskt namn\else English name\fi} & \Emph{\ifswedish Grundtyper\else Base types\fi} \\ \hline
...
numeriska typer & numeric types & \ifswedish heltalstyper och flyttalstyper\else integral types and floating-point types\fi \\
...
\item Delar i ett uttryck kan t.ex. vara: \\ litteraler (42), operatorer (+), funktioner (math.sin), ...
\item Exempel:
\begin{itemize}
...
\ifkompendium
\ExplainStringInterpolator
\fi 

\end{Slide}

\begin{Slide}{Heltalsaritmetik}\SlideFontTiny
\begin{itemize}
\item De fyra räknesätten skrivs ungefär som i matematiken, med samma evalueringsordning (\href{https://sv.wikipedia.org/wiki/Operatorprioritet}{wikipedia.org/wiki/operatorprioritet}):
\begin{REPL}
scala> 3 + 5 * 2 - 1
res0: Int = 12
\end{REPL}
...
val res2: Int = 20
\end{REPL}
\item \ifswedish\Emph{Moduloräkning} med restoperatorn \texttt{\%} ger resten vid vid heltalsdivision:\else\Emph{Modulo arithmetic} with the remainder operator \texttt{\%} gives the remainder in integer division:\fi
\begin{REPL}
scala> 41 % 2
val res3: Int = 1               // resten är 41 - 2 * (41 / 2) == 1
\end{REPL}
\ifswedish Läs mer om moduloräkning här: \else Read more about modulo arithmetic here: \fi \href{https://sv.wikipedia.org/wiki/Modul%C3%A4r_aritmetik}{sv.wikipedia.org/wiki/Modulär\_aritmetik}
\end{itemize}
\end{Slide}
...
\begin{itemize}
\item \ifswedish Decimaltal representeras med s.k. \href{https://sv.wikipedia.org/wiki/Flyttal}{\Emph{flyttal}} av typen \code{Double}:\else Decimal numbers are represented by so-called \href{https://sv.wikipedia.org/wiki/Flyttal}{\Emph{floating-point numbers}} of type \code{Double}:\fi
\begin{REPL}
scala> math.Pi
val res4: Double = 3.141592653589793
\end{REPL}

\item \ifswedish Stora tal så som $\pi*10^{12}$ skrivs:\else Large numbers such as $\pi*10^{12}$ are written:\fi
\begin{REPL}
scala> math.Pi * 1E12
val res5: Double = 3.141592653589793E12
\end{REPL}
\item \ifswedish Det finns \Alert{inte} oändligt antal decimaler vilket ger problem med \Alert{avvrundingsfel}:\else There is \Alert{not} an infinite number of decimals, which causes problems with \Alert{rounding errors}:\fi
\begin{REPL}
scala> 0.1 + 0.2
...
\end{REPL}
\end{itemize}
\SlideFontTiny\vspace{-0.2em}\hfill \ifswedish Läs mer här:\else Read more here:\fi~\url{https://0.30000000000000004.com}
\end{Slide}
...
def gurklängd = 42 + x
\end{Code}
\item \ifswedish Uttrycket till höger evalueras \Alert{varje} gång \Emph{anrop} sker,\\
d.v.s. varje gång namnet används på annat ställe i koden.\else The expression on the right is evaluated \Alert{every} time the \Emph{call} happens,\\
i.e. every time the name is used elsewhere in the code.\fi
\begin{Code}
gurklängd
\end{Code}
...
\else
\href{https://en.wikipedia.org/wiki/Augustus_De_Morgan}{\Emph{De Morgan's laws}} describe what happens when you \Alert{negate} a logical expression. Can be used to make \Emph{simplifications}.
\fi
...
\else
Write an \Emph{expression} when you are interested in the \Emph{value} that is computed.\\
Write a \Alert{statement} when you want something to \Alert{be done}.\\~\\
{\SlideFontSmall Both statements and expressions can in turn contain statements and expressions in arbitrarily complex \textbf{nested structures} (more on that later).}
\fi
\end{Slide}
...
\ifkompendium
\ExplainControlStructure
\else
Kontrollstrukturer kan ge \Emph{alternativa} vägar genom koden. Vägen bestäms vid körtid.
\fi
...
\end{Code}
\pause
Du kan efter vissa nyckelord (t.ex. \code{do}, \code{then}, \code{else}) välja bort klammerparenteser, s.k. {valfria klammerparenteser} \Eng{optional braces}. 
\begin{Code}
while math.random() < 0.8 do
  println("gurka")
  println("tomat")
...
\begin{Slide}{Problemlösning: nedbrytning i abstraktioner som kan kombineras}\SlideFontSmall
\begin{itemize}
...
\begin{SlideExtra}{Frågestunder vid behov utöver resurstid/labbar}\SlideFontSmall
\begin{itemize}
  \item Enkla/snabba individuella frågor på rasten i samband med föreläsningarna.
  \item Du får individuell hjälp på resurtider och labbar. Du ska gå på all schemalagd undervisning för att klara kursen och lära till max!
  \item Vid svårigheter med kursen erbjuds frågestunder med Björn Regnell
  \begin{itemize}\SlideFontTiny
    \item När: Tisdagar lunchtid 12:15-12:45
    \item Var: Samling utanför Björn Regnells kontor kl. 12:15 här:\\
    {\footnotesize\url{https://fileadmin.cs.lth.se/cs/Bilder/rum/E2411.gif}}
\end{itemize}  
  \item Studievägledare finns också till er hjälp, alla ska delta i schemalagd ''Workshop'' med Tobias Björklund och Joakim Cao, se anslag i Canvas.
\end{itemize}
\end{SlideExtra}


\begin{SlideExtra}{Kom till studierådets pluggkväll!}
\begin{itemize}
  \item Studierådet ordnar \Emph{pluggkvällar} på måndagar i iDét.
  \item Där får du plugghjälp och installationshjälp.
  \item Mer info: \url{https://www.dsek.se/nollning/events?week=1}
  \item Se även: \href{https://github.com/lunduniversity/introprog/blob/master/web/tools/tools.md}{github.com/lunduniversity/introprog}\\i filen web/tools/tools.md
\end{itemize}  
\end{SlideExtra}

\begin{SlideExtra}{IT-support för studenter i alla Datavetenskap-kurser}
\begin{itemize}
  \item Alla dagar kl 12-13 läsvecka 1--2 i E:2116. 
  \item Hitta dit: \\
  {\footnotesize\url{https://fileadmin.cs.lth.se/cs/Bilder/rum/E2116.gif}}
  \item Se även: \href{https://github.com/lunduniversity/introprog/blob/master/web/tools/tools.md}{github.com/lunduniversity/introprog}\\i filen web/tools/tools.md
\end{itemize}  
\end{SlideExtra}
