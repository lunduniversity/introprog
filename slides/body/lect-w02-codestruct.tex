...
\ifswedish Förenkla dessa if-uttryck:\else Simplify these if expressions:\fi
\begin{Code}
...
\begin{SlideExtra}{Muntaträning}
\begin{itemize}\SlideFontSmall
\item Kör muntabot \Alert{efter} att du gjort \Emph{grundövningarna} för vecka 1 \& 2
\item \url{https://fileadmin.cs.lth.se/pgk/muntabot/}
\item Välj: från vecka 1 till vecka 2 i muntabot och verifiera att du kan förklara och kontrastera veckans begrepp (grön och gul knapp i muntabot)
\item Vid behov gör utvalda uppgifter ur veckans \Emph{Extraövningar}  
\item När du har koll på begreppen: Skriv kod \Alert{självständigt} på \Emph{papper} genom att trycka på den röda ''Skriv kod''-knappen i muntabot
\item Diskutera med handledare vad du tycker är knepigt och be om pluggråd
\item Muntabot ska \Alert{inte} ersätta övningsuppgifterna; använd muntabot som stickprov för verifiering av dina kunskaper. Övningsuppgifterna i kompendiet är mer helteckande, med utförligare uppgiftsbeskrivningar och förklaringar i facit etc. Plugga i kompendiet! Kolla sen i muntabot.
\item I vecka 4 \& vecka 8: \Alert{obligatorisk kontroll} av att du gjort \Emph{muntaträning}
\end{itemize}
\end{SlideExtra}
...
\ifkompendium Scalas erbjuder många användbara samlingar i paketet \code{scala.collection} som är en del av Scalas standardbibliotek.\fi
...
\begin{itemize}\SlideFontTiny
\item sparar data i variabler, s.k. \Emph{attribut}
\item erbjuder funktioner, s.k. \Emph{metoder} (kallas även \Emph{operationer}) 
\end{itemize}
...
\ifkompendium
Vi ska fördjupa is i klasser, attribut och metoder i Kapitel \ref{chapter:W05}: \ModWeekFIVE.
...
\ifswedish En \Emph{vektor}\footnote{Vektor kallas ibland på svenska även \href{https://sv.wikipedia.org/wiki/F\%C3\%A4lt_\%28datastruktur\%29}{fält}, men det skapar stor förvirring eftersom det engelska ordet \emph{field} ofta används för \emph{attribut} (som är ngt annat).}
\Eng{vector} är en \Emph{sekvens} som är \Alert{snabb} att \Emph{indexera} i.\else
A \Emph{vector} is a \Emph{sequence} that is \Alert{fast} to \Emph{index} into.\fi
...
\end{REPL}
\SlideFontSmall \ifswedish Notera typen \code{Matchable} som betyder ''\Emph{nästan vilken typ som helst}''\else Note the type \code{Matchable} which means ''\Emph{almost any type at all}''\fi\\
%\footnote{
...
\end{REPL}
\code{for ... do ...} \ifswedish gör så att följande händer:\else makes the following happen:\fi
\begin{itemize}
...
\begin{Slide}{Samlingen {\tt Range} håller reda på intervall}
\begin{itemize}
...
Hej Scala! 
\end{REPL}
Samkör \Alert{alla} \code{.scala}-filer i \Emph{aktuell} katalog och \Alert{alla} underkataloger:\\ \code{scala run .}\\\vspace{1em} 
Innan körning samkompileras dina kodfiler automatiskt vid behov. Du kan se maskinkoden i en underkatalog i till katalogen \texttt{.scala-build}: 
\begin{REPL}
...
\ifkompendium\else
\begin{SlideExtra}{Öppna en projektkatalog med kodfiler i din kodeditor}
  \begin{itemize} \SlideFontSmall
    \item Lägg de kodfiler som hör ihop i en \Alert{egen} \Emph{katalog} (mapp, folder, directory, dir, ... kärt barn har många namn)
    \item Du kan \Alert{INTE} ha alla dina kodfiler i Downloads eller Documents eller ngn anna katalog med en massa underkataloger som har andra kodfiler som kan krocka med de du vill samkompilera just nu.
    \item I VS Code ska du öppna dina kodfiler som ett \Alert{avgränsat} \Emph{projekt} så att de samkompileras och inte blandas med andra projekt.
    \item Detta gör du med hjälp av File -> Open Folder i VS Code.
    \item Om du inte gör detta så kommer inte dina kodfiler att samkompileras och du får inte upp ''run | debug'' över ditt huvudprogram när det är felfritt.
    \item Du behöver också explicit klicka ''Trust'' när VS Code frågar om detta. Om du inte har gjort det så finns en blå knapp ''Restricted...'' nere till vänster och ingen kompilering/körning innifrån VS Code funkar. Detta krävs för att du ska tänka efter innan du råkar köra elak kod... 
  \end{itemize}
\end{SlideExtra}
\fi

\begin{Slide}{Typsäkra argument till ett program med @main}
  \SlideFontSmall
...
\end{REPL}
\ifswedish Med \code{@main} genereras automatiskt en primitiv main som kollar att argumenten har rätt typ.\else With \code{@main}, a primitive main is automatically generated that checks that the arguments have the correct type.\fi
\end{Slide}
...
\Emph{Algoritm} i \Alert{pseudo-kod} (lättläst, förenklat naturligt språk)\pause
\begin{enumerate}
...
\end{Code}
\pause \ifswedish Returtypen kan härledas av kompilatorn:\else The return type can be inferred by the compiler:\fi
\begin{Code}[basicstyle=\ttfamily\fontsize{9}{11}\selectfont]
def öka(i: Int) = i + 1
\end{Code}
...
\pause
\begin{itemize}
  \item Denna bättre \code{isHighscore} är nu en \Emph{äkta funktion} som alltid ger samma svar för samma argument och \Alert{saknar sidoeffekter}; dessa funktioner är ofta lättare att förstå.
...
\end{REPLnonum}
{\SlideFontSmall \ifswedish Namngivna argument kan ges i valfri ordning; då riskerar man inte fel ordning.\else Named arguments can be given in any order, so you don't risk the wrong order.\fi}
...
\node(inptext) [right of=input, text width=5.5cm, scale=1.2,xshift=3.5cm]{\ifswedish Källkodsfil\else Source code file\fi};
\node (compile) [process, below of=input] {\texttt{scalac}};
\node(comptext) [right of=compile, text width=7.2cm, scale=1.0,xshift=4.5cm]{\ifswedish Kompilatorn skapar \textit{abstrakt} maskinkod (s.k. bytekod)\else The compiler creates \textit{abstract} machine code (so-called bytecode)\fi};
\node (output) [startstop, below of=compile] {\texttt{hello.class}};
\node(outtext) [right of=output, text width=5.5cm, scale=1.2,xshift=3.5cm]{\ifswedish\texttt{.class}-fil med bytekod\else\texttt{.class} file with bytecode\fi};
\node (jvm) [process, below of=output] {JVM};
\node(jvmtext) [right of=jvm, text width=7.2cm, scale=1.0,xshift=4.5cm]{\ifswedish\textit{Java Virtual Machine}\\Översätter bytekod till \textit{konkret}\\ maskinkod som passar din specifika CPU \textbf{under körning} (s.k. interpretering)\else\textit{Java Virtual Machine}\\Translates bytecode into \textit{concrete}\\ machine code that fits your specific CPU \textbf{at run time} (so-called interpretation)\fi};
...
Det importerade namnet blir direkt synligt i den aktuella \Emph{namnrymden} \Eng{in scope}.
\end{Slide}
...
\item OBS! Ingen lab i schemat denna vecka - labben redovisas nästa vecka!
...
\item Mål: skapa \Emph{eget} program med \Emph{många små funktioner} och träna på de \Emph{begrepp} vi använt hittills. Ju fler begrepp du kan använda på olika sätt desto bättre. Fokusera på det \Alert{du} behöver träna mest på.
