...
\begin{Slide}{Deklarera funktioner, överlagring}\SlideFontSmall
\begin{itemize}
...
\item Att ha flera olika funktioner med samma namn kallas \Emph{överlagring} \Eng{overloading}.
\item Överlagring ger \Alert{flexibilitet i användningen} och vi slipper hitta på nytt namn.
\end{itemize}
\end{Slide}
...
\ifkompendium
En lokal funktion är en inre funktion som finns inuti en annan, yttre funktion. En lokal funktion syns ej utanför den yttre funktionen. 
\fi
Med hjälp av lokala funktioner kan delproblem lösas med nästlade abstraktioner.
...
\ifkompendium \ExplainAnonymousFunction \fi
\begin{itemize}\SlideFontSmall
...
\ifkompendium \ExplainPredicate \fi 
\begin{itemize}\SlideFontTiny
\ifkompendium\else
...
\begin{Slide}{Funktionsvärde vid tom parameterlista: använd ''thunk''}\SlideFontTiny
\begin{itemize}\SlideFontTiny

\item Vi har tidigare sett: \\Vill du ha funktionen som värde -- \Emph{skriv bara namnet} och inte parameterlistan:
\begin{REPLsmall}
scala> val f = add     // inget anrop av add sker 
val f: (Int, Int) => Int = Lambda7210/0x0000000841e4e040@1ce2db23
\end{REPLsmall}
\pause
\item MEN: Vid \Alert{tom parameterlista} behövs anonym funktion som \Emph{fördröjer anrop} för att få motsvarande funktionsvärde: 
\begin{REPLsmall}
scala> def a() = 42
...
\begin{Slide}{Hur fungerar egentligen \texttt{upprepa} i Kojo?}
...
Om en funktion har multipla parameterlistor kan man skapa \Emph{stegade funktioner} \Eng{curried functions}, även kallat \Emph{partiellt applicerade} funktioner \Eng{partially applied functions} eller \Emph{''Curry''-funktioner}.
\begin{REPLnonum}
...
\begin{Slide}{Rekursiva datastrukturer}\SlideFontSmall
\begin{itemize}\SlideFontSmall
...
\end{itemize}
\ifkompendium\else
\pause {\hfill\SlideFontTiny För den nyfikne: se filmen \href{https://youtu.be/IDatlPjZ58A?si=nD2KZx6o8ujPqT0L}{''Fatta Rekursion''} (eller vänta till pfk).}
\fi
\end{Slide}
...
\begin{itemize}\SlideFontSmall
  ...
  \item Detta görs automatiskt i vscode och du får röda understrykningar vid fel.
  \item Men du kan också använda terminalen:\\\code{scala compile . --watch}\\Ändringar bevakas och du kan tydligt se ev. felmeddelande i en kompilator-utskrift som sker först när du sparar din ändring med Ctrl+S.
  \item Du \Alert{slipper} felmeddelande som beror på att du ännu inte skrivit klart...
...
\begin{itemize}\SlideFontTiny
  ...
  \end{enumerate}
  \item Börja enkelt och bygg vidare steg för steg i \Alert{små} steg.
  \item Fixa \Alert{alla} kompileringsfel och \Alert{alla} körtidsfel \Emph{innan} du går vidare.
  \item Fokusera på kodens \Emph{läsbarhet}: Om en funktion blir stor så försök dela upp den i flera mindre funktioner med bra namn. 
