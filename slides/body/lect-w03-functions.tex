...
\begin{itemize}
\item Om en funktion \Emph{deklareras} med tom parameterlista \code{()}, eller helt utan parameterlista, så kan den \Emph{anropas} \Alert{utan parameterlista}.
\begin{REPL}
...
\item Denna flexibilitet är grunden för \Emph{enhetlig access}: namnet kan användas enhetligt oavsett om det är en funktion eller en variabel.
...
\begin{itemize}\SlideFontSmall
\item En \Emph{äkta} funktion är en funktion som ger ett resultat som \Alert{enbart} beror av dess argument.
\item En äkta funktion är \Emph{referentiellt transparent} \Eng{referentially transparent}, vilket innebär att varje anrop kan bytas ut mot funktionskroppen där parametrarna ersatts med motsvarande argument.
\item En äkta funktion har \Alert{inga sidoeffekter}, t.ex. utskrift, inläsning, skriva/läsa filer,  eller uppdatering av variabler som är synliga \Alert{utanför} funktionen.
\item Exempel:
\begin{Code}
...
\item Uttrycket \code{add(41, 1)} kan ersättas med 41 + 1 utan att det påverkar resultatet.
\item Uttrycket \code{rnd(42)} kan \Alert{inte} bytas ut mot ett visst heltal.
\end{itemize}  
\end{itemize}  
\end{Slide}
...
\begin{Slide}{Exempel på oäkta funktioner: slumptal}
  ...
  \begin{itemize}
    ...
    \item Du har nytta av funktionen \code{Random.nextInt} och slumptalsfrö i veckans uppgifter.
  \end{itemize}
...
\begin{Slide}{Begränsningar i Java}
\begin{itemize}\SlideFontTiny
