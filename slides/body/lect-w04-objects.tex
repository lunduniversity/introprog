...
\ifkompendium\else
\Subsection{Obligatorisk muntaträning w04}
\begin{SlideExtra}{Denna vecka är det obligatoriskt att göra muntaträning}
\begin{itemize}
\item Gör uppgifterna i grundövningarna för innevarande vecka
\item Repetera grundövningarna för tidigare veckor
\item Verfiera att du förstått med stickprov i muntabot: \\\url{https://fileadmin.cs.lth.se/pgk/muntabot/}
\item Redovisning av din muntaträning \Alert{på resurstid}:
\begin{itemize}
\item Gör dig redo att skriva kod med \Emph{papper \& penna}
\item Kotakta handledare som väljer koduppgift
\item Du löser koduppgiften självständigt
\item Ni diskuterar vad som är knepigt och hur du kan plugga för att öka din förmåga att självständigt skriva kod utan andra hjälpmedel än \Alert{snabbreferensen}
\end{itemize}
\end{itemize}
\end{SlideExtra}
\fi
...
\begin{itemize}\SlideFontSmall
\item Ett singelobjekt \Eng{singleton object} deklareras med nyckelordet \code{object} och används för att samla \Emph{medlemmar} \Eng{members} som \Alert{hör ihop}.
...
\begin{Slide}{Punktnotation och operatornotation}\SlideFontSmall
...
\code{objekt metod argument}
\\~\\Exempel:\\
\code{1 + 2}\pause\hfill\code{1.+(2)}\pause\\\code{1 to 42}\hfill\code{1.to(42)}
\\\pause\vspace{0.5em}
\code{Highscore isHighscore 1000}
\pause 
{
\SlideFontSmall
\\\vspace{0.5em}Metoder vars namn \Emph{börjar med bokstäver} \Alert{ger varning} vid anrop med operatornotation om ej deklarerad med \code{infix} före \code{def}, detta för att uppmuntra konsekvent användning av punktnotation.
}
\end{Slide}
...
\begin{Slide}{Modul}
  ...
  \begin{itemize}
    ...
    \begin{itemize}\SlideFontTiny
      ...
      \item Skillnader:
      \begin{itemize} \SlideFontTiny
        \item för varje paket skapar kompilatorn \Alert{underkataloger} för maskinkoden 
        \item paket kan delas upp i \Emph{flera} kodfiler -- ett objekt måste vara i \Alert{en} kodfil
        \item objekt kan \Emph{ärva medlemmar} från klasser \& traits (mer sen om detta)
      \end{itemize}
      %\item Om du vill att ditt paket ska ärva medlemmar kan du använda ett \code{package object}
    \end{itemize}
...
\item Tupler är smidiga som \Emph{parametrar} om man vill kombinera värden som hör ihop, till exempel
 x- och y-värdena i en punkt: \code{(3.0, 4.0)}
\pause
...
Med nyckelordet \code{lazy} före \code{val} sker \Alert{fördröjd} (ä.k. ''\Emph{lat}'') evaluering av initialiseringsuttrycket. \\Motsatsen (det normala i Scala) kallas \Alert{strikt} evaluering.
\begin{REPL}
...
En \code {lazy val} initialiseras \Alert{inte} vid deklarationen utan \Emph{senare} när den \Alert{refereras första gången}. 
\end{Slide}
...
\begin{Slide}{Fallgrop: initialiseringsordning och defaultvärden }
\begin{REPL}
scala> object X:
         val a = b
         val b = 42
...
Lösning: byt ordning om det går, annars gör \code{a} till en \code{lazy val}
\begin{REPL}
scala> object Y:
         lazy val a = b
         val b = 42
...
\begin{Slide}{Använda klassen \texttt{Color}}\SlideFontSmall
\begin{itemize}
...
% autotranslate build-safety clamp (below): this DISABLED slide is kept out of the English-mirror
% translation via \ifswedish..\fi, so the model cannot flakily mangle its commented-out tikz into a
% fatal compendium-en error. Renders nothing either way (SV stays commented; EN whole block skipped).
\ifswedish
...
Welcome to Scala 3.9.0 (17.0.3, Java OpenJDK 64-Bit Server VM).
...
\begin{Slide}{Köra program + kodbibliotek med Scala CLI}\SlideFontTiny
\begin{itemize}
