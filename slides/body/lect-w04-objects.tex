...
\item Senaste versionen av kursbiblioteket \code{introprog} är $\LibVersion$ och jar-filens namn är \href{http://cs.lth.se/pgk/lib}{\texttt{introprog\_2.12-\LibVersion.jar}} \\Se dokumentation här: \url{http://cs.lth.se/pgk/api}
\end{itemize}
\end{SlideExtra}
\fi
...
scala> A2.a  
\end{REPLnonum}
    \item Många andra språk har olika syntax för access av attribut och anrop av metoder (t.ex. Java, där alla metodanrop måste ha parenteser).
    ...
    \item Nackdel: Det kan bli namnkrockar mellan metoder och attribut eftersom de finns i samm namnrymd.
  \end{itemize}
...
\begin{itemize}
\item Jarfilen \code{introprog_2.13-1.1.4.jar} innehåller paketet \code{introprog} som bl.a. innehåller klassen \code{PixelWindow}.
...
\item Ladda ner \url{http://cs.lth.se/pgk/lib} och lägg sedan jar-filen i den katalog där du startar REPL med: \code{scala -cp introprog_2.13-1.1.4.jar}
\end{itemize}
%\pause
\begin{REPLnonum}
...
\begin{Code}
scalaVersion := "2.12.10"    // ange version av Scala-kompilatorn
\end{Code}
...
\code{~/.ivy2/cache/se.lth.cs/introprog_2.13/jars}
\end{Slide}
