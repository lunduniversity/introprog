...
\item Senaste versionen av kursbiblioteket \code{introprog} är \Emph{\LibVersion} och jar-filens namn är \texttt{introprog\_3-\LibVersion.jar} som kan laddas ned här: \url{http://cs.lth.se/pgk/lib}\\Se dokumentation här: \url{http://cs.lth.se/pgk/api}
\end{itemize}
\end{SlideExtra}
\fi
...
{
\SlideFontSmall
\\\vspace{0.5em}Operatornotation med metoder vars namn börjar med bokstäver kommer efter Scala 3.1 kräva deklaration med \code{infix} före \code{def}, men du kan redan nu använda \code{infix def} för att visa att de är tänkta att användas så.
}
\end{Slide}
...
\begin{itemize}
\item Jarfilen \code{introprog_3-1.3.1.jar} innehåller paketet \code{introprog} som bl.a. innehåller klassen \code{PixelWindow}.
...
\item Ladda ner \url{http://cs.lth.se/pgk/lib} och lägg sedan jar-filen i den katalog där du startar REPL med: \code{scala -cp introprog_3-1.3.1.jar}
%\pause
\begin{REPLnonum}
...
\begin{Slide}{Använda färdiga kodbiblitek med Scala CLI:}\SlideFontSmall
\begin{itemize}
\item \code{scala-cli} kan inkludera en jar-fil på classpath med optionen \code{--jar}
\begin{REPLsmall}
> curl -o introprog.jar -sL http://cs.lth.se/pgk/lib
...
scala> introprog.Dialog.show("hello introprog")
\end{REPLsmall}
\item Läs mer här: \url{https://index.scala-lang.org/lunduniversity/introprog-scalalib} och i Appendix C, stycket om Scala CLI.
\end{itemize}
...
\begin{Code}
scalaVersion := "3.1.3"    // ange version av Scala-kompilatorn
\end{Code}
