...
\item Du kan komma åt de enskilda elementen med \Emph{\code{(0)}}, \Emph{\code{(1)}}, ...  \code{(n)}
...
\begin{REPL}
scala> def längd(p: (Double, Double)): Double = math.hypot(p(0), p(1))

scala> def vinkel(p: (Double, Double)): Double = math.atan2(p(1), p(0))
