...
\item En stämpel kan \Alert{tillverkas} -- motsvarar \Emph{deklaration} av klassen.
 \item Det händer inget förrän man \Alert{stämplar} -- motsvarar instansiering med \code{new}.
...
\item Ett sådant objekt kallas för en \Emph{instans} av \code{Klassnamn}.
...
\item I Java (m.fl. språk) får man \Alert{explicit} skriva kod för konstruktorer med speciell syntax och göra alla initialiseringar av attribut själv. Man får ha många olika konstruktorer (t.ex. i stället för defaultargument).

\item I Scala \Emph{kan} man också skriva egna alternativa s.k. \Emph{hjälpkonstruktorer}, men det är \Alert{ovanligt}, eftersom man har möjligheten med fabriksmetoder i kompanjonsobjekt och default-argument (saknas i Java, m.fl.).
\end{itemize}
\end{Slide}
...
\begin{itemize}
\item I många språk t.ex. Java finns inget motsvarande nyckelord \code{val} som garanterar oföränderliga attributreferenser.
% \footnote{Det finns visserligen \jcode{final} men det är annorlunda som vi ska se senare.}

\item Därför gör man i dessa språk nästan alltid gör alla attribut \Alert{privata} för att förhindra att de ändras på ett okontrollerat sätt.
