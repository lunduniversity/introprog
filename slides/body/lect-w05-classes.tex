...
Någon gång i \Alert{framtiden} kommer följande gälla:
\begin{itemize}
...
\end{Code}
\code{-source:future} aktiverar regler för framtida Scala-versioner\\ \code{-deprecation} varnar för sådant som kommer att så småningom tas bort.
\begin{REPLsmall}
...
\end{Code}
\item Varför? Enklare att använda från \Alert{Java}-kod jämfört med apply i kompanjonsobjekt. (Men om din Scala-kod inte ska användas från Java så är detta onödigt.)
