...
\item Parametrar som inte föregås av modifierare (t.ex. private val, val, var) blir \Emph{attribut} som bara är synliga i \Alert{denna} instans.
...
\end{Code}
\begin{REPLsmall}
> scala -source future // kör igång REPL med regler för framtida versioner
...
\begin{Slide}{Lösningar på initialiseringsproblem under konstruktion}
\begin{itemize}
  \item Ändra ordningen på attribut-deklarationer om möjligt
...
\end{itemize}
Om du \Alert{verkligen} behöver hantera ett oinitialiserat värde:
\begin{Code}
class Box:
  private var x: String = scala.compiletime.uninitialized // tydlig null-risk
  def get: String = if x != null then x else ""          // glöm ej null-koll
  def getOrElse(alt: String): String = if x != null then x else alt  
  def set(value: String): Unit = x = value
\end{Code}
Försök att \Alert{undvika} detta eftersom det ger stor risk för buggar!\\
{\SlideFontSmall (I ovan fiktiva exempel hade vi kunnat undvika detta enkelt genom att ge x startvärdet \code{""} i stället för null. En sådan lösing förutsätter att det finns en rimlig representation av ett saknat värde. Mer om hantering av saknade värden senare...)}
