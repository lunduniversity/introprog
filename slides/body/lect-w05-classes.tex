...
\tkzLabelSegment[below=5pt](O,A){\ifswedish\textit{real-delen} är $3$\else\textit{the real part} is $3$\fi}
\tkzLabelSegment[above left=5pt](O,B){\textit{r}}
\tkzLabelSegment[right=5pt](A,B){\ifswedish\textit{imaginär-delen} är $4$\else\textit{the imaginary part} is $4$\fi}
...
\begin{Slide}{Attribut i kompanjonsobjekt används för sådant som är gemensamt för alla instanser}
...
\item Du får ett \Alert{kompilator-genererat} \Emph{kompanjonsobjekt} med en \Emph{fabriksmetod} \code{apply} för indirekt instansiering där \Alert{alla} klassparametrarnas \code{val}-attribut initialiseras.
\pause
...
\begin{Slide}{Exempel: oföränderliga case-klassen \texttt{Point}}
...
\item I Scala \emph{kan} man också skriva egna s.k. \Emph{hjälpkonstruktorer} \Eng{auxilliary constructor}, men det är \Alert{ovanligt}, eftersom man har möjligheten med fabriksmetoder i kompanjonsobjekt och default-argument.
\end{itemize}
\end{Slide}
...
\item
En \Emph{setter} kan i Scala skapas med en procedur vars \Alert{namn slutar med} \texttt{\_=}
\pause
