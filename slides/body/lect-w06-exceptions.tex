...
\Subsection{Hantera undantag indirekt med \texttt{Try}}
...
\node [umlclass, rectangle split parts = 2, xshift=0cm, text width=4.2cm] (BaseType)  {
...
\node [umlclass, rectangle split parts = 2, text width=2.2cm]  at (-2.5cm,-4.2cm) (SubType1) {
            \textbf{\centerline{\TextBox{\code{Success[T]}}}}
            \nodepart[]{second} \TextBox{\code{val value: T}}
        };

\node [umlclass, rectangle split parts = 2, text width=4.5cm] at (2.5cm,-4.2cm) (SubType2)  {
...
\begin{Slide}{Hantera undantag som ett värde med \texttt{Try}}
...
\Subsection{Hantera undantag direkt med \texttt{try}-\texttt{catch}}


\begin{Slide}{Primitiva \texttt{try}-\texttt{catch}-uttryck}\SlideFontSmall
Man kan fånga undantag direkt med ett \code{try ... catch}-uttryck:
\begin{Code}
def carola = 
  try 
...
\begin{itemize}
\item Det finns en risk att hantering av undantag glöms bort.\\Vad händer då? \Alert{Pang!} -- det blir körtidsfel och tvärstopp :(
\item Det finns möjligheter i Scala att låta kompilatorn kontrollera om undantag hanteras med s.k. \Emph{kontrollerade undantag} \Eng{checked exceptions}
