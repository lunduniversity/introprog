...
\begin{itemize}\SlideFontTiny
  \item Exakt alla existerande värden i Scala är \Emph{objekt} som har en \Alert{typ}.
  \item Alla typer är subtyper till bastypen \code{Any}, och därför kallas \Emph{topptyp}.
  \item Typen \code{Nothing} är subtyp till alla typer, och kallas därför \Emph{bottentyp}. \\ Inget värde har enbart typen \code{Nothing}; alla objekttyper är mindre specifika än så.
  \pause
  \item En förenklad beskrivning av topptypen \code{Any} som ger alla objekt \code{toString} etc:
\begin{CodeSmall}
trait Any:
  // metoder som får överskuggas
  def toString: String                        // en strängrepresentation
  def equals(other: Any): Boolean = eq(other) // definierar likhet
  def hashCode: Int                           // ska ge samma heltal om equals true
  ...
  final def ## = hashCode                 // annorlunda för värdetyper och null
\end{CodeSmall}
\end{itemize}
...
\begin{Slide}{Alla typer är subtyper till \texttt{Any}}\SlideFontSmall
...
\node [umlclass, rectangle split parts = 1]  at (-2.5cm,-4.0cm) (SubType1) {
            \textbf{\centerline{\TextBox{\code{Gurka}}}}
            % \nodepart[]{second} \TextBox{\code{val x: A}}
        };

\node [umlclass, rectangle split parts = 1] at (2.5cm,-4.0cm) (SubType2)  {
            \textbf{\centerline{\TextBox{\code{Tomat}}}}
        };

\draw[umlarrow] (SubType1.north) -- ++(0,0.7) -| (Base.south);
\draw[umlarrow] (SubType2.north) -- ++(0,0.7) -| (Base.south);
\end{tikzpicture}
\end{center}
...
\begin{REPLnonum}
scala> val Point(a, b) = p      // plocka isär en punkt
val a: Int = 1
val b: Int = 2
\end{REPLnonum}
\pause \code{Point(a, b)} kallas ett \Emph{konstruktormönster}.\\ De godtyckliga namnen \code{a} och \code{b} blir nya variabler.\\ Det finns \emph{många} olika sorters mönster. \\
...
\end{Code}
\pause
\vspace{0.5em} Matchning mot specifika typer enl. ovan används i idiomatisk Scala hellre än \code{isInstanceOf} och \code{asInstanceOf} men man kan göra motsvarande ovan så här:
\begin{Code}
val i2: Int =  
  val x = f()
...
\end{REPLsmall}
\item Notera att \Alert{krullparentes behövs} vid \code{case} utan \code{match}.
