...
\item Partiella funktioner med \code{case} utan \code{match}:\\ \code|val pf: Int => Int = { case z if z != 0 => 1/z }| \\ Notera att klammerparenteserna behövs för att skapa partiell funktion med \code{case} utan \code{match}. Funktionen är inte definierad för argumentet \code{0}:
