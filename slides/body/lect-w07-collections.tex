...
\item Men man kan gå igenom element i \Emph{någon} ordning med till exempel \code{xs.map(f)} eller \code{for (x <- xs) yield f(x)}
...
scala> mängd.contains(-1)                     // samma som xs(-1)
\end{REPL}
