...
\item \Alert{Förändringsbar}: kan ändra elemententreferenserna
  \begin{itemize}
  \item Kan \Alert{ej ändra storlek} efter allokering: \\ \Emph{Array}: indexera och uppdatera varsomhelst
...
\begin{Slide}{Lämna det öppet: använd \texttt{Seq}}\SlideFontSmall
  Typen \Emph{\code{Seq}} är supertyp till alla sekvenssamlingar i \code{scala.collection}.
  ...
  \begin{Code}
  def copyIntSeq[Int](xs: Seq[Int]): Seq[Int] = for (x <- xs) yield x
  \end{Code}
  \end{itemize}
  \pause
  \begin{REPL}
