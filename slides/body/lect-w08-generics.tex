...
\end{Code}
\pause
\item \Alert{Skuggning kan förekomma}: Om \code{replaced} i \code{Cell} hade använt namnet A på sin typparameter hade den \Emph{skuggat} klassens typparameter, därav nytt namn \code{B} -- vore förvirrande att använda samma namn på olika typparametrar (jämför  namnöverskuggning vid \Emph{lokala} namn i nästlade block).
\end{itemize}
...
\begin{SlideExtra}{Om veckans övning: \code{matrices}}\SlideFontSmall
