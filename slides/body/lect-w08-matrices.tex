...
\emph{Övning:} Vad blir resultatet av förändringarna nedan i en array av arrayer ''på plats'' med tilldelning?
\begin{REPL}
scala> val xss = Array(Array(5,2,42,4,5),Array(3,4,18,6,7))
...
\begin{Slide}{Uppdatering av en förändringsbar nästlad struktur}
\emph{Lösning.} Så blir resultatet av förändringarna nedan i en array av arrayer ''på plats'' med tilldelning:
\begin{REPL}
scala> val xss = Array(Array(5,2,42,4,5),Array(3,4,18,6,7))
...
2 3 4 5 6
\end{REPL}
Övning: \\Vad ska det stå vid \code{???} för att alla element ska skrivas ut som ovan?
\end{Slide}
...
\vspace{1em} Övning: implementera 
\begin{Code}
def isEqual(xss: Array[Array[Int]], yss: Array[Array[Int]]): Boolean = ???
\end{Code}
...
\begin{Slide}{Kolla likhet mellan två heltalsmatriser (uppfinner hjulet)}
\ifkompendium 
Algoritmer som går igenom nästlade strukturer behöver göra nästlade repetitioner, till exempel en while-loop inuti en while-loop. 

Algoritmen nedan jämför innehållet i två array-matriser med nästlade while-loopar och avslutar itereringen så fort en olikhet hittats. Notera while-villkoren och användningen av den booelska variabeln \code{foundUnequal}.
\fi 
\begin{CodeSmall}
def isEqual(xss: Array[Array[Int]], yss: Array[Array[Int]]): Boolean = 
  if xss.length != yss.length then false else
...
\ifkompendium\else
\begin{Slide}{Träna på matriser med veckans uppgifter:}\SlideFontSmall
Övning \code{matrices}:
\begin{itemize}
\item Träna på att iterera över nästlade strukturer
...
\item Extrauppgift där du ska bygga ett yatzy-spel i terminalen\\(kunde varit en tentauppgift...)
