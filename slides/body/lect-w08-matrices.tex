...
\begin{Slide}{Använd \code{deep} för test av innehållslikhet på arrayer}

  I Scala kan du använda metoden \code{deep} på arrayer innan du jämför med \code{==}
...
\end{REPL}
Funkar fint även på motsvarande nästlade strukturer: \\ \code{xss.deep == yss.deep} \\~\\ \pause
{\small I Java finns följande hjälpmetod för innehållstest på arrayer med primitiva typer \code{java.util.Arrays.equals(xs, ys)} och för referenstyper finns \code{java.util.Arrays.deepEquals(xs, ys)}}
\end{Slide}
