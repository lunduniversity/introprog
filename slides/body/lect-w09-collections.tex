...
\begin{Slide}{Ändra på plats i förändringsbar samling}\SlideFontSmall
Några speciell metoder som ändra \Alert{på plats}:
\begin{itemize}
\item \code{xs.addOne(x).addAll(ys)}
\item returnerar \code{this} så att du kan kedja flera anrop
\item \code{xs.filterInPlace(p)} filtrerar genom att ta bort i befintlig samling
\item \code{xs.mapInPlace(f)} transformerar element i befintlig samling
\end{itemize}
Exempel:
\begin{REPLsmall}
scala> case class Person(name: String, age: Int)

scala> val xs = collection.mutable.ArrayBuffer.empty[Person]

scala> xs.addOne(Person("Björn", 58)).addAll(Seq(Person("Ken", 25), Person("Kim", 45)))

scala> xs.filterInPlace(_.age > 50).mapInPlace(p => p.copy(name = p.name.reverse))

scala> println(xs)
ArrayBuffer(Person(nröjB,58))
...
\item En sekvens är en \Emph{följd av element} som
  \begin{itemize}
   ...
   \item alla är av samma \Alert{typ} (t.ex. heltal).
  \end{itemize}
  \pause
...
\item Metoden \texttt{to} tar ett \Emph{kompanjonsobjekt} ur samlingsbiblioteket som argument och kan användas för konvertering till godtycklig samlingstyp.
...
\end{REPLnonum}
\pause
\item En mängd är \Alert{inte}  en sekvens: du kan \Alert{inte} utgå från att elementen ligger i någon viss ordning, t.ex. den ordning som de ges vid konstruktion; en mängd har ej längd, men en \Emph{storlek}; metoden \code{size} ger antalet element men metoden \code{length} saknas.
...
\end{REPLsmall}
\code{addOne}, \code{addAll}, \code{subtractOne}, \code{subtractAll}, \code{filterInPlace} returnerar \code{this} så du kan ändra på plats med \Emph{kedjade anrop} med punktnotation.
\end{Slide}
...
\begin{itemize}
\item En \Emph{nyckel-värde-tabell} är en samling av \Alert{par} med 
(\Emph{nyckel}, \Emph{värde}).
...
\end{REPL}
\item \code{Map} har \Alert{två} typparametrar: \code{K} för nyckeltypen och \code{V} vår värdetypen.
...
\begin{Slide}{Fördjupning: groupMap, groupMapReduce}\SlideFontSmall
  Användbara men avancerade metoder som körs i \Alert{en enda} iteration:
  \begin{itemize}
    \item \code{xs.groupMap(f)(g)}\\ger Map grupperad enl. f med värden beräknade enl. g 
    \item \code{xs.groupMapReduce(f)(g)(h)}\\ger Map efter en groupMap(f)(g) som reduceras enl. h 
    \item Läs om \code{groupMap} här:\\
    \url{https://www.scala-lang.org/blog/2018/06/13/scala-213-collections.html}
    \item Se visualisering av \code{groupMapReduce} här:\\
    \url{https://superruzafa.github.io/visual-scala-reference/groupMapReduce/}
  \end{itemize}
\end{Slide}
...
Gör gärna detta \Alert{lokalt} i en \Alert{förändringsbar} samling och returnera sedan en \Emph{oföränderlig} samling, genom att köra t.ex. \code{toSet}, \code{toMap}, \code{toVector} på förändringsbar samling.\\~\\
