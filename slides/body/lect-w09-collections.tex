...
Med \code{iterator} kan man iterera med \code{while}, men endast \Alert{en   gång}; sedan är iteratorn ''förbrukad''. (Men man kan be om en ny.) Används för ''under huven'' i samlingsbiblioteket för att implementera andra metoder.
\begin{REPL}
...
\end{REPL}
I veckans uppgifter används detta mönster i en s.k. \Emph{builder}: \\ Först bygga upp en förändringsbar struktur i \code{FreqMapBuilder} steg för steg,  
och sedan, när alla tillägg är gjorda, övergå till oföränderlig struktur \code{Map[String, Int]}. 
\end{Slide}
