...
%!TEX root = ../lect-w09-dod.tex

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\Subsection{Bakgrund}

\begin{Slide}{Vad är versionshantering?}
...
\begin{Slide}{Versionshanteringssystem}
...
\begin{Slide}{Typer av versionshanteringssystem (1/2)}
...
\begin{Slide}{Centraliserad versionshantering}

    \begin{center}
        \includegraphics[width=0.80\textwidth]{../img/git/fig1_central_model.png}
    \end{center}
\end{Slide}

\begin{Slide}{Typer av versionshanteringssystem (2/2)}
...
\begin{Slide}{Distrubuerad versionshantering}

    \begin{center}
        \includegraphics[width=\textwidth]{../img/git/fig2_git_model.png}
    \end{center}
\end{Slide}

\Subsection{Git och Github}

\begin{Slide}{Introduktion till Git}
    ...
    \blankline
    \ti{Klona def officiella Git repot: \code{git clone git@github.com:git/git.git}}
    \ti{Testa: \code{git rev-list --max-parents=0 HEAD}}

\end{Slide}

\begin{Slide}{Git vs. GitHub: Vad är skillnaden?}
...
\begin{Slide}{Grundläggande Git-kommandon}
\SlideFontSmall
    \begin{itemize}
        \ii{\texttt{git init}\\ Initierar ett nytt Git-repository lokalt.}
        \ii{\texttt{git clone}\\ Kopierar ett existerande repository (t.ex. från GitHub) till din dator.}
        \ii{\texttt{git add}\\ Lägger till ändringar i staging-området inför nästa commit.}
        \ii{\texttt{git commit}\\ Sparar ändringar lokalt med en beskrivande kommentar.}
        \ii{\texttt{git push}\\ Laddar upp dina lokala ändringar till ett fjärrrepository (t.ex. GitHub).}
        \ii{\texttt{git pull}\\ Hämtar och integrerar ändringar från ett fjärrrepository.}
    \end{itemize}

\end{Slide}


\begin{Slide}{Git-modellen}
\begin{itemize}
\ii{\emph{Workspace}: En vanlig mapp på datorn där du jobbar med filer.}
\ii{\emph{Staging Area}: En \enquote{kopia} av workspace där du förbereder ändringar inför nästa sparning.}
\ii{\emph{Repository}: Ett arkiv som lagrar alla tidigare versioner av projektet.}
\end{itemize}

\ifkompendium
\includegraphics[width=0.75\textwidth]{../img/git/model-add-commit.png}
\else
\begin{center}
\includegraphics[height=.5\textheight]{../img/git/model-add-commit.png}
\end{center}
\fi
\end{Slide}

\begin{Slide}{Vad är en Commit?}

    \begin{minipage}{0.7\textwidth}
        \begin{itemize}
            \ii{En \emph{commit} är en \enquote{ögonblicksbild} (\emph{snapshot}) av projektet vid ett specifikt tillfälle.}
    ...
    \begin{minipage}{0.20\textwidth}
        \begin{center}
            \begin{tikzpicture}
                ...
                \draw[thick] (0, 0) -- (0, 3);
                \node[above, yshift=2pt] at (0, 3) {\emph{main}};
                ...
                % Label the commits
                \node[right, xshift=5pt] at (0, 3) {Commit 4};
                \node[right, xshift=5pt] at (0, 2) {Commit 3};
                \node[right, xshift=5pt] at (0, 1) {Commit 2};
                \node[right, xshift=5pt] at (0, 0) {Commit 1};
            \end{tikzpicture}
...
\begin{Slide}{Grenar (\emph{Branches})}

    \begin{minipage}{0.7\textwidth}

        \ti{Man kan skapa parallella utvecklingsgrenar, som sedan kan slås samman (\emph{merge}) med huvudgrenen igen. Punkterna i figuren är individuella commits.}
...
\begin{Slide}{Varför grenar (\emph{branches})?}
...
\begin{Slide}{Vilka problem kan uppstå?}
...
\begin{Slide}{Exempel: Merge-konflikt i källkoden}
\SlideFontSmall
    ...
    \begin{itemize}\SlideFontTiny
      \ii{Konflikten visas med \verb|<<<<<<<|, \verb|=======|, och \verb|>>>>>>>|.}
        \ii{Du måste välja vilken version som ska behållas eller kombinera dem manuellt.}
    \end{itemize}

\end{Slide}


\begin{Slide}{Ett större exempel}
...
\Subsection{Arbeta distribuerat med Git}

\begin{Slide}{Arbeta distribuerat med Git}\SlideFontSmall

    \begin{itemize}
        \ii{\emph{Clone} (klona): Kopiera ett repository från internet till din dator.}
        \ii{\emph{Pull} (dra ner): Hämta ändringar från ett fjärrrepository.}
        \ii{\emph{Push} (trycka upp): Skicka dina ändringar till ett fjärrrepository.}
        \ii{\emph{Pull request} (ändringsbegäran): Föreslå ändringar till ett repository.}
...
\ifkompendium
\includegraphics[width=.75\textwidth]{../img/git/model-rm-reset.png}
\else
\begin{center}
\includegraphics[height=.75\textheight]{../img/git/model-rm-reset.png}
\end{center}
\fi
\end{Slide}

\begin{Slide}{Local vs remote: commit och push}
\ifkompendium
\includegraphics[width=1.0\textwidth]{../img/git/local_remote_commit_push.png}
\else
\begin{center}
\includegraphics[width=.9\textwidth]{../img/git/local_remote_commit_push.png}
\end{center}
\fi

\end{Slide}

\begin{Slide}{Local vs remote: fetch och merge}
\ifkompendium
\includegraphics[width=1.0\textwidth]{../img/git/local_remote_fetch_merge.png}
\else
\begin{center}
\includegraphics[width=.9\textwidth]{../img/git/local_remote_fetch_merge.png}
\end{center}
\fi
\end{Slide}

\begin{Slide}{Båda samtidigt!? -- push}
\ifkompendium
\includegraphics[width=\textwidth]{../img/git/local_remote_push_conflict.png}
\else
\begin{center}
\includegraphics[width=\textwidth]{../img/git/local_remote_push_conflict.png}
\end{center}
...
\begin{Slide}{Båda samtidigt!? -- fetch}

    \begin{center}
        \includegraphics[width=\textwidth]{../img/git/local_remote_fetch_conflict.png}
    \end{center}

\end{Slide}

\begin{Slide}{Båda samtidigt!? -- merge efter fetch}

    \begin{center}
        \includegraphics[width=\textwidth]{../img/git/local_remote_fetch_merge_conflict.png}
    \end{center}

\end{Slide}


\begin{Slide}{Merge-situationer}
...
\begin{Slide}{Fast-forward merge}
\ifkompendium
\includegraphics[width=\textwidth]{../img/git/merge_case_ff.png}
\else
\vspace{-0.5em}
\begin{center}
\begin{tikzpicture}
    % Place the image at 50% scale
    \node (img) at (0, 0) {\includegraphics[scale=0.45]{../img/git/merge_case_ff.png}};
...
\begin{Slide}{Vanlig merge -- skapar en \enquote{merge commit}}
\ifkompendium
\includegraphics[width=\textwidth]{../img/git/merge_case_commit.png}
\else
    \begin{center}
        \begin{tikzpicture}
            % Place the image at 50% scale
            \node (img) at (0, 0) {\includegraphics[scale=0.45]{../img/git/merge_case_commit.png}};

            % Only show partial overlays in the presentation mode
            \mode<beamer>{
                % Overlay 1: Cover the lower 2/3 of the image with a white box
                \only<1>{
                    \fill[white] ([yshift=-0.30\textheight]img.north west) rectangle (img.south east);
                }
                % % Overlay 2: Cover the lower 1/3 of the image with a white box
                \only<2>{
...
\begin{Slide}{Rebase}
\ifkompendium
\includegraphics[width=\textwidth]{../img/git/merge_case_rebase.png}
\else
    \vspace{-0.8em}
    \begin{center}
        \begin{tikzpicture}
            % Place the image at 50% scale
            \node (img) at (0, 0) {\includegraphics[scale=0.45]{../img/git/merge_case_rebase.png}};

            % Only show partial overlays in the presentation mode
            \mode<beamer>{
                % Overlay 1: Cover the lower 2/3 of the image with a white box
                \only<1>{
                    \fill[white] ([yshift=-0.16\textheight]img.north west) rectangle (img.south east);
                }
                % % Overlay 2: Cover the lower 1/3 of the image with a white box
                \only<2>{
                    \fill[white] ([yshift=-0.42\textheight]img.north west) rectangle (img.south east);
                }
                % % Overlay 3: Cover the lower 1/3 of the image with a white box
                \only<3>{
                    \fill[white] ([yshift=-0.67\textheight]img.north west) rectangle (img.south east);
                }
...
\Subsection{Om veckans laboration}

\begin{Slide}{Veckans laboration}
    ...
    \halfblankline\noindent
    \ti{Glöm inte att installera Git på din egen dator.}

\end{Slide} 

\begin{Slide}{Rekommenderad läsning}
    \begin{itemize}
        %\ii{Läs avsnittet om Git i Scalakompendiet (av Björn Regnell).}
        \ii{Se avsnitt 1.1, 1.2, 1.3, 1.6 samt 1.7 av youtube-serien \enquote{Git and GitHub for Poets}.}
        \ii{Läs kapitel 1, 2 och 3.1--3.2 i Pro Git-boken (av Chacon \& Straub).}
