...
\ifkompendium\noindent Exempel: En gurka är en grönsak. En tomat är en grönsak. Det går att skala alla grönsaker. \fi
\end{Slide}
...
Typen \textit{\textbf{\ifswedish\texttt{Grönsak}\else\texttt{Vegetable}\fi}} är en \Emph{bastyp} i nedan arvshierarki:
...
\begin{center}
\newcommand{\TextBox}[1]{\raisebox{0pt}[1em][0.5em]{#1}}
\tikzstyle{umlclass}=[rectangle, draw=black,  thick, anchor=north, text width=3.4cm, rectangle split, rectangle split parts = 3]
\begin{tikzpicture}[inner sep=0.5em]
...
\end{itemize}
\item Scala 3 varnar vid arv från klass utanför samma kodfil (mer om s.k. öppna klasser senare).
\end{itemize}
\end{Slide}


\begin{Slide}{Klasser, arv och klassparametrar}\SlideFontTiny
Klasser, singelobjekt och traits kan ärva andra klasser och traits med \code{extends}.\\Om supertypen har parametrar så \Alert{måste} subtypen ge argument efter \code{extends}.
...
\begin{Code}
    var p: Person = Forskare("Robin Smith", "Lund", "Professor")
\end{Code}
\begin{itemize}\SlideFontTiny
...
\pause
\item Den \Emph{dynamiska typen}, även kallad \Emph{körtidstypen} som gäller vid exekvering kan vara mer specifik: den dynamiska typen för \code{p} är nu efter ovan tilldelning Student, men också \ifswedish Akademiker\else Academic\fi{} och Person.
...
\else
\begin{Slide}{Inmixning}\SlideFontTiny
Man kan ärva \Alert{flera} traits. Detta kallas \Emph{inmixning} \Eng{mix-in} och görs med en komma-separerad typ-lista efter \code{extends}.
...
\begin{Slide}{Körtidstyp vid inmixning}\SlideFontTiny
...
\noindent Den allra enklaste (och mest meningslösa) anonyma klassen är:
\begin{REPLsmall}
scala> new {}
val res0: Object = anon1@5bb37371
\end{REPLsmall}
Anonyma klasser är användbara när man vill implementera medlemmar utan att behöva ge subtypen ett namn.
\end{Slide}
...
Nyckelordet \code{abstract} behövs framför \code{class} om abstrakta medlemmar finns:
\begin{REPLsmall}
...
\item ...du vill korta ned tiden för omkompilering vid ändringar om \emph{mycket stor} kodbas.
\end{itemize}
\end{multicols}
\end{Slide}
