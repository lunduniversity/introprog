...
\begin{itemize}
  \item \ifswedish med \code{T <: U} blir \code{U} en övre gräns \Eng{upper bound} för \code{T}\\\code{U} är ''högsta'' möjliga typ som \code{T} får vara (jämför ''mindre eller lika med'')\else with \code{T <: U}, \code{U} becomes an upper bound for \code{T}\\\code{U} is the ''highest'' possible type that \code{T} can be (compare ''less than or equal to'')\fi
  \item \ifswedish med \code{T >: L} blir \code{L} en undre gräns \Eng{lower bound} för \code{T}\\\code{L} är ''lägsta'' möjliga typ som \code{T} får vara  (jämför ''större eller lika med'')\else with \code{T >: L}, \code{L} becomes a lower bound for \code{T}\\\code{L} is the ''lowest'' possible type that \code{T} can be (compare ''greater than or equal to'')\fi
...
\begin{Slide}[shrink]{Kovarians \Eng{covariance}}
\begin{itemize}\SlideFontSmall
...
\end{Code}
\else
\item[] \ifswedish\code|enum Toalett[+T] { case Upptagen(x: T); case Ledig }|\else\code|enum Toilet[+T] { case Occupied(x: T); case Vacant }|\fi
\fi 
\end{itemize}
...
case class Bur[+A](a: A): 
  def bytTill(x: A): Bur[A] = Bur(x)     // fel!
\end{CodeSmall}
\pause
\begin{REPLsmall}
-- Error:
6 |  def bytTill(x: A): Bur[A] = Bur(x)
  |          ^^^^
...
\begin{Slide}{Intersektionstyp med och-operator}\SlideFontSmall
...
object Gurka extends Grönsak(42), HarSmak("vattnig")
...
\begin{Slide}{Transparenta typer}\SlideFontSmall
...
\begin{Slide}{Api-design med Scala}
\begin{itemize}\SlideFontSmall
...
scala> import EnAnnanNamnrymd.given    //importerar alla givna värden i EnAnnanNamnrymd
...
\item \code{given} och \code{import given ...} i aktuell namnrymd \Eng{current namespace} 
...
\end{itemize}
\begin{CodeSmall}
//en typklass för tolkning och omvandling av strängar till godtycklig typ 
trait Parser[T]: 
...
1 |xs.sorted
  | ^
  | No given instance of type Ordering[Gurka] was found
\end{REPLsmall}
\pause
...
\end{Code}
  ...
  \item \code{fromLessThan} returnerar en \code{Ordering} som du kan låta vara given.
