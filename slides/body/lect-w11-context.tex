...
\begin{itemize}\SlideFontSmall
\item Vi skulle kunna använda ett default-värde som är en variabel:
\begin{REPLsmall}
scala> var ctx = 42
var ctx: Int = 42
...
\end{itemize}
\begin{REPLsmall}
scala> def parseOrElse[T](s: String)(using p: Parser[T]) = p.fromString(s)

scala> parseOrElse[Int]("12")
...
\begin{REPL}
scala> parseOrElse[java.awt.Color]("Color(120,10,0)")
-- Error:
1 |parseOrElse[java.awt.Color]("Color(120,10,0)")
  |                                        ^
  | no implicit argument of type Parser[java.awt.Color] was found 
  | for parameter p of method parseOrElse
\end{REPL}
\begin{Code}
...
\begin{Code}
def parseOrElse[T](s: String)(using Parser[T]) = 
...
% \begin{CodeSmall}
% def parseOrElse[T](s: String)(using Parser[T]) = Parser[T].fromString(s)
% \end{CodeSmall}
...
%   extension (s: String) 
%     def parseOrElse(default: T): T = fromString(s).getOrElse(default)
