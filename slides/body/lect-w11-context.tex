...
scala> scala> val g = f(1)  // en ny funktion utan default-arg
...
\end{enumerate}
\item Om flera givna värden kan framkallas för flera typer som ingår i en gemensam arvhierarki så väljer kompilatorn det givna värdet som är av den \emph{mest specifika} typen.
\end{itemize}
\end{Slide}

\begin{Slide}{Ad hoc polymorfism, ''typklasser''}\SlideFontSmall
\begin{itemize}\SlideFontSmall
\item Kan göras i Scala genom att kombinera parametrisk polymorfism med kontextuella abstraktioner.
\item \TODO \code{Ordning[T]} 
\end{itemize}
\end{Slide}
