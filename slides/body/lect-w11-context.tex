...
\begin{itemize}
  ...
  \item \ifswedish med \code{T >: L} blir \code{L} en undre gräns \Eng{lower bound} för \code{T}\\\code{U} är ''lägsta'' möjliga typ som \code{T} får vara  (jämför ''större eller lika med'')\else with \code{T >: L}, \code{L} becomes a lower bound for \code{T}\\\code{U} is the ''lowest'' possible type that \code{T} can be (compare ''greater than or equal to'')\fi
