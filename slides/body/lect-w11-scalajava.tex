...
\begin{Slide}{Vad gör en kompilator?}
...
\begin{minipage}{.5\textwidth}
  Kompilatorns stegvisa resultat:
  \begin{itemize}\SlideFontSmall
    \item \Emph{Syntaktisk analys}: textens \Alert{utformning} kontrolleras enligt en väldefinierad grammatik.
    \item En \Emph{datastruktur} skapas som representerar kodens olika delar; ett s.k. abstrakt syntaxträd \Emph{AST}.
    \item \Emph{Semantisk analys}: kodens \Alert{betydelse} kontrolleras i flera steg enligt språkets alla regler.
    \item \Emph{Maskinkod} genereras, smarta \Alert{optimeringar} görs.
  \end{itemize}
\end{minipage}%

\pause\vspace{1em}\SlideFontSmall Med statiskt typade språk (Scala, Java, C\#, C++, ...) kan fler kontroller ske under kompilering, jmf m dynamiskt typade språk (Python, Javascript, Clojure, Matlab, ...). I de senare släpps många typer av buggar vidare till körtid.
\end{Slide}
...
\item Statisk typning: ger snabb maskinkod, kompilatorn kan ge stöd vid förändring av kod (s.k. refactoring) och hittar vissa buggar redan vid kompilering
\end{itemize}
...
\begin{Slide}{Exempel från tidigare vecka: Scala-klassen Complex}
...
\begin{Slide}{Hjälp att använda Java-samlingar i Scala med \texttt{JavaConverters}}\SlideFontSmall
