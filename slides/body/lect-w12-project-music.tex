...
\item Symbolen ''$\flat$'' (skrives \code{b} i ascii) uttalas \emph{ess} representerar en sänkning med ett halvt tonsteg.
...
\item Case-klassen \code{Pitch(number: Int)} representerar en \Emph{tonhöjd} där \code{number} är i intervallet \code{(0 until 128)}.
\item Metoderna \code{pitchClass} och \code{octave} i \code{Pitch} returnerar tonklassen respektive oktaven av en tonhöjd.
\item En tonhöjd namnges med tonklassnamnet+oktaven, t.ex. \code{"A4"}.
...
\item Exempel: om en sträng är stämd i D3 blir tonen om man trycker ned fingret på fjärde bandet G$\flat$3.
...
\url{https://github.com/lunduniversity/introprog/blob/master/workspace/w13_music_proj/src/main/scala/music/Pitch.scala}
\end{Slide}
...
\SlideFontTiny
Se implementationer här:
\url{https://github.com/lunduniversity/introprog/blob/master/workspace/w13_music_proj/src/main/scala/music/Chord.scala}
\end{Slide}
...
\node [umlclass, rectangle split parts = 2, xshift=-3cm, text width=5cm] (BaseType)  {
            ...
            \nodepart[]{second}
            \TextBox{\code{def toChordOption: Option[Chord]}}%\vspace{-0.25em}\newline
...
\node [umlclass, rectangle split parts = 2, xshift=0cm, text width=4.5cm] at (0cm,-2.2cm)  (Fretted)  {
            ...
            \nodepart[]{second}
            \TextBox{\code{def numberOfStrings: Int}}\vspace{-0.25em}\newline
