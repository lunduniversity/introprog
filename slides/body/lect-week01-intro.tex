...
{Minnet innehåller endast \Alert{heltal} som \newline representerar \Emph{data} och \Emph{instruktioner}.};
...
\begin{Slide}{Virtuell maskin == abstrakt hårdvara}
\begin{multicols}{2}

En virtuell maskin är en \Emph{abstraktion av en dator} med en generell ''maskinkod'' som sedan översätts under körning till den verkliga maskinen. \\
\vspace{1em}
Källkoden blir då \Emph{plattformsoberoende} och fungerar på många olika maskiner.
...
\node (interp) [process, below of=output] {Interpretator};
...
\Subsection{Programmeringsbyggstenar: literaler, uttryck, satser}
