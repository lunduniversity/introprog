...
\begin{Slide}{Verktyg}
Att kunna verktygen väl är grunden för hög produktivitet.
\begin{itemize}
\item Scala \Emph{REPL}: från övn 1
\item Integrerad utvecklingsmiljö (IDE), \Emph{Kojo}: från lab 1
\item Kompilera med \Emph{\code{scalac}} och \Emph{\code{javac}}: från övn 2
\item \Emph{Texteditor} för kod: från övn 2
\item Byggverktyg, \Emph{\code{sbt}}: från övn 3, lab 2
\item \Emph{Eclipse} med plugin \Emph{ScalaIDE}: från W04
\item \Emph{jar}
\item \Emph{javadoc} och \Emph{scaladoc}
\end{itemize}
\end{Slide}
