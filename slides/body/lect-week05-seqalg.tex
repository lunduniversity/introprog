...
\Subsection{SEQ-INSERT/REMOVE}
\begin{Slide}{Insättning/borttagning till ny kopia}
\vspace{-0.6em}\scalainputlisting[numbers=left,numberstyle=,basicstyle=\fontsize{6.5}{8}\ttfamily\selectfont]{../compendium/examples/workspace/w05-seqalg/src/pointSeqUtils.scala}
\end{Slide}
...
\begin{Slide}{Exempel: SEQ-INSERT/REMOVE}
...
Om vi inte vill ''uppfinna hjulet'' och inte använda \code{patch} kan vi göra så här: \\Använd \code{take} och \code{drop} tillsammans med \code{:+} och \code{++} och göra sedan insättningen generiskt användbar för alla sekvenser:
\begin{REPLnonum}
