...
%\title[Föreläsning, \CourseCode pgk, Björn Regnell, senast uppdaterad: \today]{Vecka \vecka. \veckotema}
%\subtitle{Programmering, grundkurs}
\title{Programmering, grundkurs}
\author{Björn Regnell}
%\institute{Datavetenskap, LTH, Lunds universitet}
%\date{\CourseCode, Lp1-2, HT \CurrentYear}
\date{}
...
\node (input) [startstop] {\ifswedish Källkod\else Source code\fi};
\node (compile) [process, below of=input] {\ifswedish Kompilator\else Compiler\fi};
\node (output) [startstop, below of=compile] {\ifswedish Generell ''maskinkod''\else Generic ''machine code''\fi};
\node (interp) [process, below of=output] {\ifswedish Virtuell Maskin\else Virtual Machine\fi};
\node (output2) [startstop, below of=interp] {\ifswedish Specifik maskinkod\else Specific machine code\fi};
\draw [arrow] (input) -- (compile);
\draw [arrow] (compile) -- (output);
