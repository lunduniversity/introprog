...
case class Text(source: String):
  ...
  lazy val wordFreq: Map[String, Int] = ???  // use FreqMapBuilder

  def ngrams(n: Int): Vector[Vector[String]] = ???  // use sliding

  lazy val bigrams: Vector[(String, String)] =
    ngrams(2).map(xs => (xs(0), xs(1)))

  lazy val followFreq: Map[String, Map[String, Int]] = ??? //nested table
  ...
  lazy val follows: Map[String, String] =
    ...
    //or shorter with the same result? (easier or harder to read?)
