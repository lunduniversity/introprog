...
abstract class SnakeGame(settings: Settings) extends introprog.BlockGame(
  ...
  // examples of different visibility (discussing the choice of visibility based on usage)
  ...
  def enterStartingState(): Unit = ??? //press space to start

  def enterPlayingState(): Unit = ??? //sudden, for each entity: reset & draw
  ...
  def randomFreePos(): Pos = 
    ??? // draw random pos until empty spot, used by fruit, monsters
  ...
  final def start(ps: Player*)(es: Entity*): Unit = 
    ...
    isPaused = false
    pixelWindow.show()  // enables restart even if the window is closed...
    enterStartingState()
